...
import SeymourEight.Certificates.BSevenKThree.RSeven.XThree.Capacity
import SeymourEight.Certificates.BSevenKThree.RSeven.XThree.SharpKing
import SeymourEight.Certificates.BSevenKThree.RSeven.XThree.SmallDefs
...
namespace SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Assembly
...
theorem rUnreached_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    rUnreached (graphBits G L) = true := by
  ...
  · rw [all_eq_true_iff]
    ...
    rw [haAdd]
    rw [SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.hArc_coreBits
      G.Adj _ _ _ a 6 (by omega) (by omega)]
    have hn := BSixKThreeCoreGraphBridge.A1_not_adj_R G C hG
      (L.a ⟨a + 1, by omega⟩).1 (L.a 7).1 (by
        have heq : (⟨a + 1, by omega⟩ : Fin 8) = ⟨a + 1, by omega⟩ := rfl
        rw [heq]
        exact L.a_aOne ⟨a, ha⟩) L.a_r
    simp [hn]
  · rw [all_eq_true_iff]
    intro p hp
    rw [SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.pToH_coreBits
      G.Adj _ _ _ p 6 hp (by omega)]
    have hn := BSixKThreeCoreGraphBridge.P_not_adj_R G C
      (L.p ⟨p, hp⟩).1 (L.a 7).1 (L.p ⟨p, hp⟩).2 L.a_r
...
theorem inactiveZZero_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  rw [all_eq_true_iff]
  intro p hp
  rw [SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.pToZ_coreBits_inactive
...
theorem pMinimumDegree_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hG : G.IsOriented) (hPB : C.P = C.B)
    (hHCard : C.H.card = 6)
    (hMin : ∀ v, 8 ≤ G.outdegree v) (hzCount : zCount ≤ 6) :
...
theorem degreeThreeTie_all_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hk : C.k = 3) (hPB : C.P = C.B) (hzLe : zCount ≤ 6) :
...
theorem hallCondition_all_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hNoSeymour : ¬G.HasSeymourVertex) (hzLe : zCount ≤ 6) :
...
theorem pSecondPCount_le_qCount {zCount : Nat} (C : G.LocalConfiguration)
  ...
  have hMiddle : labelledVertex G L middle ∈ C.P ∪ C.H := by
    ...
    · have hmZero : middle ≠ 0 := by
      ...
      · by_cases hmR : middle = 7
        · subst middle
          exact (BSixKThreeCoreGraphBridge.P_not_adj_R G C
            (L.p ⟨p, hp⟩).1 (L.a 7).1 (L.p ⟨p, hp⟩).2 L.a_r
              (by simpa [labelledVertex] using hFirstGraph)).elim
        · have hIdx : (⟨middle, hmA⟩ : Fin 8) =
...
theorem individualEffectiveLower_graph (C : G.LocalConfiguration)
    ...
    (individualEffectiveLower 5 (graphBits G L) p).toNat ≤
  ...
  change (individualEffectiveLower 5 bits p).toNat ≤ U.card
  simp only [individualEffectiveLower, individualEffectiveLowerFive]
  rw [hMBV, hSBV]
...
theorem pEffectiveCondition_all_true (C : G.LocalConfiguration)
    ...
    (hmBound : 35 - edgeCount G C.P (externalTargets G C) ≤ 14) :
    all 7 (pEffectiveCondition 5 (graphBits G L)) = true := by
  rw [all_eq_true_iff]
  intro p hp
  let v := (L.p ⟨p, hp⟩).1
  have hvP : v ∈ C.P := (L.p ⟨p, hp⟩).2
  ...
  have hNatural : (pSecondPCount 5 (graphBits G L) p).toNat +
      (individualEffectiveLower 5 (graphBits G L) p).toNat + 1 ≤
    ...
    · have hEquation := EpsilonOneRootCoreGraphBridge.rootNeighborhoodEquation
      ...
      have hTable : (individualEffectiveLower 5 (graphBits G L) p).toNat ≤ 8 := by
  ...
  change ((pSecondPCount 5 (graphBits G L) p).toNat +
      (individualEffectiveLower 5 (graphBits G L) p).toNat + 1) % 256 ≤
    ((pOut (graphBits G L) p).toNat +
      2 * (pHOut (graphBits G L) p).toNat +
  ...
  have hRightSmall : (pOut (graphBits G L) p).toNat +
    ...
    have hhLe : (pHOut (graphBits G L) p).toNat ≤ 6 := by
      rw [hH]
...
theorem commonCore_true (C : G.LocalConfiguration) (L : Labels G 5 C)
    ...
    (hA1Card : C.A1.card = 3) (hHCard : C.H.card = 6) :
    commonCore 5 (graphBits G L) = true := by
  let bits := graphBits G L
  ...
  have hR : rUnreached bits = true := rUnreached_true G C L hG
  ...
  have hMissing : (externalMissing 5 bits).ule 12 = true :=
    externalMissing_five_le_twelve bits hOrA hOrP hAMin hPMin hDual
  have hmNat : 35 - edgeCount G C.P (externalTargets G C) ≤ 14 := by
    ...
    omega
  have hPEff : all 7 (pEffectiveCondition 5 bits) = true :=
    pEffectiveCondition_all_true G C L hG hPB hMin hNoSeymour
      hRootDegree hHCard hmNat
  have hSharp : sharpKing 5 bits = true := sharpKing_of_orientedP 5 bits hOrP
  change commonCore 5 bits = true
  simp only [commonCore, hOrA, hOrP, hOrPH, hX, hR, hZ, hAMin, hANon,
    hPMin, hPEff, hHall, hTie, hDegreeThree.1, hDegreeThree.2,
    hDual, hSharp, Bool.and_self]

end SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Assembly
