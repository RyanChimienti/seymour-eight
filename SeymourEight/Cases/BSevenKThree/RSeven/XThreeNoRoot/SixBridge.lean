...
import SeymourEight.Certificates.BSevenKThree.RSeven.XThree.Six

set_option linter.style.header false

namespace SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.SixBridge
...
theorem impossibleAt
    ...
    (hx : C.x = 3) (hExternalCard : (externalTargets G C).card = 6) : False := by
  ...
  have hHCard : C.H.card = 6 := by
    ...
    omega
  let L := Labels.canonicalLabels G 6 C hPCard hACard hA1Card hXCard hRCard
    hExternalCard
...
theorem impossible
    ...
    (hRootDegree : G.outdegree C.s = 8) (hPivot : IsMinimalPivot G C)
    (hBCard : C.B.card = 7) (hk : C.k = 3) (hr : C.r = 7)
    (hx : C.x = 3) (hNoRoot : epsilonS G C = 0) (hz : C.z = 6) : False := by
...
theorem impossibleRoot
    ...
    (hRootDegree : G.outdegree C.s = 8) (hPivot : IsMinimalPivot G C)
    (hBCard : C.B.card = 7) (hk : C.k = 3) (hr : C.r = 7)
    (hx : C.x = 3) (hRoot : epsilonS G C = 1) (hz : C.z = 5) : False := by
  apply impossibleAt G hBound C hG hMin hNoSeymour hRootDegree hPivot hBCard hk hr hx
  rw [card_externalTargets G C, hz, hRoot]

end SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.SixBridge
