...
namespace SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Symmetry
...
theorem hToPIn_toNat (C : G.LocalConfiguration) (L : Labels G 5 C)
    (hHCard : C.H.card = 6) (p : Nat) (hp : p < 7) :
    (hToPIn (graphBits G L) p).toNat =
      Labels.incomingCount G C.H (L.p ⟨p, hp⟩).1 := by
  rw [hToPIn, toNat_count_eq_fin_sum 6 _ (by omega), Labels.incomingCount,
...
theorem pInvariantKey_toNat (C : G.LocalConfiguration) (L : Labels G 5 C)
    ...
    (p : Nat) (hp : p < 7) :
    (SymmetricCore.pInvariantKey 5 (graphBits G L) p).toNat =
...
@[simp] theorem hToP_graphBits (C : G.LocalConfiguration) (L : Labels G 5 C)
    (h p : Nat) (hh : h < 6) (hp : p < 7) :
    hToP (graphBits G L) h p =
      decide (G.Adj (L.a ⟨h + 1, by omega⟩).1 (L.p ⟨p, hp⟩).1) := by
  exact SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.hToP_coreBits
    G.Adj _ _ _ h p (by omega) hp
...
@[simp] theorem pToH_graphBits (C : G.LocalConfiguration) (L : Labels G 5 C)
  ...
  exact SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.pToH_coreBits
    G.Adj _ _ _ p h hp (by omega)
...
@[simp] theorem pToZ_graphBits (C : G.LocalConfiguration) (L : Labels G 5 C)
  ...
  exact SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding.pToZ_coreBits
    G.Adj _ _ _ p z hp (by omega) hz
...
theorem orderedP_true (C : G.LocalConfiguration) (L : Labels G 5 C)
    ...
    SymmetricCore.orderedP 5 (graphBits G L) = true := by
...
theorem orderedZ_true (C : G.LocalConfiguration) (L : Labels G 5 C)
    ...
    SymmetricCore.orderedZ 5 (graphBits G L) = true := by
...
theorem ordered_true (C : G.LocalConfiguration) (L : Labels G 5 C)
    ...
    (hZOrder : ∀ q : Fin 4,
      ...
      (Labels.zIncidenceCode G (fun i => (L.p i).1)
        (L.z ⟨q.val, by omega⟩).1).toNat) :
    SymmetricCore.ordered 5 (graphBits G L) = true := by
...
theorem canonical_symmetricCore_true (C : G.LocalConfiguration)
    ...
    (hHCard : C.H.card = 6) :
    let L := Labels.canonicalLabels G 5 C hPCard hACard hA1Card hXCard hRCard hZCard
    SymmetricCore.symmetricCore 5 (graphBits G L) = true := by
  dsimp only
...
end SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Symmetry
