...
namespace SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding
...
def coreBitAt {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  else if hnPZ : n < 228 then
    let q := n - 186
    let j := q % 6
    if hj : j < zCount then decide (R (p ⟨q / 6, by omega⟩) (z ⟨j, hj⟩))
    else false
  else false
...
def coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  BitVec.cast (by simp only [List.length_ofFn])
    (BitVec.ofBoolListLE (List.ofFn fun n : Fin 228 => coreBitAt R p a z n))

@[simp] theorem getLsbD_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
    (z : Fin zCount → V) (n : Nat) (hn : n < 228) :
...
@[simp] theorem aToZero_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  rw [show 4 + i - 4 = i by omega, getLsbD_coreBits R p a z i (by omega)]
...
@[simp] theorem pArc_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  · have hIndex : 46 + Core.pDirectedIndex i j < 88 := by
    ...
    have hd : Core.pDirectedIndex i j / 6 = i := by
      unfold Core.pDirectedIndex; split <;> omega
    have hm : Core.pDirectedIndex i j % 6 = if j < i then j else j - 1 := by
      unfold Core.pDirectedIndex
      by_cases hji : j < i <;> simp only [hji, ↓reduceIte]
      · simpa [Nat.mul_comm] using Nat.mul_add_mod_of_lt (a := i) (show j < 6 by omega)
      · simpa [Nat.mul_comm] using Nat.mul_add_mod_of_lt (a := i) (show j - 1 < 6 by omega)
    have hTarget :
        (if (if j < i then j else j - 1) < i then
          (if j < i then j else j - 1)
        else (if j < i then j else j - 1) + 1) = j := by
...
@[simp] theorem pToH_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  simp [coreBitAt, show ¬88 + 7*i+h < 4 by omega,
    ...
    show 88+7*i+h < 137 by omega, show 88+7*i+h-88=7*i+h by omega, hd, hm]
...
@[simp] theorem hToP_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  simp [coreBitAt, show ¬137+7*h+i < 4 by omega,
    show ¬137+7*h+i < 46 by omega, show ¬137+7*h+i < 88 by omega,
    show ¬137+7*h+i < 137 by omega, show 137+7*h+i < 186 by omega,
...
@[simp] theorem pToZ_coreBits {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  simp [coreBitAt, show ¬186+6*i+j < 4 by omega,
    show ¬186+6*i+j < 46 by omega, show ¬186+6*i+j < 88 by omega,
    show ¬186+6*i+j < 137 by omega, show ¬186+6*i+j < 186 by omega,
    show 186+6*i+j < 228 by omega, show 186+6*i+j-186=6*i+j by omega, hd, hm, hj]
...
@[simp] theorem pToZ_coreBits_inactive {zCount : Nat} (p : Fin 7 → V)
  ...
  simp [coreBitAt, show ¬186+6*i+j < 4 by omega,
    show ¬186+6*i+j < 46 by omega, show ¬186+6*i+j < 88 by omega,
    show ¬186+6*i+j < 137 by omega, show ¬186+6*i+j < 186 by omega,
    show 186+6*i+j < 228 by omega, show 186+6*i+j-186=6*i+j by omega, hm, hj]

end SeymourEight.BSevenKThree.RSeven.XThreeNoRoot.Encoding
