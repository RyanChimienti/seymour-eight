...
/-!
# Labels for the no-root `r = 7`, `x = 4` core

Besides arbitrary labels used by order-free lemmas, this module constructs a
canonical sequential labelling: `P` is sorted by graph-invariant scalar data,
then `A₁`, `X`, and `Z` are sorted by complete incidences to labelled `P`.
-/

namespace SeymourEight.BSevenKThree.RSeven.XFourNoRoot.Labels
...
@[simp] theorem canonicalLabels_x (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 3) (hXCard : C.X.card = 4)
    (hRCard : C.R.card = 0)
    (hZCard : (externalTargets G C).card = zCount) (i : Fin 4) :
    ((canonicalLabels G zCount C hPCard hACard hA1Card hXCard hRCard hZCard).a
      ⟨i.val + 4, by omega⟩).1 =
...
theorem canonicalLabels_aOne_order (zCount : Nat) (C : G.LocalConfiguration)
  ...
  rw [hFin, canonicalLabels_aOne G zCount C hPCard hACard hA1Card hXCard hRCard hZCard
      ⟨q.val + 1, by omega⟩,
    canonicalLabels_aOne G zCount C hPCard hACard hA1Card hXCard hRCard hZCard
      ⟨q.val, by omega⟩]
...
theorem canonicalLabels_x_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 3) (hXCard : C.X.card = 4)
    (hRCard : C.R.card = 0)
    (hZCard : (externalTargets G C).card = zCount) (q : Fin 3) :
    let L := canonicalLabels G zCount C hPCard hACard hA1Card hXCard hRCard hZCard
    (hInvariantKey G C (fun i => (L.p i).1)
  ...
  rw [hFin, canonicalLabels_x G zCount C hPCard hACard hA1Card hXCard hRCard hZCard
      ⟨q.val + 1, by omega⟩,
    canonicalLabels_x G zCount C hPCard hACard hA1Card hXCard hRCard hZCard
      ⟨q.val, by omega⟩]
...
end SeymourEight.BSevenKThree.RSeven.XFourNoRoot.Labels
