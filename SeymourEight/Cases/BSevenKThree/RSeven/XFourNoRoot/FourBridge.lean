import SeymourEight.Cases.BSevenKThree.RSeven.XFourNoRoot.FourSymmetry
import SeymourEight.Certificates.BSevenKThree.RSeven.XFour.FourDispatcher

set_option linter.style.header false

/-!
# Graph bridge for the four-target no-root row
-/

namespace SeymourEight.BSevenKThree.RSeven.XFourNoRoot.FourBridge
...
theorem impossibleAt
    ...
    (hr : C.r = 7) (hx : C.x = 4)
    (hZCard : (externalTargets G C).card = 4) : False := by
  ...
  have hCore := FourSymmetry.canonical_symmetricCore_true G C hG hPB hPivot hMin hk
    hNoSeymour hRootDegree hPCard hACard hA1Card hXCard hRCard hZCard hHCard
  exact FourDispatcher.impossible (graphBits G L) hCore
...
theorem impossible
    ...
    (hBCard : C.B.card = 7) (hk : C.k = 3)
    (hr : C.r = 7) (hx : C.x = 4)
    (hNoRoot : epsilonS G C = 0) (hz : C.z = 4) : False := by
...
theorem impossibleRoot
    ...
    (hBCard : C.B.card = 7) (hk : C.k = 3)
    (hr : C.r = 7) (hx : C.x = 4)
    (hRoot : epsilonS G C = 1) (hz : C.z = 3) : False := by
  apply impossibleAt G C hG hMin hNoSeymour hRootDegree hPivot hBCard hk hr hx
  rw [card_externalTargets G C, hz, hRoot]

end SeymourEight.BSevenKThree.RSeven.XFourNoRoot.FourBridge
