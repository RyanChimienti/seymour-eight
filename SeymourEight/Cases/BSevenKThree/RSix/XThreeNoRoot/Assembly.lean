import SeymourEight.Cases.BSevenKThree.RSix.XThreeNoRoot.GraphFacts
...
namespace SeymourEight.BSevenKThree.RSix.XThreeNoRoot.Assembly
...
theorem orientedAPQ_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 4) :
    orientedAPQ (encodedArc (graphBits G L)) = true := by
  ...
  · constructor
    ...
    · intro a ha
      ...
      rw [show encodedArc (graphBits G L) 14 a = false by simp [encodedArc]]
      simp
  · intro p hp
    rw [pToQ_graphBits G C L hG hzLe p hp]
    rw [show encodedArc (graphBits G L) 14 (8+p) = false by simp [encodedArc]]
    simp

theorem fixedPivot_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (_hG : G.IsOriented) (_hzLe : zCount ≤ 4) :
...
theorem qReached_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 4)
    (hy : BSevenKThree.y G C = 1) :
    qReached (encodedArc (graphBits G L)) = true := by
  ...
  rcases (Digraph.mem_outNeighborFinsetOf (G := G)).mp (Finset.mem_inter.mp hm).2 with
    ⟨u, hu, huq⟩
  ...
  · left
    ...
    have hiRange : 1 ≤ i.val ∧ i.val ≤ 3 := by
      have hai : G.Adj C.a1 (L.a i).1 := by
        rw [congrArg Subtype.val hi]
        exact (Finset.mem_filter.mp huA).2
      by_contra hn
      have hfixed := fixedPivot_true G C L hG hzLe
      rw [fixedPivot, all_eq_true_iff] at hfixed
      have hh := hfixed i.val (by omega)
      rw [aArc_graphBits G C L hG hzLe 0 i (by omega) i.isLt] at hh
      simp [L.a_zero, hai, hn] at hh
      omega
    refine ⟨i.val-1, by omega, ?_⟩
    rw [aToQ_graphBits G C L hG hzLe (1+(i.val-1)) (by omega)]
  ...
  · right
    ...
    obtain ⟨i, hi⟩ := L.p.surjective ⟨u, huP⟩
    refine ⟨i, i.isLt, ?_⟩
    rw [pToQ_graphBits G C L hG hzLe i i.isLt]
...
theorem aMinimumAndPivot_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  have hTie : (aOut (encodedArc (graphBits G L)) a).toNat = 3 →
      6 ≤ (aBOut (encodedArc (graphBits G L)) a).toNat := by
    intro heq
    rw [hBO]
...
theorem sharpKing_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  rw [all_eq_true_iff]
  intro p hp
  simp [encodedArc, show 8 + p < 14 by omega]
...
theorem hallCondition_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  · by_cases hAQ : encodedArc (graphBits G L) a 14 = true
    · simp [hInner, hAQ]
...
end SeymourEight.BSevenKThree.RSix.XThreeNoRoot.Assembly
