import SeymourEight.Cases.BSevenKThree.RSix.XThreeNoRoot.UnreachedEncoding
...
namespace SeymourEight.BSevenKThree.RSix.XThreeNoRoot.UnreachedGraphFacts

open Shared Shared.FiniteCore Labels UnreachedEncoding Core
...
abbrev graphBits {zCount : Nat} (L : Labels G zCount C) : UnreachedCore.Encoding :=
  UnreachedEncoding.graphBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
...
theorem encodedArc_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) source target =
  ...
  · by_cases htA : target < 8
    · by_cases hs0 : source = 0
      · subst source
        ...
        · simpa [UnreachedCore.encodedArc, labelledVertex] using decide_eq_false (hG.1 (L.a 0).1)
        · have hm : (L.a ⟨target, htA⟩).1 ∈ C.A1 := by
          ...
          simp [UnreachedCore.encodedArc, labelledVertex, htA, htA1, ha]
        · by_cases htR : target = 7
          · subst target
            ...
            simp [UnreachedCore.encodedArc, labelledVertex, hn]
          · have hm : (L.a ⟨target, htA⟩).1 ∈ C.X := by
            ...
            simp [UnreachedCore.encodedArc, labelledVertex, htA, hn]
            omega
      · by_cases ht0 : target = 0
        · subst target
          ...
          · have hm : (L.a ⟨source, hsA⟩).1 ∈ C.A1 := by
            ...
            have ha : G.Adj (L.a 0).1 (L.a ⟨source, hsA⟩).1 := by
              rw [L.a_zero]
              exact (Finset.mem_filter.mp hm).2
            simp [UnreachedCore.encodedArc, labelledVertex, hsA, hsA1, hG.2 ha]
          · have hi : source - 4 < 4 := by omega
            ...
            rw [show source = 4 + (source - 4) by omega,
              UnreachedEncoding.aToZero_graphBits G.Adj _ _ _ _ (source - 4) hi, heq]
        ...
        · have hi : source - 1 < 7 := by omega
          ...
          rw [show source = (source - 1) + 1 by omega,
            ...
            UnreachedEncoding.hArc_graphBits G.Adj _ _ _ _ _ _ hi hj]
    ...
    · by_cases htP : target < 14
      · by_cases hs0 : source = 0
        · subst source
          simp [UnreachedCore.encodedArc, labelledVertex, htA, htP, hA0P]
        · rw [show source = (source - 1) + 1 by omega,
            show target = 8 + (target - 8) by omega,
            UnreachedEncoding.hToP_graphBits G.Adj _ _ _ _ _ _ (by omega) (by omega)]
          have hsFin : (⟨source - 1 + 1, by omega⟩ : Fin 8) =
              ⟨source, hsA⟩ := Fin.ext (by simp; omega)
          rw [hsFin]
          simp [labelledVertex, show source - 1 + 1 = source by omega,
      ...
      · by_cases htQ : target = 14
        · subst target
          ...
          · subst source
            ...
            simp [UnreachedCore.encodedArc, labelledVertex, hn]
          · rw [show source = (source - 1) + 1 by omega,
              UnreachedEncoding.aToQ_graphBits G.Adj _ _ _ _ _ (by omega)]
            have hsFin : (⟨source - 1 + 1, by omega⟩ : Fin 8) =
                ⟨source, hsA⟩ := Fin.ext (by simp; omega)
            rw [hsFin]
        ...
        · have hn := A_not_adj_external G C hG (L.a ⟨source, hsA⟩).1
          ...
          simp [UnreachedCore.encodedArc, labelledVertex, hsA, htA, htP, htQ, ht, hn]
  · have hsP : source < 14 := hs
    by_cases htA : target < 8
    · by_cases ht0 : target = 0
      · subst target
        simp [UnreachedCore.encodedArc, labelledVertex, hsA, hsP, hP0]
      · by_cases htH : target ≤ 6
        · rw [show source = 8 + (source - 8) by omega,
            show target = (target - 1) + 1 by omega,
            UnreachedEncoding.pToH_graphBits G.Adj _ _ _ _ _ _ (by omega) (by omega)]
          have htFin : (⟨target - 1 + 1, by omega⟩ : Fin 8) =
              ⟨target, htA⟩ := Fin.ext (by simp; omega)
          rw [htFin]
        ...
        · have hn := BSixKThreeCoreGraphBridge.P_not_adj_R G C
          ...
          subst target
          simp [UnreachedCore.encodedArc, labelledVertex, hsA, hsP, hn]
    · by_cases htP : target < 14
      · rw [show source = 8 + (source - 8) by omega,
          show target = 8 + (target - 8) by omega,
          UnreachedEncoding.pArc_graphBits G.Adj _ _ _ _ _ _ (by omega) (by omega)]
      ...
      · by_cases htQ : target = 14
        · subst target
          rw [show source = 8 + (source - 8) by omega,
            UnreachedEncoding.pToQ_graphBits G.Adj _ _ _ _ _ (by omega)]
          simp [labelledVertex, show 8 + (source - 8) = source by omega, hsA, hsP]
        · rw [show source = 8 + (source - 8) by omega,
            show target = 15 + (target - 15) by omega,
            UnreachedEncoding.pToZ_graphBits G.Adj _ _ _ _ _ _ (by omega) (by omega) (by omega)]
          simp [labelledVertex, show 8 + (source - 8) = source by omega,
...
theorem aArc_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) i j =
...
theorem aToP_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) a (8+p) =
...
theorem pToA_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) (8+p) a =
...
theorem pArc_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) (8+i) (8+j) =
...
theorem aToQ_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (a : Nat) (ha : a < 8) :
    UnreachedCore.encodedArc (graphBits G L) a 14 =
...
theorem pToQ_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (p : Nat) (hp : p < 6) :
    UnreachedCore.encodedArc (graphBits G L) (8+p) 14 =
...
theorem pToZ_graphBits {zCount : Nat} (C : G.LocalConfiguration)
    ...
    UnreachedCore.encodedArc (graphBits G L) (8+p) (15+z) =
...
theorem strictSecond_true_mem {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source target : Nat) (hs : source < 14) (ht : target < 15 + zCount)
    (hSecond : strictSecond (UnreachedCore.encodedArc (graphBits G L)) source target = true) :
...
theorem secondCount_le_graph {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (secondCount zCount (UnreachedCore.encodedArc (graphBits G L)) source).toNat ≤
  ...
  have hFiltered := count_le_filterCard (V := V) (retainedVertexSet G C)
    ...
    (strictSecond (UnreachedCore.encodedArc (graphBits G L)) source)
...
theorem aOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 8) :
    (aOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem aPOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 8) :
    (aPOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem aBOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 8) :
    (aBOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
  ...
  have hBit : (bitCount (UnreachedCore.encodedArc (graphBits G L) source 14)).toNat =
      if G.Adj (L.a ⟨source, hs⟩).1 (L.q 0).1 then 1 else 0 := by
    rw [bitCount, aToQ_graphBits G C L hG hzLe source hs]
    by_cases h : G.Adj (L.a ⟨source, hs⟩).1 (L.q 0).1 <;>
      simp [h, BitVec.toNat_ofNat]
  have hBitLe : (bitCount (UnreachedCore.encodedArc (graphBits G L) source 14)).toNat ≤ 1 := by
    rw [hBit]
    split <;> omega
...
theorem pOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (pOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem pHOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (pHOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem hPOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (hHCard : C.H.card = 6) (source : Nat) (hs : source < 6) :
    (hPOut (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem pAuxOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 6) :
    (pAuxOut zCount (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem pZOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 6) :
    (pZOut zCount (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
...
theorem innerSecond_true_iff {zCount : Nat} (C : G.LocalConfiguration)
    ...
    innerSecond (UnreachedCore.encodedArc (graphBits G L)) source target = true ↔
      (L.a ⟨target, ht⟩).1 ∈
...
theorem innerSecondCount_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 8) :
    (innerSecondCount (UnreachedCore.encodedArc (graphBits G L)) source).toNat =
      (CertificateBridge.internalSecondNeighbors (G := G) C.A
  ...
  by_cases hb : innerSecond (UnreachedCore.encodedArc (graphBits G L)) source j = true
...
theorem hallReached_true_mem {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hReach : hallReached zCount (UnreachedCore.encodedArc (graphBits G L)) source e = true) :
...
theorem hallCount_le_card {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (source : Nat) (hs : source < 8) :
    (hallCount zCount (UnreachedCore.encodedArc (graphBits G L)) source).toNat ≤
  ...
  have hCountEq : (∑ e : Fin (1+zCount),
      if hallReached zCount (UnreachedCore.encodedArc (graphBits G L)) source e then 1 else 0) ≤
      S.card := by
    rw [Finset.card_filter]
    apply Finset.sum_le_sum
    intro e _
    by_cases hb : hallReached zCount (UnreachedCore.encodedArc (graphBits G L)) source e = true
...
theorem pSecondP_le_graph {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (p : Nat) (hp : p < 6) :
    (pSecondP (UnreachedCore.encodedArc (graphBits G L)) p).toNat ≤
      (C.P.filter fun v ↦
        v ∈ G.secondOutNeighborFinset (L.p ⟨p, hp⟩).1).card := by
  apply count_le_filterCard C.P L.p
    (fun q ↦ strictSecond (UnreachedCore.encodedArc (graphBits G L)) (8+p) (8+q))
...
end SeymourEight.BSevenKThree.RSix.XThreeNoRoot.UnreachedGraphFacts
