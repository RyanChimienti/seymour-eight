...
namespace SeymourEight.BSevenKThree.RSix.XThreeNoRoot.UnreachedOrderBridge
...
theorem pAOneOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 3)
    (p : Nat) (hp : p < 6) :
    (count 3 fun a => UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L) (8+p) (1+a)).toNat =
      directCount G C.A1 (L.p ⟨p, hp⟩).1 := by
  rw [toNat_count_eq_fin_sum 3 _ (by omega)]
  symm
  apply directCount_eq_sum_bool G C.A1 (UnreachedGraphFacts.aOneLabelEquiv G C L hA1Card) _
  intro i
  rw [UnreachedGraphFacts.aOneLabelEquiv_val]
...
theorem pXOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hXCard : C.X.card = 3)
    (p : Nat) (hp : p < 6) :
    (count 3 fun x => UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L) (8+p) (4+x)).toNat =
      directCount G C.X (L.p ⟨p, hp⟩).1 := by
  rw [toNat_count_eq_fin_sum 3 _ (by omega)]
  symm
  apply directCount_eq_sum_bool G C.X (UnreachedGraphFacts.xLabelEquiv G C L hXCard) _
  intro i
  rw [UnreachedGraphFacts.xLabelEquiv_val]
...
theorem pQBit_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hG : G.IsOriented) (hzLe : zCount ≤ 5)
    (p : Nat) (hp : p < 6) :
    (bitCount (UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L) (8+p) 14)).toNat =
...
theorem pDegree_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (pDegree zCount (UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L)) p).toNat =
...
theorem pLexLe_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    pLexLe zCount (UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L)) i j = true := by
  ...
  have hZi := UnreachedGraphFacts.pZOut_toNat G C L hG hzLe i hi
  have hZj := UnreachedGraphFacts.pZOut_toNat G C L hG hzLe j hj
  ...
  have hPDirectLe (v : V) (hv : v ∈ C.P) : directCount G C.P v ≤ 5 := by
    apply (Finset.card_le_card (show C.P.filter (G.Adj v) ⊆ C.P.erase v by
      intro w hw
      rw [Finset.mem_erase]
      exact ⟨by
        intro heq
        subst w
        exact hG.1 v (Finset.mem_filter.mp hw).2,
        (Finset.mem_filter.mp hw).1⟩)).trans_eq
    simp [Finset.card_erase_of_mem hv, hPCard]
  have hDiLe : di ≤ 17 := by
    rw [hDi]
    have hP := hPDirectLe (L.p ⟨i, hi⟩).1 (L.p ⟨i, hi⟩).2
  ...
  have hDjLe : dj ≤ 17 := by
    rw [hDj]
    have hP := hPDirectLe (L.p ⟨j, hj⟩).1 (L.p ⟨j, hj⟩).2
...
theorem zIn_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : SeymourEight.BSevenKThree.RSix.XThreeNoRoot.Labels.Labels G zCount C)
    (hG : G.IsOriented) (hzLe : zCount ≤ 5) (z : Nat) (hz : z < zCount) :
    (count 6 fun p ↦ UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L) (8 + p) (15 + z)).toNat =
  ...
  intro p hp
  rw [UnreachedGraphFacts.pToZ_graphBits G C L hG hzLe p z p.isLt hz]
...
theorem reducedOrdered_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    reducedOrdered zCount (UnreachedCore.encodedArc (UnreachedGraphFacts.graphBits G L)) = true := by
...
end SeymourEight.BSevenKThree.RSix.XThreeNoRoot.UnreachedOrderBridge
