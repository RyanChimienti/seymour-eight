import SeymourEight.Cases.BSevenKThree.RFive.XThreeNoRoot.GraphFacts
import SeymourEight.Cases.BSevenKThree.Counting
import SeymourEight.Cases.BSevenKTwo.RSix.XTwoRoot.GraphBridge
import SeymourEight.Certificates.BSevenKThree.RFive.XThree.Derived
import SeymourEight.Shared.InnerDegreeThree

set_option linter.style.header false
set_option maxRecDepth 10000

namespace SeymourEight.BSevenKThree.RFive.XThreeNoRoot.Assembly
...
theorem orientedA_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    orientedA (encodedArc (graphBits G L)) = true := by
...
theorem orientedP_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    orientedP (encodedArc (graphBits G L)) = true := by
...
theorem orientedPH_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    orientedPH (encodedArc (graphBits G L)) = true := by
  rw [orientedPH, all_eq_true_iff]
  intro p hp
  rw [all_eq_true_iff]
  intro h hh
  rw [pToA_graphBits G C L hG hzLe p (1+h) hp (by omega),
    aToP_graphBits G C L hG hzLe (1+h) p (by omega) hp]
  by_cases ha : G.Adj (L.p ⟨p, hp⟩).1 (L.a ⟨1+h, by omega⟩).1
  · simp [ha, hG.2 ha]
  · simp [ha]

theorem fixedPivot_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (_hG : G.IsOriented) (_hzLe : zCount ≤ 3) :
    fixedPivot (encodedArc (graphBits G L)) = true := by
  simp [fixedPivot, all, encodedArc]

theorem everyXReached_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3)
    (hA1Card : C.A1.card = 3) :
    everyXReached (encodedArc (graphBits G L)) = true := by
...
theorem rUnreached_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    rUnreached (encodedArc (graphBits G L)) = true := by
...
theorem qInB_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    qInB (encodedArc (graphBits G L)) = true := by
  ...
  refine ⟨i, i.isLt, ?_⟩
  rw [aToQ_graphBits G C L hG hzLe i q i.isLt hq]
  simpa [congrArg Subtype.val hi] using haq

theorem qReached_iff_mem {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3)
    (hA1Card : C.A1.card = 3) (q : Fin 2) :
    qReached (encodedArc (graphBits G L)) q = true ↔
      (L.q q).1 ∈ BSevenKThree.reachedQ G C := by
  rw [qReached, Bool.or_eq_true]
  constructor
  · rintro (h | h)
    · obtain ⟨i, hi, hadj⟩ := (any_eq_true_iff 3 _).mp h
      rw [aToQ_graphBits G C L hG hzLe (1+i) q (by omega) q.isLt] at hadj
      exact Finset.mem_inter.mpr ⟨(L.q q).2,
        (Digraph.mem_outNeighborFinsetOf (G := G)).mpr ⟨_,
          Finset.mem_union_left C.P (aOneLabelEquiv G C L hA1Card ⟨i, hi⟩).2,
          by simpa [aOneLabelEquiv, Nat.add_comm] using of_decide_eq_true hadj⟩⟩
    · obtain ⟨i, hi, hadj⟩ := (any_eq_true_iff 5 _).mp h
      rw [pToQ_graphBits G C L hG hzLe i q hi q.isLt] at hadj
      exact Finset.mem_inter.mpr ⟨(L.q q).2,
        (Digraph.mem_outNeighborFinsetOf (G := G)).mpr ⟨_,
          Finset.mem_union_right C.A1 (L.p ⟨i, hi⟩).2, of_decide_eq_true hadj⟩⟩
  · intro hmem
    rcases (Digraph.mem_outNeighborFinsetOf (G := G)).mp
    ...
    · left
      ...
      refine ⟨i, i.isLt, ?_⟩
      rw [aToQ_graphBits G C L hG hzLe (1+i) q (by omega) q.isLt]
    ...
    · right
      ...
      obtain ⟨i, hi⟩ := L.p.surjective ⟨u, huP⟩
      refine ⟨i, i.isLt, ?_⟩
      rw [pToQ_graphBits G C L hG hzLe i q i.isLt q.isLt]
      simpa [congrArg Subtype.val hi] using huq

theorem qReachStatus_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3)
    (hA1Card : C.A1.card = 3) (yValue : Nat)
    (hy : BSevenKThree.y G C = yValue) :
    qReachStatus yValue (encodedArc (graphBits G L)) = true := by
  have hCount : (count 2 (qReached (encodedArc (graphBits G L)))).toNat =
      (BSevenKThree.reachedQ G C).card := by
    rw [toNat_count_eq_fin_sum 2 _ (by omega)]
    calc
      _ = ∑ q : Fin 2, if (L.q q).1 ∈ BSevenKThree.reachedQ G C then 1 else 0 := by
        apply Finset.sum_congr rfl
        intro q _
        by_cases h : qReached (encodedArc (graphBits G L)) q = true
        · simp [h, (qReached_iff_mem G C L hG hzLe hA1Card q).mp h]
        · have hn : (L.q q).1 ∉ BSevenKThree.reachedQ G C :=
            fun hm ↦ h ((qReached_iff_mem G C L hG hzLe hA1Card q).mpr hm)
      ...
      _ = (BSevenKThree.reachedQ G C).card := by
        ...
        exact ⟨And.right, fun h ↦ ⟨Finset.inter_subset_left h, h⟩⟩
  rw [qReachStatus]
...
theorem allZReached_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    allZReached zCount (encodedArc (graphBits G L)) = true := by
...
theorem inactiveZZero_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hz : zCount = 1 ∨ zCount = 2 ∨ zCount = 3) :
    inactiveZZero zCount (encodedArc (graphBits G L)) = true := by
  have hInactive (p j : Nat) (hp : p < 5) (hj3 : j < 3)
      (hj : zCount ≤ j) :
      encodedArc (graphBits G L) (8+p) (15+j) = false := by
    exact SeymourEight.BSevenKThree.RFive.XThreeNoRoot.Encoding.pToZ_graphBits_inactive
      G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1) (fun i ↦ (L.q i).1)
        (fun i ↦ (L.z i).1) p j hp hj3 hj
  rcases hz with rfl | rfl | rfl
  · rw [inactiveZZero, if_pos rfl, all_eq_true_iff]
    intro p hp
    rw [Bool.and_eq_true]
    exact ⟨by simpa using hInactive p 1 hp (by decide) (by decide),
      by simpa using hInactive p 2 hp (by decide) (by decide)⟩
  · rw [inactiveZZero, if_neg (by omega), if_pos rfl, all_eq_true_iff]
    intro p hp
    simpa using hInactive p 2 hp (by decide) (by decide)
  · simp [inactiveZZero]
...
theorem aMinimumAndPivot_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hk : C.k = 3) (hr : C.r = 5) :
    aMinimumAndPivot (encodedArc (graphBits G L)) = true := by
  ...
  have hAmin : 3 ≤ (aOut (encodedArc (graphBits G L)) a).toNat := by
    rw [hAO]
    simpa [hk, Shared.directCount,
      CertificateBridge.internalFirstNeighbors] using hPivotA.1
  have hTie : (aOut (encodedArc (graphBits G L)) a).toNat = 3 →
      5 ≤ (aBOut (encodedArc (graphBits G L)) a).toNat := by
    intro heq
    rw [hBO]
  ...
  have hTotal : 8 ≤ (aOut (encodedArc (graphBits G L)) a).toNat +
      (aBOut (encodedArc (graphBits G L)) a).toNat := by
    rw [hAO, hBO, ← A_outdegree_eq_A_add_B G C hG _ (L.a _).2]
    exact hMin _
  have hSmall : (aOut (encodedArc (graphBits G L)) a).toNat +
      (aBOut (encodedArc (graphBits G L)) a).toNat < 256 := by
    rw [hAO, hBO]
  ...
  refine ⟨⟨hAmin, ?_⟩, ?_⟩
  · rw [Bool.or_eq_true]
    by_cases heq : aOut (encodedArc (graphBits G L)) a = 3
    · right
...
theorem aNonSeymour_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    aNonSeymour zCount (encodedArc (graphBits G L)) = true := by
...
theorem pMinimum_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    pMinimum zCount (encodedArc (graphBits G L)) = true := by
  ...
  have hPLe : Shared.directCount G C.P (L.p ⟨p, hp⟩).1 ≤ 5 :=
  ...
  have hLower := hMin (L.p ⟨p, hp⟩).1
  omega

theorem degreeThreeConsequences_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C)
    (hOriented : orientedA (encodedArc (graphBits G L)) = true)
    (hMinimum : ∀ a < 8,
      (3 : BitVec 8).ule (aOut (encodedArc (graphBits G L)) a) = true) :
    degreeThreeClassification (encodedArc (graphBits G L)) = true ∧
      threeInnerWitnesses (encodedArc (graphBits G L)) = true := by
...
theorem sharpKing_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hOriented :
      orientedP (encodedArc (graphBits G L)) = true) :
    sharpKing (encodedArc (graphBits G L)) = true := by
  apply Core.sharpKing_of_orientedP _ hOriented
  rw [all_eq_true_iff]
  intro p hp
  simp [encodedArc, show 8 + p < 13 by omega]

theorem hallCondition_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3)
    (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hNoSeymour : ¬G.HasSeymourVertex) :
    all 8 (hallCondition zCount (encodedArc (graphBits G L))) = true := by
  rw [all_eq_true_iff]
  intro a ha
  unfold hallCondition
  by_cases hInner : innerSeymour (encodedArc (graphBits G L)) a = true
  · by_cases hAQ : any 2 (fun q ↦ encodedArc (graphBits G L) a (13+q)) = true
    · simp [hInner, hAQ]
    · have hAQFalse := Bool.eq_false_of_not_eq_true hAQ
      ...
      have hInnerNat : (aOut (encodedArc (graphBits G L)) a).toNat ≤
          (innerSecondCount (encodedArc (graphBits G L)) a).toNat := by
        unfold innerSeymour at hInner
        simpa [BitVec.ule_eq_decide] using hInner
      have hNotQ (q : Fin 2) : ¬G.Adj v (L.q q).1 := by
        intro hadj
        apply hAQ
        rw [any_eq_true_iff]
        refine ⟨q, q.isLt, ?_⟩
        rw [aToQ_graphBits G C L hG hzLe a q ha q.isLt]
        simpa [v] using decide_eq_true hadj
      have hOutEq : G.outdegree v = S.card + Shared.directCount G C.A v := by
        ...
        have hQZero : Shared.directCount G C.Q v = 0 := by
          ...
          intro w hwQ hwAdj
          obtain ⟨q, hq⟩ := L.q.surjective ⟨w, hwQ⟩
          exact hNotQ q (by simpa [congrArg Subtype.val hq] using hwAdj)
      ...
      have hUSub : U ⊆ G.secondOutNeighborFinset v := by
        ...
        have hNot : ¬G.Adj v e := by
          ...
          · obtain ⟨q, hq⟩ := L.q.surjective ⟨e, heQ⟩
            intro hve
            apply hNotQ q
            simpa [congrArg Subtype.val hq] using hve
...
theorem arithmetic_true {yValue zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C)
    (hA : orientedA (encodedArc (graphBits G L)) = true)
    (hP : orientedP (encodedArc (graphBits G L)) = true)
    (hPH : orientedPH (encodedArc (graphBits G L)) = true)
    (hFixed : fixedPivot (encodedArc (graphBits G L)) = true)
    (hX : everyXReached (encodedArc (graphBits G L)) = true)
    (hR : rUnreached (encodedArc (graphBits G L)) = true)
    (hQB : qInB (encodedArc (graphBits G L)) = true)
    (hQR : qReachStatus yValue (encodedArc (graphBits G L)) = true)
    (hAMin : aMinimumAndPivot (encodedArc (graphBits G L)) = true)
    (hPMin : pMinimum zCount (encodedArc (graphBits G L)) = true)
    (hyz : (yValue = 1 ∧ zCount = 3) ∨
      (yValue = 2 ∧ (zCount = 1 ∨ zCount = 2))) :
    arithmetic yValue zCount (encodedArc (graphBits G L)) = true := by
  exact Core.arithmetic_of_local yValue zCount (graphBits G L) hA hP hPH
    hFixed hX hR hQB hQR hAMin hPMin hyz

end SeymourEight.BSevenKThree.RFive.XThreeNoRoot.Assembly
