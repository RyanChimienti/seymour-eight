...
namespace SeymourEight.BSevenKThree.RFive.XThreeNoRoot.Labels

open Shared CertificateBridge
open SeymourEight.BSevenKThree.RFive.XFourNoRoot.Labels
...
noncomputable def canonicalLabels (zCount : Nat) (C : G.LocalConfiguration)
  ...
  let aOne := sortedFinsetEquiv C.A1 (finsetEquivFin C.A1 hA1Card)
    (aInvariantKey G C)
...
theorem canonicalLabels_p_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hZCard : (externalTargets G C).card = zCount)
    (hA1Card : C.A1.card = 3) (hXCard : C.X.card = 3) (hRCard : C.R.card = 1)
    (i : Fin 4) :
  ...
  exact sortedFinsetEquiv_key_anti C.P (finsetEquivFin C.P hPCard)
    (pInvariantKey G C) (Fin.mk_le_mk.mpr (by omega))
...
theorem canonicalLabels_x_order (zCount : Nat) (C : G.LocalConfiguration)
  ...
  simpa [canonicalLabels, BSixKThreeCoreGraphBridge.aLabelEquiv_apply,
    ...
    show i.val + 4 < 7 by omega, show i.val + 5 < 7 by omega] using h
...
end SeymourEight.BSevenKThree.RFive.XThreeNoRoot.Labels
