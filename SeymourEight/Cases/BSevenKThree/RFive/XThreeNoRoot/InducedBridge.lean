...
namespace SeymourEight.BSevenKThree.RFive.XThreeNoRoot.InducedBridge
...
theorem indexedInducedWitness {n : Nat}
    ...
    (any n fun source ↦
      ...
      (count n fun target ↦ strictSecond arc (idx source) (idx target))) = true := by
  ...
  have hSecond : (Shared.inducedSecondFinset G S v).card ≤
      (count n fun target ↦ strictSecond arc (idx source)
        (idx target)).toNat := by
    have hFilter :=
      SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.GraphFacts.filterCard_le_count
        S e (fun target ↦ strictSecond arc (idx source) (idx target))
        (fun w ↦ w ∈ Shared.inducedSecondFinset G S v) (by omega) (by
          ...
          simp only [strictSecond, Bool.and_eq_true, decide_eq_true_eq]
          refine ⟨⟨?_, ?_⟩, ?_⟩
          · intro heq
            apply hne
...
theorem contiguousInduced_true {zCount start size : Nat}
    ...
    (hzLe : zCount ≤ 3) (hSizePos : 0 < size) (hEnd : start + size ≤ 13) :
    inducedWitness size (fun i ↦ start + i)
      (encodedArc (graphBits G L)) = true := by
  unfold inducedWitness
  apply indexedInducedWitness G hBound hG (encodedArc (graphBits G L))
    (fun i ↦ start + i)
  ...
  · intro i j hij
    ...
    omega
  · intro i j
    exact encodedArc_graphBits G C L hG (by omega) _ _ (by omega) (by omega)

theorem aOnePInduced_true {zCount : Nat}
    (hBound : Digraph.LimitedSeymourConjectureOn V 7) (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    inducedWitness 8 aOnePIndex (encodedArc (graphBits G L)) = true := by
  unfold inducedWitness
  apply indexedInducedWitness G hBound hG (encodedArc (graphBits G L)) aOnePIndex
  ...
  · intro i j
    have hi : aOnePIndex i < 13 := by simp [aOnePIndex]; split <;> omega
    have hj : aOnePIndex j < 13 := by simp [aOnePIndex]; split <;> omega
    exact encodedArc_graphBits G C L hG (by omega) _ _ hi (by omega)

theorem hPInduced_true {zCount : Nat}
    (hBound : Digraph.LimitedSeymourConjectureOn V 7) (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    inducedWitness 11 hPIndex (encodedArc (graphBits G L)) = true := by
  unfold inducedWitness
  apply indexedInducedWitness G hBound hG (encodedArc (graphBits G L)) hPIndex
    (fun i : Fin 11 ↦ labelledVertex G L (hPIndex i)) (by omega) (by omega)
  · intro i hi
    simp [hPIndex]
    split <;> omega
  · intro i j hij
    apply Fin.ext
    simp only [hPIndex] at hij
    split at hij <;> split at hij <;> omega
  · intro i j hij
    have hi : hPIndex i < 13 := by simp [hPIndex]; split <;> omega
    have hj : hPIndex j < 13 := by simp [hPIndex]; split <;> omega
    have hFin : (⟨hPIndex i, by omega⟩ : Fin (15 + zCount)) =
        ⟨hPIndex j, by omega⟩ := by
      apply (retainedLabelEquiv G C L hG).injective
      apply Subtype.ext
      simpa using hij
    exact Fin.ext_iff.mp hFin |> fun h ↦ Fin.ext (by
      simp only [hPIndex] at h ⊢
      split at h <;> split at h <;> omega)
  · intro i j
    have hi : hPIndex i < 13 := by simp [hPIndex]; split <;> omega
    have hj : hPIndex j < 13 := by simp [hPIndex]; split <;> omega
...
theorem allInduced_true {zCount : Nat}
    (hBound : Digraph.LimitedSeymourConjectureOn V 7) (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    inducedWitness 13 id (encodedArc (graphBits G L)) = true := by
  unfold inducedWitness
  apply indexedInducedWitness G hBound hG (encodedArc (graphBits G L)) id
    (fun i : Fin 13 ↦ labelledVertex G L i) (by omega) (by omega)
  · intro i hi
    exact hi
  · intro i j hij
    exact Fin.ext hij
  · intro i j hij
    have hFin : (⟨i, by omega⟩ : Fin (15 + zCount)) = ⟨j, by omega⟩ := by
      apply (retainedLabelEquiv G C L hG).injective
      apply Subtype.ext
      simpa using hij
    apply Fin.ext
    exact congrArg (fun x : Fin (15 + zCount) ↦ x.val) hFin
  · intro i j
    exact encodedArc_graphBits G C L hG hzLe i j i.isLt (by omega)

set_option maxHeartbeats 1000000 in
theorem inducedConditions_true {zCount : Nat}
    (hBound : Digraph.LimitedSeymourConjectureOn V 7) (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hzLe : zCount ≤ 3) :
    inducedConditions (encodedArc (graphBits G L)) = true := by
  unfold inducedConditions
  rw [Bool.and_eq_true]
  constructor
  · rw [Bool.and_eq_true]
    constructor
    · rw [Bool.and_eq_true]
      exact ⟨contiguousInduced_true (start := 8) (size := 5)
          G hBound C L hG hzLe (by omega) (by omega),
        aOnePInduced_true G hBound C L hG hzLe⟩
    · exact hPInduced_true G hBound C L hG hzLe
  · exact allInduced_true G hBound C L hG hzLe

end SeymourEight.BSevenKThree.RFive.XThreeNoRoot.InducedBridge
