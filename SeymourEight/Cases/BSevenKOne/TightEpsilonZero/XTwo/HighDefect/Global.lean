import SeymourEight.Cases.BSevenKOne.TightEpsilonZero.XTwo.HighDefect.NonSeymour
...
theorem orientedSquare_coreBits_true
    ...
    orientedSquare 20 (coreArc (coreBits G.Adj p h r z a)) = true := by
  classical
...
theorem totalHToP_coreBits_toNat (C : G.LocalConfiguration)
    ...
    (FiveZHighDefect.totalHToP (coreBits G.Adj (fun i ↦ (p i).1) (fun i ↦ (h i).1)
      r z a)).toNat = edgeCount G C.H C.P := by
  let bits := coreBits G.Adj (fun i ↦ (p i).1) (fun i ↦ (h i).1) r z a
  rw [FiveZHighDefect.totalHToP,
    FiveZExactGraphBridge.toNat_count_eq_fin_sum 21 _ (by omega)]
...
theorem totalMissingPZ_eq_rowSum (bits : BitVec 218) :
    FiveZHighDefect.totalMissingPZ bits =
      sumCount 7 (fun i => count 5 fun j => !FiveZHighDefect.pToZ bits i j) := by
  simp only [FiveZHighDefect.totalMissingPZ, sumCount, count, bitCount,
    FiveZHighDefect.pToZ]
  bv_decide

theorem totalMissingPZ_toNat_rows (bits : BitVec 218) :
    (FiveZHighDefect.totalMissingPZ bits).toNat =
      ∑ i : Fin 7, ∑ j : Fin 5,
        if !FiveZHighDefect.pToZ bits i j then 1 else 0 := by
  rw [totalMissingPZ_eq_rowSum,
    FiveZExactGlobalBridge.toNat_sumCount_of_le 7 5 _ (by omega) (by
      intro i hi
...
theorem totalMissingPZ_coreBits_toNat_add_edges (C : G.LocalConfiguration)
    (p : Fin 7 ≃ {v : V // v ∈ C.P})
    (z : Fin 5 ≃ {v : V // v ∈ C.Z})
    (h : Fin 3 → V) (r : Fin 4 → V) (a : Fin 8 → V) :
    (FiveZHighDefect.totalMissingPZ (coreBits G.Adj (fun i ↦ (p i).1) h r
      (fun i ↦ (z i).1) a)).toNat + edgeCount G C.P C.Z = 35 := by
  let bits := coreBits G.Adj (fun i ↦ (p i).1) h r (fun i ↦ (z i).1) a
  have hMissing : (FiveZHighDefect.totalMissingPZ bits).toNat =
      ∑ i : Fin 7, ∑ j : Fin 5,
        if !FiveZHighDefect.pToZ bits i j then 1 else 0 := by
  ...
  have hDecode : ∀ i : Fin 7, ∀ j : Fin 5,
      FiveZHighDefect.pToZ bits i j = decide (G.Adj (p i).1 (z j).1) := by
    intro i j
