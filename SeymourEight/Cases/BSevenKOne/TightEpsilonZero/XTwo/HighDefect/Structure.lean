import SeymourEight.Cases.BSevenKOne.TightEpsilonZero.XTwo.HighDefect.Deletion
...
theorem aPOut_coreBits_toNat (C : G.LocalConfiguration)
  ...
  · subst source
    simp [aToP, hA0P]
  by_cases hs4 : source < 4
  · have hi : source - 1 < 3 := by omega
    rw [aToP, if_neg hs0, if_pos hs4,
  ...
  · have hi : source - 4 < 4 := by omega
    rw [aToP, if_neg hs0, if_neg hs4,
      rToP_coreBits G.Adj (fun i ↦ (p i).1) h r z a
        (source - 4) j hi j.isLt]
    have heq := hAR ⟨source - 4, hi⟩
    have heq' : a ⟨source, hs⟩ = r ⟨source - 4, hi⟩ := by
      simpa [show source - 4 + 4 = source by omega] using heq
    rw [heq']
    simp
...
theorem fixedStructure_coreBits_true (C : G.LocalConfiguration)
    ...
    (hAR : ∀ i : Fin 4, (a ⟨i + 4, by omega⟩).1 = (r i).1) :
  ...
  have hA1R : all 4 (fun q => !aArc bits 1 (q + 4)) = true := by
    ...
    rw [aArc_coreBits G.Adj (fun i ↦ (p i).1) (fun i ↦ (h i).1)
      (fun i ↦ (r i).1) (fun i ↦ (z i).1) (fun i ↦ (a i).1)
      1 (q + 4) (by omega) (by omega)]
    have hSource : (a 1).1 = (h 0).1 := by simpa using hAH 0
    have hTarget : (a ⟨q + 4, by omega⟩).1 = (r ⟨q, hq⟩).1 := by
  ...
  have hHTotal : (11 : BitVec 8).ule (totalHToP bits) = true := by
    simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
    rw [totalHToP_coreBits_toNat G C p h (fun i ↦ (r i).1)
      (fun i ↦ (z i).1) (fun i ↦ (a i).1)]
    have hc := eight_add_choose_x_succ_le_H_to_P G C hG hMin hPB hRootDegree hk
    norm_num [hx, Nat.choose] at hc ⊢
    exact hc
  have hZReach : all 5 (fun zi => any 7 (fun i => pToZ bits i zi)) = true := by
    rw [all_eq_true_iff]
    intro zi hzi
  ...
  simpa only [Bool.and_eq_true] using
    ⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨hOrient, hA01⟩, hA0Tail⟩, hA1R⟩, hX1⟩, hX2⟩,
      hAOut⟩, hTie⟩, hDegrees⟩, hHTotal⟩, hZReach⟩
