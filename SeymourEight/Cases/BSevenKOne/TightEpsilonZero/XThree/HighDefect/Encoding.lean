import SeymourEight.Certificates.BSevenKOne.TightEpsilonZero.XThree.HighDefect.CoreBridge
...
The segmented certificate layout is exposed through one unified nineteen-vertex
...
open FourZHighDefect FourZHighDefectBridge

variable {V : Type*} (G : Digraph V)
variable [Fintype V] [DecidableEq V] [DecidableRel G.Adj]

def labelledVertex (a : Fin 8 → V) (p : Fin 7 → V) (z : Fin 4 → V)
    (i : Nat) : V :=
  if hiA : i < 8 then a ⟨i, hiA⟩
  else if hiP : i < 15 then p ⟨i - 8, by omega⟩
  else if hiZ : i < 19 then z ⟨i - 15, by omega⟩
  else z 0
