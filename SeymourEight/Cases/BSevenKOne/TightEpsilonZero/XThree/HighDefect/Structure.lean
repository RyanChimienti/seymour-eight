import SeymourEight.Cases.BSevenKOne.TightEpsilonZero.XThree.HighDefect.Deletion
...
theorem fixedStructure_coreBits_true (C : G.LocalConfiguration)
  ...
  have hA1R : all 3 (fun q => !aArc bits 1 (q + 5)) = true := by
    ...
    rw [aArc_coreBits G.Adj (fun i ↦ (p i).1) (fun i ↦ (h i).1)
      (fun i ↦ (r i).1) (fun i ↦ (z i).1) (fun i ↦ (a i).1)
      1 (q + 5) (by omega) (by omega)]
    have hSource : (a 1).1 = (h 0).1 := by simpa using hAH 0
    have hTarget : (a ⟨q + 5, by omega⟩).1 = (r ⟨q, hq⟩).1 := by
  ...
  have hHTotal : (14 : BitVec 8).ule (totalHToP bits) = true := by
    ...
    simp [hx, Nat.choose] at hc ⊢
    exact hc
  have hZReach : all 4 (fun zi => any 7 (fun i => pToZ bits i zi)) = true := by
    rw [all_eq_true_iff]
    intro zi hzi
  ...
  simpa only [Bool.and_eq_true] using
    ⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨⟨hOrient, hA01⟩, hA0Tail⟩, hA1R⟩, hX1⟩, hX2⟩,
      hX3⟩, hAOut⟩, hTie⟩, hDegrees⟩, hHTotal⟩, hZReach⟩
