import SeymourEight.Cases.BSevenKOne.TightEpsilonZero.XFour.HighDefect.Labels
...
open ThreeZHighDefect ThreeZHighDefectBridge FiveZExactGraphBridge
...
theorem coreOutdegree_coreBits_toNat (C : G.LocalConfiguration)
  ...
  rw [coreOutdegree, FiveZExactGraphBridge.toNat_count_eq_fin_sum 18 _ (by omega)]
...
theorem pDegree_coreBits_toNat (C : G.LocalConfiguration)
  ...
  have hZ : (count 3 (pToZ bits source)).toNat =
    ...
    simp
  have hH : (count 5 (pToH bits source)).toNat =
  ...
  rw [ThreeZHighDefect.pDegree, BitVec.toNat_add, BitVec.toNat_add,
...
theorem pDegreeSum_coreBits_toNat (C : G.LocalConfiguration)
    ...
    (r : Fin 2 → V) (z : Fin 3 ≃ {v : V // v ∈ C.Z})
    (a : Fin 8 → V) :
    (ThreeZHighDefect.pDegreeSum (coreBits G.Adj (fun i ↦ (p i).1)
      (fun i ↦ (h i).1) r (fun i ↦ (z i).1) a)).toNat =
      ∑ u ∈ C.P, G.outdegree u := by
  let bits := coreBits G.Adj (fun i ↦ (p i).1) (fun i ↦ (h i).1)
    r (fun i ↦ (z i).1) a
  rw [ThreeZHighDefect.pDegreeSum,
  ...
  · intro i hi
    ...
    have hZLe : directCount G C.Z (p ⟨i, hi⟩).1 ≤ 3 :=
      (Finset.card_le_card (Finset.filter_subset _ _)).trans_eq
        (by simpa using (Fintype.card_congr z).symm)
    have hHLe : directCount G C.H (p ⟨i, hi⟩).1 ≤ 5 :=
      (Finset.card_le_card (Finset.filter_subset _ _)).trans_eq
        (by simpa using (Fintype.card_congr h).symm)
