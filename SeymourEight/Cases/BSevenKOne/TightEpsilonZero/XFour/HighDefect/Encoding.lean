import SeymourEight.Certificates.BSevenKOne.TightEpsilonZero.XFour.HighDefect.CoreBridge
...
The segmented certificate layout is exposed through one unified eighteen-vertex
...
open ThreeZHighDefect ThreeZHighDefectBridge

variable {V : Type*} (G : Digraph V)
variable [Fintype V] [DecidableEq V] [DecidableRel G.Adj]

def labelledVertex (a : Fin 8 → V) (p : Fin 7 → V) (z : Fin 3 → V)
    (i : Nat) : V :=
  if hiA : i < 8 then a ⟨i, hiA⟩
  else if hiP : i < 15 then p ⟨i - 8, by omega⟩
  else if hiZ : i < 18 then z ⟨i - 15, by omega⟩
  else z 0
...
theorem coreArc_coreBits
  ...
  have hA5 : a (5 : Fin 8) = h 4 := by simpa using hAH 4
  have hA6 : a (6 : Fin 8) = r 0 := by simpa using hAR 0
  have hA7 : a (7 : Fin 8) = r 1 := by simpa using hAR 1
  have hA1' (ph : 0 < 5) (pa : 1 < 8) : h ⟨0, ph⟩ = a ⟨1, pa⟩ := by
    simpa using hA1.symm
  have hA2' (ph : 1 < 5) (pa : 2 < 8) : h ⟨1, ph⟩ = a ⟨2, pa⟩ := by
    simpa using hA2.symm
  have hA3' (ph : 2 < 5) (pa : 3 < 8) : h ⟨2, ph⟩ = a ⟨3, pa⟩ := by
    simpa using hA3.symm
  have hA4' (ph : 3 < 5) (pa : 4 < 8) : h ⟨3, ph⟩ = a ⟨4, pa⟩ := by
    simpa using hA4.symm
  have hA5' (ph : 4 < 5) (pa : 5 < 8) : h ⟨4, ph⟩ = a ⟨5, pa⟩ := by
    simpa using hA5.symm
  have hA6' (pr : 0 < 2) (pa : 6 < 8) : r ⟨0, pr⟩ = a ⟨6, pa⟩ := by
    simpa using hA6.symm
  have hA7' (pr : 1 < 2) (pa : 7 < 8) : r ⟨1, pr⟩ = a ⟨7, pa⟩ := by
  ...
  · rw [coreArc, if_pos huA]
    ...
    · rw [if_neg hvA, if_pos hvP]
      ...
      · rw [aToP, if_neg (by omega), if_pos (by omega),
          hToP_coreBits G.Adj p h r z a 3 (v - 8) (by omega) hvP',
          hA4' (by omega) (by omega)]
      · rw [aToP, if_neg (by omega), if_pos (by omega),
          hToP_coreBits G.Adj p h r z a 4 (v - 8) (by omega) hvP',
  ...
  · rw [coreArc, if_neg huA, if_pos hu]
    ...
    · rw [if_pos hvA]
      ...
      · rw [pToA, pToH_coreBits G.Adj p h r z a (u - 8) 2 huP (by omega),
          hA3' (by omega) (by omega)]
        simp [labelledVertex]
      · rw [pToA, pToH_coreBits G.Adj p h r z a (u - 8) 3 huP (by omega),
          hA4' (by omega) (by omega)]
        simp [labelledVertex]
      · rw [pToA, pToH_coreBits G.Adj p h r z a (u - 8) 4 huP (by omega),
          hA5' (by omega) (by omega)]
        simp [labelledVertex]
