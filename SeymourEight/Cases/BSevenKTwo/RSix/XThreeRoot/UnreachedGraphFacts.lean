import SeymourEight.Cases.BSevenKTwo.RSix.XThreeRoot.UnreachedEncoding
...
namespace SeymourEight.BSevenKTwo.RSix.XThreeRoot.UnreachedGraphFacts
...
noncomputable def retainedEquiv (C : G.LocalConfiguration) (q : V)
  ...
  let f : Fin 19 → {v : V // v ∈ retainedSet G C q} := fun i =>
    if hiA : i.val < 8 then
      ⟨(L.a ⟨i, hiA⟩).1,
        Finset.mem_union_left (externalTargets G C ∪ {q})
      ...
      (Finset.mem_union_right (externalTargets G C) (by simp))⟩
  ...
  · have hAP : Disjoint C.A C.P := by
    ...
    have hEAP : Disjoint (C.A ∪ C.P) (externalTargets G C ∪ {q}) := by
      rw [Finset.disjoint_left]
      intro v hvAP hvE
      rcases Finset.mem_union.mp hvAP with hvA | hvP
      · rcases Finset.mem_union.mp hvE with hvZ | hvq
    ...
    have he : (externalTargets G C ∪ {q}).card = 5 := by simp [hz, hqNotZ]
...
def representedTargetSet (C : G.LocalConfiguration) : Finset V := C.X ∪ externalTargets G C
...
noncomputable def representedTargetEquiv (C : G.LocalConfiguration) (q : V)
  ...
  let f : Fin 7 → {v : V // v ∈ representedTargetSet G C} := fun i ↦
    ⟨representedTargetVertex G L i.val, by
      ...
      · apply Finset.mem_union_right C.X
        change labelledVertex G L (UnreachedCore.secondTarget i.val) ∈ externalTargets G C
  ...
  · intro i j hij
    ...
    split at hIndex <;> split at hIndex <;> omega
  · have hDis : Disjoint C.X (externalTargets G C) := by
      rw [Finset.disjoint_left]
      intro v hvX hvZ
      exact (Finset.disjoint_left.mp
...
theorem directCount_graphBits_toNat (C : G.LocalConfiguration) (q : V)
  ...
  · simp only [labelledVertex, dif_neg hsA, dif_pos hs]
    ...
    · rcases Finset.mem_union.mp hvLocal with hvHP | hvQ
      ...
      · rw [hQ] at hvQ
        exact Finset.mem_union_right _ (Finset.mem_union_right _ hvQ)
    · exact Finset.mem_union_right _ (Finset.mem_union_left _ hvExt)
...
theorem nonSeymour_graphBits_true (C : G.LocalConfiguration) (q : V)
  ...
  rw [directCount_graphBits_toNat G C q hqQ hQ hqUnreached L hG source hs]
...
end SeymourEight.BSevenKTwo.RSix.XThreeRoot.UnreachedGraphFacts
