import SeymourEight.Cases.BSevenKTwo.RSix.XThreeRoot.Encoding
...
import SeymourEight.Cases.BSevenKTwo.RSeven.XThreeRoot.GraphFacts
...
namespace SeymourEight.BSevenKTwo.RSix.XThreeRoot.GraphFacts
...
def retainedSet (C : G.LocalConfiguration) (q : V) : Finset V :=
  C.A ∪ C.P ∪ ({q} ∪ externalTargets G C)

def labelledVertex (L : Labels G C q) (n : Nat) : V :=
  if hnA : n < 8 then (L.a ⟨n, hnA⟩).1
  else if hnP : n < 14 then (L.p ⟨n - 8, by omega⟩).1
  else if hnE : n < 18 then (L.e ⟨n - 14, by omega⟩).1
  else C.s

noncomputable def retainedEquiv (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented) :
    Fin 18 ≃ {v : V // v ∈ retainedSet G C q} := by
  let f : Fin 18 → {v : V // v ∈ retainedSet G C q} := fun i =>
    if hiA : i.val < 8 then
      ⟨(L.a ⟨i, hiA⟩).1,
        Finset.mem_union_left ({q} ∪ externalTargets G C)
          (Finset.mem_union_left C.P (L.a _).2)⟩
    else if hiP : i.val < 14 then
      ⟨(L.p ⟨i.val - 8, by omega⟩).1,
        Finset.mem_union_left ({q} ∪ externalTargets G C)
          (Finset.mem_union_right C.A (L.p _).2)⟩
    else ⟨(L.e ⟨i.val - 14, by omega⟩).1,
      Finset.mem_union_right (C.A ∪ C.P) (L.e _).2⟩
  ...
  · rintro ⟨v, hv⟩
    ...
    · obtain ⟨i, hi⟩ := L.e.surjective ⟨v, hvE⟩
      refine ⟨⟨i.val + 14, by omega⟩, ?_⟩
      apply Subtype.ext
      simpa [f, show ¬i.val + 14 < 8 by omega,
        show ¬i.val + 14 < 14 by omega] using congrArg Subtype.val hi
  · have hAP : Disjoint C.A C.P := by
    ...
    have hEAP : Disjoint (C.A ∪ C.P) ({q} ∪ externalTargets G C) := by
      rw [Finset.disjoint_left]
      intro v hvAP hvE
      rcases Finset.mem_union.mp hvAP with hvA | hvP
...
@[simp] theorem hEquiv_val (C : G.LocalConfiguration) (q : V)
    (L : Labels G C q) (hHCard : C.H.card = 5) (i : Fin 5) :
...
@[simp] theorem xEquiv_val (C : G.LocalConfiguration) (q : V)
    (L : Labels G C q) (hx : C.x = 3) (i : Fin 3) :
    (xEquiv G C q L hx i).1 = (L.a ⟨i.val + 3, by omega⟩).1 := rfl

@[simp] theorem retainedEquiv_val (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented) (i : Fin 18) :
    (retainedEquiv G C q hqQ L hG i).1 = labelledVertex G L i.val := by
  by_cases hiA : i.val < 8
  · simp [retainedEquiv, labelledVertex, hiA]
  by_cases hiP : i.val < 14
  · simp [retainedEquiv, labelledVertex, hiA, hiP]
  · simp [retainedEquiv, labelledVertex, hiA, hiP, i.isLt]

def pivotNeighborSet (C : G.LocalConfiguration) : Finset V := C.A1 ∪ C.P

def representedTargetSet (C : G.LocalConfiguration) (q : V) : Finset V :=
  C.X ∪ ({q} ∪ externalTargets G C)

def pivotNeighborVertex (L : Labels G C q) (i : Nat) : V :=
  labelledVertex G L (uVertex i)

def representedTargetVertex (L : Labels G C q) (i : Nat) : V :=
  labelledVertex G L (secondTarget i)

noncomputable def pivotNeighborEquiv (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented) (hk : C.k = 2) :
  ...
  let f : Fin 8 → {v : V // v ∈ pivotNeighborSet G C} := fun i ↦
    ⟨pivotNeighborVertex G L i.val, by
      ...
      · apply Finset.mem_union_left C.P
        change labelledVertex G L (uVertex i.val) ∈ C.A1
        rw [show uVertex i.val = 1 + i.val by simp [uVertex, hi]]
      ...
      · apply Finset.mem_union_right C.A1
        change labelledVertex G L (uVertex i.val) ∈ C.P
        rw [show uVertex i.val = 6 + i.val by simp [uVertex, hi]]
  ...
  constructor
  · intro i j hij
    have hIndex : uVertex i.val = uVertex j.val := by
      have hFin : (⟨uVertex i.val, by unfold uVertex; split <;> omega⟩ : Fin 18) =
          ⟨uVertex j.val, by unfold uVertex; split <;> omega⟩ := by
        apply (retainedEquiv G C q hqQ L hG).injective
        apply Subtype.ext
        simpa [retainedEquiv_val, f, pivotNeighborVertex] using
          congrArg Subtype.val hij
      exact Fin.ext_iff.mp hFin
    apply Fin.ext
    simp only [uVertex] at hIndex
...
@[simp] theorem pivotNeighborEquiv_val (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented)
    (hk : C.k = 2) (i : Fin 8) :
    (pivotNeighborEquiv G C q hqQ L hG hk i).1 =
      pivotNeighborVertex G L i.val := rfl

noncomputable def representedTargetEquiv (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented) (hx : C.x = 3) :
    Fin 7 ≃ {v : V // v ∈ representedTargetSet G C q} := by
  let f : Fin 7 → {v : V // v ∈ representedTargetSet G C q} := fun i ↦
    ⟨representedTargetVertex G L i.val, by
      by_cases hi : i.val < 3
      · apply Finset.mem_union_left ({q} ∪ externalTargets G C)
        change labelledVertex G L (secondTarget i.val) ∈ C.X
        rw [show secondTarget i.val = 3 + i.val by simp [secondTarget, hi]]
      ...
      · apply Finset.mem_union_right C.X
        change labelledVertex G L (secondTarget i.val) ∈ {q} ∪ externalTargets G C
        rw [show secondTarget i.val = 11 + i.val by simp [secondTarget, hi]]
  ...
  constructor
  · intro i j hij
    have hIndex : secondTarget i.val = secondTarget j.val := by
      have hFin : (⟨secondTarget i.val,
          by unfold secondTarget; split <;> omega⟩ : Fin 18) =
          ⟨secondTarget j.val, by unfold secondTarget; split <;> omega⟩ := by
        apply (retainedEquiv G C q hqQ L hG).injective
        apply Subtype.ext
      ...
      exact Fin.ext_iff.mp hFin
    apply Fin.ext
    simp only [secondTarget] at hIndex
    split at hIndex <;> split at hIndex <;> omega
  · have hDis : Disjoint C.X ({q} ∪ externalTargets G C) := by
      rw [Finset.disjoint_left]
      intro v hvX hvE
      rcases Finset.mem_union.mp hvE with hvq | hvZ
      · have hv : v = q := Finset.mem_singleton.mp hvq
        ...
        exact (Finset.disjoint_left.mp
          ...
          (Digraph.LocalConfiguration.Q_subset_B (G := G) C hqQ)
      · exact (Finset.disjoint_left.mp
          (BSixKThree.disjoint_H_externalTargets G C hG))
            (Finset.mem_union_right C.A1 hvX) hvZ
    rw [show Fintype.card {v : V // v ∈ representedTargetSet G C q} =
        (representedTargetSet G C q).card by simp,
      representedTargetSet, Finset.card_union_of_disjoint hDis]
    have he : ({q} ∪ externalTargets G C).card = 4 := by
      simpa using (show ({q} ∪ externalTargets G C).card = 4 from by
        rw [← Fintype.card_coe]
        have he' := (Fintype.card_congr L.e).symm
        norm_num at he'
        simpa only [Finset.mem_union, Finset.mem_singleton] using he')
...
@[simp] theorem representedTargetEquiv_val (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : Labels G C q) (hG : G.IsOriented)
    (hx : C.x = 3) (i : Fin 7) :
    (representedTargetEquiv G C q hqQ L hG hx i).1 =
    ...
    · rw [if_neg htA]
      ...
      · rw [if_pos htP]
        ...
        · rw [if_neg hs0, if_neg hsH,
            rToP_coreBits G.Adj L (source - 6) (target - 8) (by omega) (by omega)]
          have hfin : (⟨source - 6 + 6, by omega⟩ : Fin 8) =
              ⟨source, hsA⟩ := Fin.ext (by simp; omega)
          simp [labelledVertex, hsA, htA, htP, hfin]
      · by_cases htq : target = 14
        · subst target
          rw [if_pos rfl, aToQ_coreBits G.Adj L source hsA]
          have heq : (L.e 0).1 = q := L.e_zero
          by_cases hs0 : source = 0
          · subst source
            simp only [if_neg (by omega : ¬14 < 14), labelledVertex,
              dif_pos (by omega : 0 < 8), dif_neg (by omega : ¬14 < 8),
              dif_neg (by omega : ¬14 < 14), dif_pos ht, ne_eq,
              not_true_eq_false]
            apply (decide_eq_false_iff_not.mpr ?_).symm
            have ha0 : (⟨0, hsA⟩ : Fin 8) = 0 := Fin.ext (by simp)
            have he0 : (⟨14 - 14, by omega⟩ : Fin 4) = 0 := rfl
            rw [ha0, he0, L.e_zero]
            exact hA0Q
          · simp only [if_neg (by omega : ¬14 < 14), labelledVertex,
              dif_pos hsA, dif_neg (by omega : ¬14 < 8),
              dif_neg (by omega : ¬14 < 14), dif_pos ht]
            ...
            rw [he0, L.e_zero]
        · rw [if_neg htq]
          have htE : 14 < target := by omega
          have heZ := L.e_tail_Z ⟨target - 15, by omega⟩
          have hnot := RSeven.XThreeRoot.GraphFacts.A_not_adj_external
            G C hG (L.a ⟨source, hsA⟩).1 (L.e ⟨target - 14, by omega⟩).1
            (L.a _).2 (by
              simpa [show target - 15 + 1 = target - 14 by omega] using heZ)
          simp only [if_neg htP, labelledVertex, dif_pos hsA, dif_neg htA,
...
theorem directCount_graphBits_toNat (C : G.LocalConfiguration) (q : V)
    ...
    (Core.directCount (graphBits G L) source).toNat =
      G.outdegree (labelledVertex G L source) := by
  rw [Core.directCount, toNat_count_eq_fin_sum 18 _ (by omega)]
  have hCount : (∑ j : Fin 18, if coreArc (graphBits G L) source j then 1 else 0) =
      Shared.directCount G (retainedSet G C q) (labelledVertex G L source) := by
    symm
    apply directCount_eq_sum_bool G (retainedSet G C q)
      (retainedEquiv G C q hqQ L hG) _
    intro j
    rw [retainedEquiv_val, coreArc_graphBits G C q hqQ L hG source j hs j.isLt]
    simp
  rw [hCount]
  ...
  · simp only [labelledVertex, dif_pos hsA]
    ...
    · rw [← Digraph.LocalConfiguration.P_union_Q (G := G) C, hQ] at hvB
      ...
      · exact Finset.mem_union_right _ (Finset.mem_union_left _ hvq)
  · simp only [labelledVertex, dif_neg hsA, dif_pos hs]
    ...
    · rcases Finset.mem_union.mp hvLocal with hvHP | hvQ
      ...
      · rw [hQ] at hvQ
        exact Finset.mem_union_right _ (Finset.mem_union_left _ hvQ)
    · exact Finset.mem_union_right _ (Finset.mem_union_right _ hvExt)
...
theorem strictSecondLocal_true_mem (C : G.LocalConfiguration) (q : V)
  ...
  rw [coreArc_graphBits G C q hqQ L hG source middle hs (by omega)] at hFirst
  rw [coreArc_graphBits G C q hqQ L hG middle target hm ht] at hLast
  rw [coreArc_graphBits G C q hqQ L hG source target hs ht] at hNotArc
...
theorem localSecondCount_le_graph (C : G.LocalConfiguration) (q : V)
  ...
  have hFiltered := SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.GraphFacts.count_le_filterCard
    (V := V) (retainedSet G C q) (retainedEquiv G C q hqQ L hG)
    (strictSecondLocal (graphBits G L) source)
    (fun v => v ∈ G.secondOutNeighborFinset (labelledVertex G L source))
    (by omega) (by
      intro j hj
      rw [retainedEquiv_val]
      exact strictSecondLocal_true_mem G C q hqQ L hG source j hs j.isLt hj)
...
theorem nonSeymour_graphBits_true (C : G.LocalConfiguration) (q : V)
    ...
    (localSecondCount (graphBits G L) source).ult
      (Core.directCount (graphBits G L) source) = true := by
  simp only [BitVec.ult_eq_decide, decide_eq_true_eq]
  rw [directCount_graphBits_toNat G C q hqQ hQ L hG source hs]
  exact (localSecondCount_le_graph G C q hqQ L hG source hs).trans_lt
    (Digraph.secondOutdegree_lt_outdegree_of_not_seymour G
      (fun h => hNoSeymour ⟨labelledVertex G L source, h⟩))

theorem aOut_toNat (C : G.LocalConfiguration) (q : V) (L : Labels G C q)
...
theorem aPOut_toNat (C : G.LocalConfiguration) (q : V) (L : Labels G C q)
    (source : Nat) (hs : source < 8) :
  ...
  by_cases hsH : source < 6
  · rw [if_neg hs0, if_pos hsH,
      hToP_coreBits G.Adj L (source - 1) j (by omega) j.isLt]
    have ha : (⟨source - 1 + 1, by omega⟩ : Fin 8) =
        ⟨source, hs⟩ := Fin.ext (by simp; omega)
    simp [ha]
  · rw [if_neg hs0, if_neg hsH,
      rToP_coreBits G.Adj L (source - 6) j (by omega) j.isLt]
...
theorem aBOut_toNat (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (hQ : C.Q = {q}) (L : Labels G C q)
    (source : Nat) (hs : source < 8) :
    (aBOut (graphBits G L) source).toNat =
      Shared.directCount G C.B (L.a ⟨source, hs⟩).1 := by
  rw [aBOut, BitVec.toNat_add, aPOut_toNat G C q L source hs]
  have hqCount : (bitCount (aToQ (graphBits G L) source)).toNat =
...
theorem pBlockCounts (C : G.LocalConfiguration) (q : V) (L : Labels G C q)
    ...
    (pEOut (graphBits G L) p).toNat =
        Shared.directCount G ({q} ∪ externalTargets G C) (L.p ⟨p, hp⟩).1 := by
  constructor
  · rw [pOut, toNat_count_eq_fin_sum 6 _ (by omega)]
    ...
    · simp [hpj]
  constructor
  · rw [pHOut, toNat_count_eq_fin_sum 5 _ (by omega)]
    symm
    apply directCount_eq_sum_bool G C.H (hEquiv G C q L hHCard) _
    intro j
    rw [pToH_coreBits G.Adj L p j hp j.isLt]
    simp
  · rw [pEOut, toNat_count_eq_fin_sum 4 _ (by omega)]
    symm
    apply directCount_eq_sum_bool G ({q} ∪ externalTargets G C) L.e _
    intro j
    rw [pToE_coreBits G.Adj L p j hp j.isLt]
    simp

theorem hPOut_toNat (C : G.LocalConfiguration) (q : V) (L : Labels G C q)
    (h : Nat) (hh : h < 5) :
...
end SeymourEight.BSevenKTwo.RSix.XThreeRoot.GraphFacts
