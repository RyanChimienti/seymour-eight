import SeymourEight.Cases.BSevenKTwo.RSix.XThreeRoot.Labels
...
namespace SeymourEight.BSevenKTwo.RSix.XThreeRoot.Encoding
...
def coreBitAt {C : G.LocalConfiguration} {q : V} (L : Labels G C q) (n : Nat) : Bool :=
  if hnA : n < 64 then
  ...
  else if hnPE : n < 211 then
    let k := n - 176
    let i := k / 5
    let e := k % 5
    if hi : i < 6 then
      if he : e < 4 then decide (R (L.p ⟨i, hi⟩).1 (L.e ⟨e, he⟩).1)
      else false
    else if he : e < 2 then decide (R (L.a ⟨e + 6, by omega⟩).1 q)
    else false
...
def coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q) : Core.Encoding :=
  BitVec.cast (by simp only [List.length_ofFn])
    (BitVec.ofBoolListLE (List.ofFn fun n : Fin 225 => coreBitAt R L n))

@[simp] theorem getLsbD_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (n : Nat) (hn : n < 225) :
...
@[simp] theorem aArc_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
...
@[simp] theorem pArc_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
...
@[simp] theorem pToH_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (i h : Nat) (hi : i < 6) (hh : h < 5) :
...
@[simp] theorem hToP_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (h i : Nat) (hh : h < 5) (hi : i < 6) :
...
@[simp] theorem pToE_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (i e : Nat) (hi : i < 6) (he : e < 4) :
    Core.pToE (coreBits R L) i e =
      decide (R (L.p ⟨i, hi⟩).1 (L.e ⟨e, he⟩).1) := by
  ...
  have hm : (5 * i + e) % 5 = e := by
    simpa [Nat.mul_comm] using Nat.mul_add_mod_of_lt (a := i)
      (show e < 5 by omega)
  simp [coreBitAt, show ¬176 + 5 * i + e < 64 by omega,
    show ¬176 + 5 * i + e < 106 by omega,
    show ¬176 + 5 * i + e < 141 by omega,
    show ¬176 + 5 * i + e < 176 by omega,
    show 176 + 5 * i + e < 211 by omega,
    show 176 + 5 * i + e - 176 = 5 * i + e by omega, hd, hm, hi, he]

@[simp] theorem rToP_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (r i : Nat) (hr : r < 2) (hi : i < 6) :
...
@[simp] theorem aToQ_coreBits {C : G.LocalConfiguration} {q : V} (L : Labels G C q)
    (a : Nat) (ha : a < 8) :
...
end SeymourEight.BSevenKTwo.RSix.XThreeRoot.Encoding
