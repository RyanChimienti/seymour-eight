...
namespace SeymourEight.BSevenKTwo.RSix.XThreeRoot.Assembly
...
theorem allZReached_true (C : G.LocalConfiguration) (q : V)
  ...
  · by_cases hReach : ∃ p ∈ C.P, G.Adj p C.s
    · obtain ⟨p, hp, hps⟩ := hReach
      have hLabel : (L.e ⟨z + 1, by omega⟩).1 = C.s := by
      ...
      rw [pToE_coreBits G.Adj L i (1 + z) i.isLt (by omega)]
      have he : (⟨z + 1, by omega⟩ : Fin 4) = ⟨1 + z, by omega⟩ :=
        Fin.ext (Nat.add_comm z 1)
      rw [← he, hLabel, congrArg Subtype.val hi]
...
theorem pMinimumDegree_true (C : G.LocalConfiguration) (q : V)
  ...
  have hSmall' : (pOut (graphBits G L) p).toNat +
    ...
    have hH : Shared.directCount G C.H (L.p ⟨p, hp⟩).1 ≤ C.H.card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hE : Shared.directCount G ({q} ∪ externalTargets G C) (L.p ⟨p, hp⟩).1 ≤
        ({q} ∪ externalTargets G C).card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hpCard : C.P.card = 6 := by simpa using (Fintype.card_congr L.p).symm
    have heCard : ({q} ∪ externalTargets G C).card = 4 := by
      rw [← Fintype.card_coe]
  ...
  have hCaptured : G.outNeighborFinset (L.p ⟨p, hp⟩).1 ⊆
    ...
    · exact Or.inr (Or.inr hvExt)
  have hPH : Disjoint C.P C.H :=
    Digraph.LocalConfiguration.disjoint_H_P (G := G) C |>.symm
  have hPHE : Disjoint (C.P ∪ C.H) ({q} ∪ externalTargets G C) := by
  ...
  have hDegree := outdegree_eq_directCount_of_captured G
    (C.P ∪ C.H ∪ ({q} ∪ externalTargets G C)) (L.p ⟨p, hp⟩).1 hCaptured
  rw [directCount_union_of_disjoint G (C.P ∪ C.H) ({q} ∪ externalTargets G C) _ hPHE,
  ...
  change 8 ≤ Shared.directCount G C.P (L.p ⟨p, hp⟩).1 +
    Shared.directCount G C.H (L.p ⟨p, hp⟩).1 +
      Shared.directCount G ({q} ∪ externalTargets G C) (L.p ⟨p, hp⟩).1
  omega
...
theorem aNonSeymour_all_true (C : G.LocalConfiguration) (q : V)
  ...
  rw [all_eq_true_iff]
  intro a ha
  exact nonSeymour_graphBits_true G C q hqQ hQ L hG
...
theorem totalPToE_toNat (C : G.LocalConfiguration) (q : V)
    ...
    (totalPToE (graphBits G L)).toNat = edgeCount G C.P ({q} ∪ externalTargets G C) := by
  rw [totalPToE, toNat_sumCount]
  have hEach : ∀ i : Fin 6, (pEOut (graphBits G L) i).toNat =
      Shared.directCount G ({q} ∪ externalTargets G C) (L.p i).1 := by
    intro i
    exact (pBlockCounts G C q L hG hHCard i i.isLt).2.2
  have hSum : (∑ i ∈ Finset.range 6, (pEOut (graphBits G L) i).toNat) =
      edgeCount G C.P ({q} ∪ externalTargets G C) := by
    rw [edgeCount_eq_sum_fin G C.P ({q} ∪ externalTargets G C) L.p,
      ← Fin.sum_univ_eq_sum_range]
    exact Finset.sum_congr rfl (fun i _ => hEach i)
  rw [hSum]
  have hCap := edgeCount_le_card_mul_card G C.P ({q} ∪ externalTargets G C)
  have hp : C.P.card = 6 := by simpa using (Fintype.card_congr L.p).symm
...
theorem externalMissing_toNat (C : G.LocalConfiguration) (q : V)
    ...
    (externalMissing (graphBits G L)).toNat =
      24 - edgeCount G C.P ({q} ∪ externalTargets G C) := by
  ...
  change ((256 - edgeCount G C.P ({q} ∪ externalTargets G C) + 24) % 256) = _
  have heq : 256 - edgeCount G C.P ({q} ∪ externalTargets G C) + 24 =
      256 + (24 - edgeCount G C.P ({q} ∪ externalTargets G C)) := by omega
  rw [heq, Nat.add_mod, Nat.mod_self, zero_add]
  have hlt : 24 - edgeCount G C.P ({q} ∪ externalTargets G C) < 256 := by omega
...
theorem H_to_P_add_missing_le (C : G.LocalConfiguration) (q : V)
    ...
    edgeCount G C.H C.P + (24 - edgeCount G C.P ({q} ∪ externalTargets G C)) ≤ 21 := by
  have hCap := BSevenKTwo.P_degree_capacity_r_six G C hG hMin hr
  have hDis : Disjoint {q} (externalTargets G C) := by
    rw [Finset.disjoint_left]
    intro v hvq hvZ
    have hvEq : v = q := Finset.mem_singleton.mp hvq
    subst v
...
theorem orderedP_true (C : G.LocalConfiguration) (q : V)
  ...
  intro p hp
  simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
  have h1 := directCount_graphBits_toNat G C q hqQ hQ L hG
    (9 + p) (by omega)
  have h0 := directCount_graphBits_toNat G C q hqQ hQ L hG
    (8 + p) (by omega)
  rw [h1, h0]
...
theorem individualEffectiveLower_graph (C : G.LocalConfiguration) (q : V)
    ...
    (hmBound : 24 - edgeCount G C.P ({q} ∪ externalTargets G C) ≤ 5)
    (p : Nat) (hp : p < 6) :
    (individualEffectiveLower (graphBits G L) p).toNat ≤
      (RSix.XFourNoRoot.directAuxEffectiveUnion G C ({q} ∪ externalTargets G C)
        (L.p ⟨p, hp⟩).1).card := by
  let bits := graphBits G L
  ...
  have hEP : Disjoint E C.P := by
    ...
    · exact (Finset.disjoint_left.mp
        (BSixKThree.disjoint_B_externalTargets G C))
          (Digraph.LocalConfiguration.P_subset_B (G := G) C hwP) hwZ
  have hs : s ≤ 4 :=
    (Finset.card_le_card
...
theorem auxiliarySet_eq_E (C : G.LocalConfiguration) (q : V)
    ...
    RSix.XFourNoRoot.auxiliarySet G C = {q} ∪ externalTargets G C := by
...
theorem pEffectiveCondition_true (C : G.LocalConfiguration) (q : V)
  ...
  have hDegree : G.outdegree v = Shared.directCount G C.P v +
    ...
    have hCaptured : G.outNeighborFinset v ⊆ C.P ∪ C.H ∪ E := by
      ...
      · have hwq : w = q := by simpa [hQ] using hwQ
        subst w
        apply Or.inr
        exact Finset.mem_union_left (externalTargets G C) (Finset.mem_singleton_self q)
      · apply Or.inr
        exact Finset.mem_union_right {q} hwExt
    have hPH : Disjoint C.P C.H :=
      (Digraph.LocalConfiguration.disjoint_H_P (G := G) C).symm
    have hPHE : Disjoint (C.P ∪ C.H) E := by
      ...
      · rcases Finset.mem_union.mp hwE with hwq | hwZ
        ...
        · exact (Finset.disjoint_left.mp
            (BSixKThree.disjoint_H_externalTargets G C hG)) hwH hwZ
  ...
  have hRightSmall : (pOut bits p).toNat + 2 * (pHOut bits p).toNat +
    ...
    have heLe : (pEOut bits p).toNat ≤ 4 := by
      rw [hBlocks.2.2]
      have hc : ({q} ∪ externalTargets G C).card = 4 := by
        rw [← Fintype.card_coe]
        have he' := (Fintype.card_congr L.e).symm
        norm_num at he'
        simpa only [Finset.singleton_union, Finset.mem_insert,
...
theorem tightPrivate_true
  ...
  have hKSecond : K ⊆ T := by
    ...
    · rcases Finset.mem_union.mp hvE with hvq | hvZ
      ...
      · exact BSixKThree.externalTargets_subset_second_a1 G C hG hvZ
...
theorem anyExact_true (C : G.LocalConfiguration) (q : V)
  ...
  have hPE : edgeCount G C.P C.Q +
    ...
    rw [← hPEsplit]
    have h := edgeCount_le_card_mul_card G C.P ({q} ∪ externalTargets G C)
    have he : ({q} ∪ externalTargets G C).card = 4 := by
      rw [← Fintype.card_coe]
      have he' := (Fintype.card_congr L.e).symm
      norm_num at he'
  ...
  apply BitVec.eq_of_toNat_eq
  rw [directCount_graphBits_toNat G C q hqQ hQ L hG
    (8 + i.val) (by omega)]
...
end SeymourEight.BSevenKTwo.RSix.XThreeRoot.Assembly
