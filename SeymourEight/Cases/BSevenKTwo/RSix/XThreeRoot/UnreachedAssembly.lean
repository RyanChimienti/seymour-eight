import SeymourEight.Cases.BSevenKTwo.RSix.XThreeRoot.UnreachedGraphFacts
...
namespace SeymourEight.BSevenKTwo.RSix.XThreeRoot.UnreachedAssembly

open RSix.XThreeNoRoot
open Shared Shared.FiniteCore Labels UnreachedEncoding Core UnreachedCore UnreachedGraphFacts

variable {V : Type*} (G : Digraph V)
variable [Fintype V] [DecidableEq V] [DecidableRel G.Adj]

private abbrev graphBits (L : UnreachedLabels G C q) : Core.Encoding :=
...
theorem reachedQ_eq_empty_of_y_zero (C : G.LocalConfiguration)
    (hy : BSevenKTwo.y G C = 0) : reachedQ G C = ∅ := by
  apply Finset.card_eq_zero.mp
  exact hy

theorem q_not_reached_of_y_zero (C : G.LocalConfiguration) (q : V)
    (hy : BSevenKTwo.y G C = 0) : q ∉ reachedQ G C := by
  rw [reachedQ_eq_empty_of_y_zero G C hy]
  simp

theorem edgeCount_source_Q_eq_zero_of_y_zero (C : G.LocalConfiguration)
    (S : Finset V) (hS : S ⊆ C.A1 ∪ C.P) (hy : BSevenKTwo.y G C = 0) :
    edgeCount G S C.Q = 0 := by
  have hEmpty := reachedQ_eq_empty_of_y_zero G C hy
  ...
  have hvReached : v ∈ reachedQ G C := Finset.mem_inter.mpr ⟨hvQ,
    (Digraph.mem_outNeighborFinsetOf (G := G)).mpr ⟨u, hS hu, huv⟩⟩
  simp [hEmpty] at hvReached

/-- When `Q = {q}` is unreached from `A₁ ∪ P`, the two labelled `A₁`
vertices miss `q`, while some other labelled `A` vertex reaches it because
`q ∈ B = N⁺⁺(s)`. -/
theorem qStructure_true (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : UnreachedLabels G C q)
    (hy : BSevenKTwo.y G C = 0) :
    qStructure (graphBits G L) = true := by
  let bits := graphBits G L
  have hqUnreached := q_not_reached_of_y_zero G C q hy
  have hAOneMiss (i : Fin 2) :
      ¬G.Adj (L.a ⟨i.val + 1, by omega⟩).1 q := by
    intro hiq
    apply hqUnreached
    exact Finset.mem_inter.mpr ⟨hqQ,
      (Digraph.mem_outNeighborFinsetOf (G := G)).mpr
        ⟨(L.a ⟨i.val + 1, by omega⟩).1,
          Finset.mem_union_left C.P (L.a_aOne i), hiq⟩⟩
  have hFirst : ¬G.Adj (L.a (1 : Fin 8)).1 q := by
    intro hAdj
    exact hAOneMiss 0 (by simpa using hAdj)
  have hSecond : ¬G.Adj (L.a (2 : Fin 8)).1 q := by
    intro hAdj
    exact hAOneMiss 1 (by simpa using hAdj)
  have hqBMem : q ∈ C.B :=
  ...
  rcases hqB.1 with ⟨u, hsu, huq⟩
  have huA : u ∈ C.A := by
    exact (Digraph.mem_outNeighborFinset (G := G)).mpr hsu
  obtain ⟨i, hi⟩ := L.a.surjective ⟨u, huA⟩
  have hiVal : (L.a i).1 = u := congrArg Subtype.val hi
  have hiZero : i.val ≠ 0 := by
    intro hi0
    have hieq : i = 0 := Fin.ext hi0
    have hua1 : u = C.a1 := by
      rw [← hiVal, hieq, L.a_zero]
    apply (Finset.mem_sdiff.mp hqQ).2
    exact Finset.mem_filter.mpr ⟨hqBMem, by simpa [hua1] using huq⟩
  have hSome : any 7 (fun a => Core.aToQ bits (1 + a)) = true := by
    rw [any_eq_true_iff]
    refine ⟨i.val - 1, by omega, ?_⟩
    rw [aToQ_coreBits G.Adj L (1 + (i.val - 1)) (by omega)]
    have hIndex : (⟨1 + (i.val - 1), by omega⟩ : Fin 8) = i := by
      apply Fin.ext
      simp
      omega
    simp [hIndex, hiVal, huq]
  rw [qStructure, aToQ_coreBits G.Adj L 1 (by omega),
    aToQ_coreBits G.Adj L 2 (by omega)]
  simp [hFirst, hSecond]
  simpa [bits] using hSome
...
theorem fixedA_true (C : G.LocalConfiguration) (q : V)
    (L : UnreachedLabels G C q) (_hG : G.IsOriented) :
...
theorem everyXReached_true (C : G.LocalConfiguration) (q : V)
  ...
  · rw [Bool.or_eq_true]
    left
    rw [any_eq_true_iff]
    obtain ⟨i, hi⟩ := (UnreachedGraphFacts.aOneEquiv G C q L hk).surjective ⟨u, huA1⟩
...
theorem allZReached_true (C : G.LocalConfiguration) (q : V)
    (L : UnreachedLabels G C q) : UnreachedCore.allZReached (graphBits G L) = true := by
  rw [UnreachedCore.allZReached, all_eq_true_iff]
  intro z hz
  rw [any_eq_true_iff]
  ...
  · rcases (Digraph.mem_outNeighborFinsetOf (G := G)).mp
    ...
    refine ⟨i.val, i.isLt, ?_⟩
    rw [pToZ_coreBits G.Adj L i z i.isLt hz]
    have hiVal : (L.p i).1 = p := congrArg Subtype.val hi
    simpa [hiVal] using hpz
  · by_cases hReach : ∃ p ∈ C.P, G.Adj p C.s
    · obtain ⟨p, hp, hps⟩ := hReach
      have hLabel : (L.z ⟨z, hz⟩).1 = C.s := by
        simpa [rootSecondFinset, show ∃ p ∈ C.P, G.Adj p C.s from ⟨p, hp, hps⟩]
          using hRootMem
      obtain ⟨i, hi⟩ := L.p.surjective ⟨p, hp⟩
      refine ⟨i.val, i.isLt, ?_⟩
      rw [pToZ_coreBits G.Adj L i z i.isLt hz, hLabel,
        congrArg Subtype.val hi]
      exact decide_eq_true hps
...
theorem pMinimumDegree_true (C : G.LocalConfiguration) (q : V)
  ...
  have hSmall' : (pOut (graphBits G L) p).toNat +
    ...
    have hH : Shared.directCount G C.H (L.p ⟨p, hp⟩).1 ≤ C.H.card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hE : Shared.directCount G (externalTargets G C) (L.p ⟨p, hp⟩).1 ≤
        (externalTargets G C).card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hpCard : C.P.card = 6 := by simpa using (Fintype.card_congr L.p).symm
  ...
  have hCaptured : G.outNeighborFinset (L.p ⟨p, hp⟩).1 ⊆
      C.P ∪ C.H ∪ externalTargets G C := by
    intro v hv
    ...
    · have hvq : v = q := by simpa [hQ] using hvQ
      ...
      exact Finset.mem_inter.mpr ⟨hqQ,
        (Digraph.mem_outNeighborFinsetOf (G := G)).mpr
          ⟨(L.p ⟨p, hp⟩).1, Finset.mem_union_right C.A1 (L.p _).2,
            (Digraph.mem_outNeighborFinset (G := G)).mp hv⟩⟩
    · exact Or.inr hvExt
  have hPH : Disjoint C.P C.H :=
    Digraph.LocalConfiguration.disjoint_H_P (G := G) C |>.symm
  have hPHE : Disjoint (C.P ∪ C.H) (externalTargets G C) := by
    rw [Finset.disjoint_left]
    intro v hvPH hvE
    rcases Finset.mem_union.mp hvPH with hvP | hvH
    · exact (Finset.disjoint_left.mp (BSixKThree.disjoint_B_externalTargets G C))
        (Digraph.LocalConfiguration.P_subset_B (G := G) C hvP) hvE
    · exact (Finset.disjoint_left.mp
        (BSixKThree.disjoint_H_externalTargets G C hG)) hvH hvE
  have hDegree := outdegree_eq_directCount_of_captured G
    (C.P ∪ C.H ∪ externalTargets G C) (L.p ⟨p, hp⟩).1 hCaptured
  rw [directCount_union_of_disjoint G (C.P ∪ C.H) (externalTargets G C) _ hPHE,
...
theorem aNonSeymour_all_true (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (hQ : C.Q = {q}) (L : UnreachedLabels G C q)
    (hG : G.IsOriented)
    (hNoSeymour : ¬G.HasSeymourVertex) (hy : BSevenKTwo.y G C = 0) :
    all 8 (UnreachedCore.aNonSeymour (graphBits G L)) = true := by
  rw [all_eq_true_iff]
  intro a ha
  exact nonSeymour_graphBits_true G C q hqQ hQ
    (q_not_reached_of_y_zero G C q hy) L hG
...
theorem pNonSeymour_all_true (C : G.LocalConfiguration) (q : V)
    ...
    (hG : G.IsOriented)
    (hNoSeymour : ¬G.HasSeymourVertex) (hy : BSevenKTwo.y G C = 0) :
    all 6 (UnreachedCore.pNonSeymour (graphBits G L)) = true := by
  rw [all_eq_true_iff]
  intro p hp
  exact nonSeymour_graphBits_true G C q hqQ hQ
    (q_not_reached_of_y_zero G C q hy) L hG
...
theorem totalPToE_toNat (C : G.LocalConfiguration) (q : V)
    (L : UnreachedLabels G C q) (hG : G.IsOriented) (hHCard : C.H.card = 5) :
    (totalPToE (graphBits G L)).toNat = edgeCount G C.P (externalTargets G C) := by
...
theorem externalMissing_toNat (C : G.LocalConfiguration) (q : V)
    ...
    (externalMissing (graphBits G L)).toNat =
      24 - edgeCount G C.P (externalTargets G C) := by
  ...
  change ((256 - edgeCount G C.P (externalTargets G C) + 24) % 256) = _
  have heq : 256 - edgeCount G C.P (externalTargets G C) + 24 =
      256 + (24 - edgeCount G C.P (externalTargets G C)) := by omega
  rw [heq, Nat.add_mod, Nat.mod_self, zero_add]
  have hlt : 24 - edgeCount G C.P (externalTargets G C) < 256 := by omega
...
theorem aOneToQ_toNat (C : G.LocalConfiguration) (q : V)
    (_hqQ : q ∈ C.Q) (L : UnreachedLabels G C q) (hk : C.k = 2) :
...
theorem H_to_P_lower (C : G.LocalConfiguration) (q : V) (hQ : C.Q = {q})
  ...
  have hA1Q : edgeCount G C.A1 C.Q = 0 :=
    edgeCount_source_Q_eq_zero_of_y_zero G C C.A1
      (Finset.subset_union_left) hy
  ...
  have hSplit : edgeCount G C.H C.Q =
    ...
    simpa [Digraph.LocalConfiguration.H] using
      BSixKThree.edgeCount_source_union G C.A1 C.X C.Q
        (Digraph.LocalConfiguration.disjoint_A1_X (G := G) C)
  have hQBound : edgeCount G C.H C.Q ≤ 3 := by omega
...
theorem H_to_P_add_missing_le (C : G.LocalConfiguration)
    (hG : G.IsOriented) (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hr : C.r = 6)
    (hHCard : C.H.card = 5) (hZCard : (externalTargets G C).card = 4)
    (hy : BSevenKTwo.y G C = 0) :
    edgeCount G C.H C.P + (24 - edgeCount G C.P (externalTargets G C)) ≤ 21 := by
  have hCap := BSevenKTwo.P_degree_capacity_r_six G C hG hMin hr
  have hPQ : edgeCount G C.P C.Q = 0 :=
    edgeCount_source_Q_eq_zero_of_y_zero G C C.P
      Finset.subset_union_right hy
  rw [hHCard, hPQ, zero_add] at hCap
  have hPECap := edgeCount_le_card_mul_card G C.P (externalTargets G C)
  have hpCard : C.P.card = 6 := hr
  rw [hpCard, hZCard] at hPECap
  omega
...
theorem orderedP_true (C : G.LocalConfiguration) (q : V)
  ...
  simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
  have hqUnreached := q_not_reached_of_y_zero G C q hy
  have h1 := directCount_graphBits_toNat G C q hqQ hQ hqUnreached L hG
    (8 + (p + 1)) (by omega)
  have h0 := directCount_graphBits_toNat G C q hqQ hQ hqUnreached L hG
    (8 + p) (by omega)
  rw [show 9 + p = 8 + (p + 1) by omega,
    UnreachedCore.pDirectCount_eq (graphBits G L) (p + 1) (by omega),
    UnreachedCore.pDirectCount_eq (graphBits G L) p (by omega), h1, h0]
  simp only [labelledVertex, dif_neg (by omega : ¬8 + (p + 1) < 8),
    dif_pos (by omega : 8 + (p + 1) < 14), dif_neg (by omega : ¬8 + p < 8),
    dif_pos (by omega : 8 + p < 14)]
  have he1 : (⟨8 + (p + 1) - 8, by omega⟩ : Fin 6) =
      ⟨p + 1, by omega⟩ := Fin.ext (show 8 + (p + 1) - 8 = p + 1 by omega)
...
theorem individualEffectiveLower_graph (C : G.LocalConfiguration) (q : V)
    (_hqQ : q ∈ C.Q) (L : UnreachedLabels G C q) (hG : G.IsOriented)
    (hMin : ∀ v, 8 ≤ G.outdegree v) (hHCard : C.H.card = 5)
    (hmBound : 24 - edgeCount G C.P (externalTargets G C) ≤ 5)
    (p : Nat) (hp : p < 6) :
    (individualEffectiveLower (graphBits G L) p).toNat ≤
      (RSix.XFourNoRoot.directAuxEffectiveUnion G C (externalTargets G C)
        (L.p ⟨p, hp⟩).1).card := by
  let bits := graphBits G L
  ...
  have hEP : Disjoint E C.P := by
    ...
    intro w hwE hwP
    exact (Finset.disjoint_left.mp (BSixKThree.disjoint_B_externalTargets G C))
      (Digraph.LocalConfiguration.P_subset_B (G := G) C hwP) hwE
  have hs : s ≤ 4 :=
    (Finset.card_le_card
...
theorem pSecondPCount_le_graph (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (L : UnreachedLabels G C q) (hG : G.IsOriented)
    (hy : BSevenKTwo.y G C = 0)
    (p : Nat) (hp : p < 6) :
  ...
  apply RSeven.XThreeNoRoot.GraphFacts.count_le_filterCard C.P L.p
    (fun j => UnreachedCore.strictSecondLocal (graphBits G L) (8 + p) (8 + j))
    (fun v => v ∈ G.secondOutNeighborFinset (L.p ⟨p, hp⟩).1)
    (by omega)
  intro j hj
  have hm := strictSecondLocal_true_mem G C q hqQ
    (q_not_reached_of_y_zero G C q hy) L hG
...
theorem auxiliarySet_eq_E (C : G.LocalConfiguration) (q : V)
    (_hQ : C.Q = {q}) (hy : BSevenKTwo.y G C = 0) :
    RSix.XFourNoRoot.auxiliarySet G C = externalTargets G C := by
  have hReached : reachedQ G C = ∅ := reachedQ_eq_empty_of_y_zero G C hy
  simp [RSix.XFourNoRoot.auxiliarySet, hReached]

theorem pEffectiveCondition_true (C : G.LocalConfiguration) (q : V)
    (hqQ : q ∈ C.Q) (hQ : C.Q = {q}) (L : UnreachedLabels G C q)
    (hG : G.IsOriented) (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hNoSeymour : ¬G.HasSeymourVertex)
    (hHCard : C.H.card = 5) (hy : BSevenKTwo.y G C = 0)
    (hmBound : 24 - edgeCount G C.P (externalTargets G C) ≤ 5) :
    all 6 (pEffectiveCondition (graphBits G L)) = true := by
  let bits := graphBits G L
  let E := externalTargets G C
  have hAux : E = RSix.XFourNoRoot.auxiliarySet G C :=
    (auxiliarySet_eq_E G C q hQ hy).symm
  rw [all_eq_true_iff]
  intro p hp
  let v := (L.p ⟨p, hp⟩).1
  have hvP : v ∈ C.P := (L.p ⟨p, hp⟩).2
  ...
  have hTable := individualEffectiveLower_graph G C q hqQ L hG hMin
    hHCard hmBound p hp
  have hPS := pSecondPCount_le_graph G C q hqQ L hG hy p hp
  ...
  have hDegree : G.outdegree v = Shared.directCount G C.P v +
    ...
    have hCaptured : G.outNeighborFinset v ⊆ C.P ∪ C.H ∪ E := by
      ...
      · have hwq : w = q := by simpa [hQ] using hwQ
        ...
        exact Finset.mem_inter.mpr ⟨hqQ,
          (Digraph.mem_outNeighborFinsetOf (G := G)).mpr
            ⟨v, Finset.mem_union_right C.A1 hvP,
    ...
    have hPHE : Disjoint (C.P ∪ C.H) E := by
      ...
      · exact (Finset.disjoint_left.mp (BSixKThree.disjoint_B_externalTargets G C))
          (Digraph.LocalConfiguration.P_subset_B (G := G) C hwP) hwE
      · exact (Finset.disjoint_left.mp
          (BSixKThree.disjoint_H_externalTargets G C hG)) hwH hwE
  ...
  have hRightSmall : (pOut bits p).toNat + 2 * (pHOut bits p).toNat +
    ...
    have heLe : (pEOut bits p).toNat ≤ 4 := by
      rw [hBlocks.2.2]
      have hc : (externalTargets G C).card = 4 := by
        simpa using (Fintype.card_congr L.z).symm
      exact (Finset.card_le_card (Finset.filter_subset _ _)).trans_eq hc
    omega
...
theorem tightPrivate_true
    ...
    (hqQ : q ∈ C.Q) (_hQ : C.Q = {q}) (L : UnreachedLabels G C q)
    (hG : G.IsOriented) (hNoSeymour : ¬G.HasSeymourVertex)
    (hk : C.k = 2) (hr : C.r = 6)
    (hx : C.x = 3) (hy : BSevenKTwo.y G C = 0) :
    UnreachedCore.tightPrivate (graphBits G L) = true := by
  let bits := graphBits G L
  let K := representedTargetSet G C
  ...
  have hKSecond : K ⊆ T := by
    ...
    · exact BSixKThree.externalTargets_subset_second_a1 G C hG hvE
  have hTCard : T.card ≤ 7 := by
    ...
    omega
  rw [UnreachedCore.tightPrivate, all_eq_true_iff]
  ...
  have huOut : u ∈ G.outNeighborFinset C.a1 := by
    rw [hOut]
    exact (pivotNeighborEquiv G C q hqQ L hG hk d).2
  ...
  have hTargetSecond (target : Fin 7) :
    ...
    apply hKSecond
    exact (representedTargetEquiv G C q hqQ L hG hx target).2
  have hPrivateNotE (target : Fin 7)
      (hPrivate : UnreachedCore.privateTarget bits deleted target.val = true) :
    ...
    obtain ⟨other, hother⟩ :=
      (pivotNeighborEquiv G C q hqQ L hG hk).surjective ⟨middle, hmU⟩
    ...
    simp only [UnreachedCore.privateTarget, Bool.and_eq_true] at hPrivate
    ...
    have hArc := coreArc_graphBits G C q hqQ
      (q_not_reached_of_y_zero G C q hy) L hG
      (UnreachedCore.uVertex other.val) (UnreachedCore.secondTarget target.val)
      (by unfold UnreachedCore.uVertex; split <;> omega)
      (by unfold UnreachedCore.secondTarget; split <;> omega)
    have hArcTrue : UnreachedCore.coreArc bits (UnreachedCore.uVertex other.val)
        (UnreachedCore.secondTarget target.val) = true := by
      rw [hArc]
  ...
  have hMiss : (T \ E).card ≤ 1 := by
    simpa [T, S, E] using Digraph.oneArcDeletion_misses_at_most_one G hBound hG
      hNoSeymour hDegree hau
  have hCountOne : (count 7 (UnreachedCore.privateTarget bits deleted)).toNat ≤ 1 := by
    have hFilter := RSeven.XThreeNoRoot.GraphFacts.count_le_filterCard K
      (representedTargetEquiv G C q hqQ L hG hx)
      (UnreachedCore.privateTarget bits deleted) (fun v ↦ v ∈ T \ E) (by omega) (by
        intro target ht
  ...
  by_cases hReached : UnreachedCore.deletedReached bits deleted = true
  · simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
    change (count 7 (UnreachedCore.privateTarget bits deleted)).toNat ≤
      (bitCount (UnreachedCore.deletedReached bits deleted)).toNat
    rw [hReached]
    simpa [bitCount] using hCountOne
  · have hReachedFalse : UnreachedCore.deletedReached bits deleted = false :=
    ...
    have huNotE : u ∉ E := by
      ...
      have hArc := coreArc_graphBits G C q hqQ
        (q_not_reached_of_y_zero G C q hy) L hG
        (UnreachedCore.uVertex other.val) (UnreachedCore.uVertex deleted)
        (by unfold UnreachedCore.uVertex; split <;> omega)
        (by unfold UnreachedCore.uVertex; split <;> omega)
      have hArcTrue : UnreachedCore.coreArc bits
          (UnreachedCore.uVertex other.val) (UnreachedCore.uVertex deleted) = true := by
        rw [hArc]
        apply decide_eq_true
        change G.Adj (pivotNeighborVertex G L other.val) u
        simpa [hmiddle] using hmu
      have : UnreachedCore.deletedReached bits deleted = true := by
        rw [UnreachedCore.deletedReached, any_eq_true_iff]
    ...
    have hNoPrivate : ∀ target : Fin 7,
        UnreachedCore.privateTarget bits deleted target.val = false := by
      intro target
      ...
      omega
    have hCountZero : (count 7 (UnreachedCore.privateTarget bits deleted)).toNat = 0 := by
...
theorem anyExact_true (C : G.LocalConfiguration) (q : V)
  ...
  have hHCard : C.H.card = 5 := by
    rw [BSevenKTwo.H_card_eq_x_add_two G C hk, hx]
  have hHP := H_to_P_lower G C q hQ hG hMin hk hx hRCard hy
  have hCross := cross_edgeCount_add_reverse_le G C.H C.P hG
  rw [hHCard, hPCard] at hCross
  have hPH : edgeCount G C.P C.H ≤ 12 := by omega
  ...
  have hPQ : edgeCount G C.P C.Q = 0 :=
    edgeCount_source_Q_eq_zero_of_y_zero G C C.P
      Finset.subset_union_right hy
  have hPZ : edgeCount G C.P (externalTargets G C) ≤ 24 := by
    have h := edgeCount_le_card_mul_card G C.P (externalTargets G C)
    have hz : (externalTargets G C).card = 4 := by
      simpa using (Fintype.card_congr L.z).symm
    rw [hPCard, hz] at h
    exact h
  have hAccounting := BSixKThree.degreeSum_P_eq_blocks G C hG
  have hSumUpper : ∑ p ∈ C.P, G.outdegree p ≤ 51 := by omega
  ...
  apply BitVec.eq_of_toNat_eq
  rw [UnreachedCore.pDirectCount_eq (graphBits G L) i.val i.isLt,
    directCount_graphBits_toNat G C q hqQ hQ
      (q_not_reached_of_y_zero G C q hy) L hG
      (8 + i.val) (by omega)]
...
end SeymourEight.BSevenKTwo.RSix.XThreeRoot.UnreachedAssembly
