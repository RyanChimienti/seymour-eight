import SeymourEight.Cases.BSevenKTwo.RSix.XThreeNoRoot.UnreachedAssembly
...
theorem unreached_impossible
  ...
  have hPMin := UnreachedAssembly.pMinimumDegree_true
    G C q hqQ hQ L hG hHCard hMin hNoRoot hy
  have hANS := UnreachedAssembly.aNonSeymour_all_true
    G C q hqQ hQ L hG hNoRoot hNoSeymour hy
  ...
  have hTightNative := UnreachedAssembly.tightPrivate_true G hBound C q hqQ hQ L
    hG hNoSeymour hNoRoot hk hr hx hy
  ...
  have hPOrder := UnreachedAssembly.orderedP_true G C q hqQ hQ L hG hNoRoot hy (by
    intro i
  ...
  have hHPUpperGraph := UnreachedAssembly.H_to_P_add_missing_le
    G C hG hMin hr hNoRoot hHCard hZCard hy
  have hmBound : 24 - edgeCount G C.P C.Z ≤ 5 := by omega
  have hEffective := UnreachedAssembly.pEffectiveCondition_true
    G C q hqQ hQ L hG hMin hNoSeymour hNoRoot hHCard hy hmBound
  have hAnyExact := UnreachedAssembly.anyExact_true
    G C q hqQ hQ L hG hMin hNoRoot hk hr hx hRCard hy
  ...
  have hUpper : (Core.totalHToP bits + Core.externalMissing bits).ule 21 = true := by
    ...
    have hSmall : edgeCount G C.H C.P +
        (24 - edgeCount G C.P C.Z) < 256 := by omega
