import SeymourEight.Cases.BSevenKTwo.RSix.XThreeNoRoot.UnreachedGraphFacts
...
namespace SeymourEight.BSevenKTwo.RSix.XThreeNoRoot.UnreachedAssembly
...
theorem pMinimumDegree_true (C : G.LocalConfiguration) (q : V)
  ...
  have hSmall' : (pOut (graphBits G L) p).toNat +
    ...
    have hH : Shared.directCount G C.H (L.p ⟨p, hp⟩).1 ≤ C.H.card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hE : Shared.directCount G C.Z (L.p ⟨p, hp⟩).1 ≤
        C.Z.card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hpCard : C.P.card = 6 := by simpa using (Fintype.card_congr L.p).symm
    have heCard : C.Z.card = 4 := by
  ...
  have hCaptured : G.outNeighborFinset (L.p ⟨p, hp⟩).1 ⊆
    ...
    · exact Or.inr (hExt ▸ hvExt)
  have hPH : Disjoint C.P C.H :=
    Digraph.LocalConfiguration.disjoint_H_P (G := G) C |>.symm
  have hPHE : Disjoint (C.P ∪ C.H) C.Z := by
    rw [Finset.disjoint_left]
    intro v hvPH hvE
    rcases Finset.mem_union.mp hvPH with hvP | hvH
    · exact (Finset.disjoint_left.mp
        (Digraph.LocalConfiguration.disjoint_Z_P (G := G) C)) hvE hvP
    · exact (Finset.disjoint_left.mp
        (Digraph.LocalConfiguration.disjoint_Z_H (G := G) C)) hvE hvH
  have hDegree := outdegree_eq_directCount_of_captured G
    (C.P ∪ C.H ∪ C.Z) (L.p ⟨p, hp⟩).1 hCaptured
  rw [directCount_union_of_disjoint G (C.P ∪ C.H) C.Z _ hPHE,
...
theorem aNonSeymour_all_true (C : G.LocalConfiguration) (q : V)
  ...
  exact nonSeymour_graphBits_true G C q hqQ hQ
    (q_not_reached_of_y_zero G C q hy) L hG hNoRoot
...
theorem totalPToE_toNat (C : G.LocalConfiguration) (q : V)
    ...
    (totalPToE (graphBits G L)).toNat = edgeCount G C.P C.Z := by
...
theorem externalMissing_toNat (C : G.LocalConfiguration) (q : V)
  ...
  change ((256 - edgeCount G C.P C.Z + 24) % 256) = _
  have heq : 256 - edgeCount G C.P C.Z + 24 =
      256 + (24 - edgeCount G C.P C.Z) := by omega
  rw [heq, Nat.add_mod, Nat.mod_self, zero_add]
  have hlt : 24 - edgeCount G C.P C.Z < 256 := by omega
...
theorem H_to_P_add_missing_le (C : G.LocalConfiguration)
    (hG : G.IsOriented) (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hr : C.r = 6) (hNoRoot : epsilonS G C = 0)
    (hHCard : C.H.card = 5) (hZCard : C.Z.card = 4)
    (hy : BSevenKTwo.y G C = 0) :
    edgeCount G C.H C.P + (24 - edgeCount G C.P C.Z) ≤ 21 := by
  ...
  rw [hHCard, hPQ, zero_add, hExt] at hCap
  have hPECap := edgeCount_le_card_mul_card G C.P C.Z
...
theorem orderedP_true (C : G.LocalConfiguration) (q : V)
  ...
  simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
  have hqUnreached := q_not_reached_of_y_zero G C q hy
  have h1 := directCount_graphBits_toNat G C q hqQ hQ hqUnreached L hG hNoRoot
    (8 + (p + 1)) (by omega)
  have h0 := directCount_graphBits_toNat G C q hqQ hQ hqUnreached L hG hNoRoot
    (8 + p) (by omega)
...
theorem individualEffectiveLower_graph (C : G.LocalConfiguration) (q : V)
    ...
    (hmBound : 24 - edgeCount G C.P C.Z ≤ 5)
    (p : Nat) (hp : p < 6) :
    (individualEffectiveLower (graphBits G L) p).toNat ≤
      (RSix.XFourNoRoot.directAuxEffectiveUnion G C C.Z
        (L.p ⟨p, hp⟩).1).card := by
  let bits := graphBits G L
  let E := C.Z
  let v := (L.p ⟨p, hp⟩).1
  ...
  have hEP : Disjoint E C.P := by
    ...
    intro w hwE hwP
    exact (Finset.disjoint_left.mp
      (Digraph.LocalConfiguration.disjoint_Z_P (G := G) C)) hwE hwP
  have hs : s ≤ 4 :=
    (Finset.card_le_card
...
theorem auxiliarySet_eq_E (C : G.LocalConfiguration) (q : V)
    ...
    RSix.XFourNoRoot.auxiliarySet G C = C.Z := by
...
theorem pEffectiveCondition_true (C : G.LocalConfiguration) (q : V)
  ...
  have hDegree : G.outdegree v = Shared.directCount G C.P v +
    ...
    have hCaptured : G.outNeighborFinset v ⊆ C.P ∪ C.H ∪ E := by
      ...
      · exact Or.inr (by simpa [hExt] using hwExt)
    have hPH : Disjoint C.P C.H :=
      (Digraph.LocalConfiguration.disjoint_H_P (G := G) C).symm
    have hPHE : Disjoint (C.P ∪ C.H) E := by
      ...
      · exact (Finset.disjoint_left.mp
          (Digraph.LocalConfiguration.disjoint_Z_P (G := G) C)) hwE hwP
      · exact (Finset.disjoint_left.mp
          (Digraph.LocalConfiguration.disjoint_Z_H (G := G) C)) hwE hwH
...
theorem tightPrivate_true
  ...
  have hKSecond : K ⊆ T := by
    ...
    · have hRootEmpty : rootSecondFinset G C = ∅ := by
      ...
      simpa [externalTargets, hRootEmpty] using hvE
...
theorem anyExact_true (C : G.LocalConfiguration) (q : V)
  ...
  have hPZ : edgeCount G C.P (externalTargets G C) ≤ 24 := by
    rw [hExternal]
    have h := edgeCount_le_card_mul_card G C.P C.Z
...
end SeymourEight.BSevenKTwo.RSix.XThreeNoRoot.UnreachedAssembly
