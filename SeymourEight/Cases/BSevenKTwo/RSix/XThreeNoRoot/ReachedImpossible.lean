...
theorem reached_impossible
  ...
  have hPMin := pMinimumDegree_true G C q hqQ hQ L hG hHCard hMin hNoRoot
  have hANS := aNonSeymour_all_true G C q hqQ hQ L hG hNoRoot hNoSeymour
  have hPNS := pNonSeymour_all_true G C q hqQ hQ L hG hNoRoot hNoSeymour
  have hTight := tightPrivate_true G hBound C q hqQ hQ L hG hNoSeymour
    hNoRoot hk hr hx hy
  have hPOrder := orderedP_true G C q hqQ hQ L hG hNoRoot (by
    intro i
  ...
  have hmBound : 24 - edgeCount G C.P ({q} ∪ C.Z) ≤ 5 := by omega
  have hEffective := pEffectiveCondition_true G C q hqQ hQ L hG hMin
    hNoSeymour hNoRoot hHCard hy hmBound
  have hAnyExact := anyExact_true G C q hqQ hQ L hG hMin hNoRoot hk hr
    hx hRCard hy
  ...
  have hUpper : (totalHToP bits + externalMissing bits).ule 21 = true := by
    ...
    change (edgeCount G C.H C.P +
      (24 - edgeCount G C.P ({q} ∪ C.Z))) % 256 ≤ 21
    have hSmall : edgeCount G C.H C.P +
        (24 - edgeCount G C.P ({q} ∪ C.Z)) < 256 := by omega
