...
/-!
# Canonical labels for the no-root `r = 7`, `x = 2` core

The fixed `A` layout is `a1, A1[2], X[2], R[3]`.  Each structural class,
`P`, and `Z` is sorted only by a graph-invariant degree key.
-/

namespace SeymourEight.BSevenKTwo.RSeven.XTwoNoRoot.Labels
...
noncomputable def aLabelEquiv (C : G.LocalConfiguration)
    (hACard : C.A.card = 8) (h : Fin 4 ≃ {v : V // v ∈ C.H})
    (eR : Fin 3 ≃ {v : V // v ∈ C.R}) :
    Fin 8 ≃ {v : V // v ∈ C.A} := by
  ...
  · rintro ⟨v, hv⟩
    ...
    · obtain ⟨i, hi⟩ := eR.surjective ⟨v, hvR⟩
      ...
      apply Subtype.ext
      simpa [f, show i.val + 5 ≠ 0 by omega,
        show ¬i.val + 5 ≤ 4 by omega] using congrArg Subtype.val hi
...
@[simp] theorem aLabelEquiv_r (C : G.LocalConfiguration)
    (hACard : C.A.card = 8) (h : Fin 4 ≃ {v : V // v ∈ C.H})
    (eR : Fin 3 ≃ {v : V // v ∈ C.R}) (i : Fin 3) :
    (aLabelEquiv G C hACard h eR ⟨i.val + 5, by omega⟩).1 = (eR i).1 := by
  simp [aLabelEquiv,
    show ¬i.val + 5 ≤ 4 by omega]

def pKey (C : G.LocalConfiguration) (v : V) : Nat :=
  (4096 * G.outdegree v + 256 * directCount G C.Z v +
...
noncomputable def canonicalLabels (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 2)
    (hRCard : C.R.card = 3) (hHCard : C.H.card = 4)
    (hZCard : C.Z.card = zCount) : Labels G zCount C := by
  ...
  let eX0 : Fin 2 ≃ {v : V // v ∈ C.X} := finsetEquivFin C.X hXCard
  let eX := sortedFinsetEquiv (structuralKey G C) C.X eX0
  let h := hLabelEquiv G C hHCard eA1 eX
  let eR0 : Fin 3 ≃ {v : V // v ∈ C.R} := finsetEquivFin C.R hRCard
  let eR := sortedFinsetEquiv (structuralKey G C) C.R eR0
  let a := aLabelEquiv G C hACard h eR
  let eZ0 : Fin zCount ≃ {v : V // v ∈ C.Z} := finsetEquivFin C.Z hZCard
  let z := sortedFinsetEquiv (zKey G (fun i => (p i).1)) C.Z eZ0
  refine {
    p := p
    a := a
    z := z
...
theorem aLabelEquiv_aOne_key_order (C : G.LocalConfiguration)
    (hACard : C.A.card = 8) (hHCard : C.H.card = 4)
    (eA1 : Fin 2 ≃ {v : V // v ∈ C.A1})
    (eX : Fin 2 ≃ {v : V // v ∈ C.X})
    (eR : Fin 3 ≃ {v : V // v ∈ C.R})
    (hOrder : ∀ q : Fin 1,
...
theorem canonicalLabels_p_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 2)
    (hRCard : C.R.card = 3) (hHCard : C.H.card = 4)
    (hZCard : C.Z.card = zCount) (q : Fin 6) :
...
theorem canonicalLabels_z_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 2)
    (hRCard : C.R.card = 3) (hHCard : C.H.card = 4)
    (hZCard : C.Z.card = zCount) (q : Fin (zCount - 1)) :
    ...
    (sorted_key_anti
      (zKey G (fun i ↦
        ((sortedFinsetEquiv (pKey G C) C.P
          (finsetEquivFin C.P hPCard)) i).1)) C.Z
      (finsetEquivFin C.Z hZCard)
      (i := ⟨q.val, by omega⟩) (j := ⟨q.val + 1, by omega⟩)
      (hij := by simp))
...
theorem canonicalLabels_aOne_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 2)
    (hRCard : C.R.card = 3) (hHCard : C.H.card = 4)
    (hZCard : C.Z.card = zCount) (q : Fin 1) :
    let L := canonicalLabels G zCount C hPCard hACard hA1Card hXCard
      hRCard hHCard hZCard
...
theorem canonicalLabels_r_order (zCount : Nat) (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 2)
    (hRCard : C.R.card = 3) (hHCard : C.H.card = 4)
    (hZCard : C.Z.card = zCount) (q : Fin 2) :
    let L := canonicalLabels G zCount C hPCard hACard hA1Card hXCard
      hRCard hHCard hZCard
    structuralKey G C (L.a ⟨q.val + 6, by omega⟩).1 ≤
      structuralKey G C (L.a ⟨q.val + 5, by omega⟩).1 := by
  dsimp only
  simpa only [canonicalLabels] using
...
end SeymourEight.BSevenKTwo.RSeven.XTwoNoRoot.Labels
