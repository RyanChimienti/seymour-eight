import SeymourEight.Cases.BSevenKTwo.RSeven.XTwoNoRoot.Assembly
...
/-!
# The no-root `r = 7`, `x = 2` family

The graph hypotheses are transported to a common 225-bit encoding.  The
three possible sizes of `Z` end in separate, isolated `bv_decide`
certificates, while a single fixed seven-vertex reduction supplies the
degree-seven expansion constraint used by all three certificates.
-/

namespace SeymourEight.BSevenKTwo.RSeven.XTwoNoRoot
...
private theorem impossibleAt (zCount : Nat)
  ...
  have hCommon := Assembly.commonCore_true G C L hG hMin hNoSeymour hPivot
    hPB hk hXCard hBound hNoRoot hHCard hzLe hPOrder hZOrder hAOneOrder
      hXOrder hROrder
  ...
  have hmBound : 7 * zCount - edgeCount G C.P C.Z ≤ 7 * zCount - 26 := by
    omega
  ...
  · have hEffective := Assembly.pEffectiveCondition_true G C L hG hPB hMin
    ...
    rw [show Core.smallCore 5 bits = true by simpa [bits] using hCore] at hUnsat
    exact Bool.noConfusion hUnsat
  · have hEffective := Assembly.pEffectiveCondition_true G C L hG hPB hMin
      hNoSeymour hNoRoot hHCard (Or.inr (Or.inr rfl)) hmBound
    have hCore := Assembly.smallCore_true_of_components G C L hG hHCard
      (by omega) hCommon hEffective hHP hCapacity
    have hUnsat := Core.six_impossible bits
    rw [show Core.smallCore 6 bits = true by simpa [bits] using hCore] at hUnsat
    exact Bool.noConfusion hUnsat
