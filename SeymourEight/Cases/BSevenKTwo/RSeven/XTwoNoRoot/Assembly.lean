import SeymourEight.Cases.BSevenKTwo.RSeven.XTwoNoRoot.GraphFacts
...
namespace SeymourEight.BSevenKTwo.RSeven.XTwoNoRoot.Assembly
...
theorem pMinimumDegree_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hPB : C.P = C.B)
    (hRoot : edgeCount G C.P {C.s} = 0) (hHCard : C.H.card = 4)
...
theorem aNonSeymour_all_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hRoot : edgeCount G C.P {C.s} = 0) (hzLe : zCount ≤ 6) :
...
theorem row_missing_le_total_missing {zCount : Nat}
    (C : G.LocalConfiguration) (hPCard : C.P.card = 7)
    (hZCard : C.Z.card = zCount) (p : V) (hpP : p ∈ C.P) :
    zCount - (FiveZUnionEightCapacity.directZNeighbors G C p).card ≤
      7 * zCount - edgeCount G C.P C.Z := by
  have hOther : ∑ q ∈ C.P.erase p, directCount G C.Z q ≤ 6 * zCount := by
    calc
      ∑ q ∈ C.P.erase p, directCount G C.Z q ≤
          ∑ _q ∈ C.P.erase p, zCount := by
        apply Finset.sum_le_sum
        intro q hq
  ...
  have hBound : edgeCount G C.P C.Z ≤
      6 * zCount + directCount G C.Z p := by
    unfold edgeCount
    omega
  have hTotal : edgeCount G C.P C.Z ≤ 7 * zCount := by
    exact (edgeCount_le_card_mul_card G C.P C.Z).trans_eq
      (by rw [hPCard, hZCard])
  rw [FiveZUnionEightCapacity.card_directZNeighbors G C p]
  omega

theorem directZ_to_P_capacity {zCount : Nat}
    (C : G.LocalConfiguration) (hG : G.IsOriented)
    (hPCard : C.P.card = 7) (hZCard : C.Z.card = zCount)
    (p : V) (hpP : p ∈ C.P) :
    edgeCount G (FiveZUnionEightCapacity.directZNeighbors G C p) C.P ≤
      (7 * zCount - edgeCount G C.P C.Z) -
        (zCount - (FiveZUnionEightCapacity.directZNeighbors G C p).card) := by
  let S := FiveZUnionEightCapacity.directZNeighbors G C p
  let T := C.Z \ S
  have hS : S ⊆ C.Z := FiveZUnionEightCapacity.directZNeighbors_subset_Z G C p
  ...
  have hSCard : S.card + T.card = zCount := by
    rw [hTCard]
    have hSLe : S.card ≤ zCount := (Finset.card_le_card hS).trans_eq hZCard
    omega
  have hPZUpper := edgeCount_le_card_mul_card G C.P C.Z
  rw [hPCard, hZCard] at hPZUpper
  change edgeCount G S C.P ≤
    (7 * zCount - edgeCount G C.P C.Z) - (zCount - S.card)
  omega
...
theorem individualEffectiveLower_graph {zCount : Nat}
    ...
    (hmBound : 7 * zCount - edgeCount G C.P C.Z ≤ 7 * zCount - 26)
    (p : Nat) (hp : p < 7) :
  ...
  have hRow : zCount - s ≤ m := by
    exact row_missing_le_total_missing G C hPCard hZCard v hvP
  have hToP : edgeCount G S C.P ≤ m - (zCount - s) := by
    exact directZ_to_P_capacity G C hG hPCard hZCard v hvP
  ...
  · change (individualEffectiveLowerFour bits p).toNat ≤ U.card
    ...
    interval_cases m <;> interval_cases s <;>
      simp only [Nat.choose, add_zero, nonpos_iff_eq_zero, tsub_zero, zero_tsub,
        ...
        Nat.one_le_ofNat, Nat.sub_eq_zero_of_le, Nat.reduceLeDiff]
  ...
  · change (individualEffectiveLowerFive bits p).toNat ≤ U.card
    ...
    interval_cases m <;> interval_cases s <;>
      simp only [Nat.choose, add_zero, nonpos_iff_eq_zero, tsub_zero, zero_tsub,
        ...
        at hInternal hToP hLower ⊢ <;>
      omega
  · change (individualEffectiveLowerSix bits p).toNat ≤ U.card
...
theorem H_to_P_add_externalMissing_le_capacity {zCount : Nat}
    ...
    (hZCard : C.Z.card = zCount) (hHCard : C.H.card = 4)
    (hzLe : zCount ≤ 6) :
  ...
  have hPHPlusPZ : 35 ≤
      edgeCount G C.P C.H + edgeCount G C.P C.Z := by omega
  have hHPPlusSeven : edgeCount G C.H C.P + 7 ≤
      edgeCount G C.P C.Z := by omega
  omega
...
theorem aOnePairDeletionExpands_true {zCount : Nat}
  ...
  have hCount : E.card ≤ (count (15 + zCount) goodTarget).toNat := by
    have hFilter := filterCard_le_count (V := V) (retainedVertexSet G C)
      (retainedLabelEquiv G C L) goodTarget (fun v ↦ v ∈ E) (by omega) (by
        intro target htE
...
theorem commonCore_true {zCount : Nat}
    ...
    (hBound : Digraph.LimitedSeymourConjectureOn V 7) (hNoRoot : epsilonS G C = 0)
...
theorem smallCore_true_of_components {zCount : Nat}
    ...
    (hCapacity : edgeCount G C.H C.P +
      (7 * zCount - edgeCount G C.P C.Z) ≤ 7 * zCount - 7) :
...
end SeymourEight.BSevenKTwo.RSeven.XTwoNoRoot.Assembly
