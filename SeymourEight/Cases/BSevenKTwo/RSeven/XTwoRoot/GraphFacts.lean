import SeymourEight.Cases.BSevenKTwo.RSeven.XTwoNoRoot.Encoding
import SeymourEight.Cases.BSevenKTwo.RSeven.XTwoRoot.Labels
...
namespace SeymourEight.BSevenKTwo.RSeven.XTwoRoot.GraphFacts
...
noncomputable def retainedLabelEquiv {zCount : Nat}
...
@[simp] theorem retainedLabelEquiv_val {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C) (hG : G.IsOriented)
    (i : Fin (15 + zCount)) :
...
@[simp] theorem hLabelEquiv_val {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hHCard : C.H.card = 4) (i : Fin 4) :
    (hLabelEquiv G C L hHCard i).1 = (L.a ⟨i + 1, by omega⟩).1 := by rfl
...
private abbrev graphBits {zCount : Nat} (L : Labels G zCount C) : XTwoNoRoot.Core.Encoding :=
    ...
    · rw [if_neg htA]
      ...
      · rw [if_pos htP]
        ...
        · rw [if_neg hs0, if_neg hsH,
            rToP_coreBits G.Adj _ _ _ (source - 5) (target - 8)
              (by omega) hti]
          have haeq : (L.a ⟨source - 5  + 5, by omega⟩).1 =
              (L.a ⟨source, hsA⟩).1 := by
  ...
  · have hsP : source < 15 := hs
    ...
    · rw [if_pos htA]
      ...
      · rw [if_neg (by simpa [Bool.and_eq_true] using htH)]
        have htCase : target = 0 ∨ target = 5 ∨ target = 6 ∨ target = 7 := by omega
        rcases htCase with rfl | rfl | rfl | rfl
        · simp [labelledVertex, hsA, hsP, hP0]
        · simpa [labelledVertex, hsA, hsP] using
            hPR ⟨source - 8, by omega⟩ (0 : Fin 3)
        · simpa [labelledVertex, hsA, hsP] using
            hPR ⟨source - 8, by omega⟩ (1 : Fin 3)
        · simpa [labelledVertex, hsA, hsP] using
            hPR ⟨source - 8, by omega⟩ (2 : Fin 3)
    · rw [if_neg htA]
      by_cases htP : target < 15
...
theorem directCount_graphBits_toNat {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hPB : C.P = C.B)
    (hzLe : zCount ≤ 6)
    (source : Nat) (hs : source < 15) :
    (XTwoNoRoot.Core.directCount zCount (graphBits G L) source).toNat =
      G.outdegree (labelledVertex G L source) := by
  let bits := graphBits G L
  rw [XTwoNoRoot.Core.directCount, toNat_count_eq_fin_sum (15 + zCount) _ (by omega)]
  have hCount : (∑ j : Fin (15 + zCount), if coreArc zCount bits source j then 1 else 0) =
    ...
    rw [retainedLabelEquiv_val G C L hG,
      coreArc_graphBits G C L hG source j hs j.isLt hzLe]
    simp
  rw [hCount]
...
theorem strictSecondLocal_true_mem {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hzLe : zCount ≤ 6)
    (source target : Nat) (hs : source < 15) (ht : target < 15 + zCount)
  ...
  rw [coreArc_graphBits G C L hG source middle hs (by omega) hzLe] at hFirst
  rw [coreArc_graphBits G C L hG middle target hm ht hzLe] at hLast
  rw [coreArc_graphBits G C L hG source target hs ht hzLe] at hNotArc
...
theorem localSecondCount_le_graph {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hzLe : zCount ≤ 6)
    (source : Nat) (hs : source < 15) :
  ...
  have hFiltered := count_le_filterCard (V := V) (retainedVertexSet G C)
    (retainedLabelEquiv G C L hG) (strictSecondLocal zCount bits source)
...
theorem nonSeymour_graphBits_true {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hPB : C.P = C.B)
    (hNoSeymour : ¬G.HasSeymourVertex)
    (hzLe : zCount ≤ 6) (source : Nat) (hs : source < 15) :
    (localSecondCount zCount (graphBits G L) source).ult
      (XTwoNoRoot.Core.directCount zCount (graphBits G L) source) = true := by
  simp only [BitVec.ult_eq_decide, decide_eq_true_eq]
  rw [directCount_graphBits_toNat G C L hG hPB hzLe source hs]
...
theorem aPOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
  ...
  by_cases hsH : source < 5
  · rw [if_neg hs0, if_pos hsH,
      hToP_coreBits G.Adj _ _ _ (source - 1) j (by omega) j.isLt]
    have hfin : (⟨source - 1 + 1, by omega⟩ : Fin 8) =
        ⟨source, hs⟩ := Fin.ext (by simp; omega)
    simp [hfin]
  · rw [if_neg hs0, if_neg hsH,
      rToP_coreBits G.Adj _ _ _ (source - 5) j (by omega) j.isLt]
    have hfin : (⟨source - 5 + 5, by omega⟩ : Fin 8) =
        ⟨source, hs⟩ := Fin.ext (by simp; omega)
    simp [hfin]
...
end SeymourEight.BSevenKTwo.RSeven.XTwoRoot.GraphFacts
