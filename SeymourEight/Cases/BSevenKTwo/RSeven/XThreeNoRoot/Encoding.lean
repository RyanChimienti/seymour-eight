...
namespace SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.Encoding
...
def coreBitAt {zCount : Nat} (p : Fin 7 → V) (a : Fin 8 → V)
  ...
  if hnA : n < 64 then
    ...
    let q := n - 106
    decide (R (p ⟨q / 5, by omega⟩)
      (a ⟨q % 5 + 1, by have := Nat.mod_lt q (by omega : 0 < 5); omega⟩))
...
@[simp] theorem pToH_coreBits {zCount : Nat} (p : Fin 7 → V)
  ...
  simp [coreBitAt, show ¬106 + 5 * i + h < 64 by omega,
    ...
    show 106 + 5 * i + h - 106 = 5 * i + h by omega, hd, hm]
...
@[simp] theorem hToP_coreBits {zCount : Nat} (p : Fin 7 → V)
  ...
  simp [coreBitAt, show ¬141 + 7 * h + i < 64 by omega,
    ...
    show 141 + 7 * h + i - 141 = 7 * h + i by omega, hd, hm]

@[simp] theorem pToZ_coreBits {zCount : Nat} (p : Fin 7 → V)
    (a : Fin 8 → V) (z : Fin zCount → V)
    (i j : Nat) (hi : i < 7) (hj5 : j < 5) (hj : j < zCount) :
  ...
  simp [coreBitAt, show ¬176 + 5 * i + j < 64 by omega,
    show ¬176 + 5 * i + j < 106 by omega,
    show ¬176 + 5 * i + j < 141 by omega,
    show ¬176 + 5 * i + j < 176 by omega,
    show 176 + 5 * i + j < 211 by omega,
    show 176 + 5 * i + j - 176 = 5 * i + j by omega, hd, hm, hj]
...
@[simp] theorem pToZ_coreBits_inactive {zCount : Nat} (p : Fin 7 → V)
  ...
  simp [coreBitAt, show ¬176 + 5 * i + j < 64 by omega,
    ...
    show ¬176 + 5 * i + j < 176 by omega,
    show 176 + 5 * i + j < 211 by omega,
    show 176 + 5 * i + j - 176 = 5 * i + j by omega, hm, hj]

@[simp] theorem rToP_coreBits {zCount : Nat} (p : Fin 7 → V)
    (a : Fin 8 → V) (z : Fin zCount → V)
    (r i : Nat) (hr : r < 2) (hi : i < 7) :
    Core.rToP (coreBits R p a z) r i =
      decide (R (a ⟨r + 6, by omega⟩) (p ⟨i, hi⟩)) := by
  ...
  simp [coreBitAt, show ¬211 + 7 * r + i < 64 by omega,
    ...
    show 211 + 7 * r + i - 211 = 7 * r + i by omega, hd, hm]

end SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.Encoding
