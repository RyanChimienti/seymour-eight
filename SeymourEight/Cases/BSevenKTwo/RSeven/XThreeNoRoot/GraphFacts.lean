...
namespace SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.GraphFacts
...
@[simp] theorem retainedLabelEquiv_val {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C) (i : Fin (15 + zCount)) :
...
private abbrev graphBits {zCount : Nat} (L : Labels G zCount C) : Core.Encoding :=
...
theorem localSecondCount_le_graph {zCount : Nat}
  ...
  have hFiltered := count_le_filterCard (V := V) (retainedVertexSet G C)
    (retainedLabelEquiv G C L) (strictSecondLocal zCount bits source)
...
theorem nonSeymour_graphBits_true {zCount : Nat}
  ...
  rw [directCount_graphBits_toNat G C L hG hPB hRoot hzLe source hs]
...
end SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.GraphFacts
