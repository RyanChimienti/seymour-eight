...
import SeymourEight.Certificates.BSevenKTwo.RSeven.XThree.SharpKing
...
namespace SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.Assembly
  ...
  have hTail : all 5 (fun i ↦ !aArc bits 0 (3 + i)) = true := by
    ...
    · have hr := L.a_r ⟨i - 3, by omega⟩
      have hn : ¬G.Adj C.a1 (L.a ⟨3 + i, by omega⟩).1 := by
        ...
        have hA1 : (L.a ⟨3 + i, by omega⟩).1 ∈ C.A1 :=
          Finset.mem_filter.mpr ⟨(L.a _).2, ha⟩
        have heq : (⟨i - 3 + 6, by omega⟩ : Fin 8) = ⟨3 + i, by omega⟩ :=
          Fin.ext (by simp; omega)
        rw [heq] at hr
        exact (Finset.mem_sdiff.mp hr).2
          (Finset.mem_union_left {C.a1} (Finset.mem_union_left C.X hA1))
      simp [hn]
  have hA1R : all 4 (fun q ↦
      let a := q / 2
      let r := q % 2
      !aArc bits (1 + a) (6 + r)) = true := by
    rw [all_eq_true_iff]
    intro q hq
    dsimp
    rw [aArc_coreBits G.Adj _ _ _ (1 + q / 2) (6 + q % 2)
      (by omega) (by omega)]
    have hn := RSeven.XFourNoRoot.RepeatedSharedOmissionBridge.A1_not_adj_R
      G C (L.a ⟨1 + q / 2, by omega⟩).1 (L.a ⟨6 + q % 2, by omega⟩).1
      (by
        have heq : (⟨(q / 2 : Nat) + 1, by omega⟩ : Fin 8) =
            ⟨1 + q / 2, by omega⟩ := Fin.ext (by simp; omega)
        rw [← heq]
        exact L.a_aOne ⟨q / 2, by omega⟩)
      (by
        have heq : (⟨(q % 2 : Nat) + 6, by omega⟩ : Fin 8) =
            ⟨6 + q % 2, by omega⟩ := Fin.ext (by simp; omega)
        rw [← heq]
        exact L.a_r ⟨q % 2, by omega⟩)
...
theorem three_le_aOneToXCount {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented)
    (hPivot : IsMinimalPivot G C) (hk : C.k = 2) (hXCard : C.X.card = 3) :
    3 ≤ (count 6 fun q ↦
      let a := q / 3
      let x := q % 3
  ...
  have hCount : (count 6 fun q ↦
      aArc (graphBits G L) (1 + q / 3) (3 + q % 3)).toNat =
...
theorem pMinimumDegree_true {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hPB : C.P = C.B)
    (hRoot : edgeCount G C.P {C.s} = 0) (hHCard : C.H.card = 5)
    (hMin : ∀ v, 8 ≤ G.outdegree v) (hzLe : zCount ≤ 5) :
  ...
  have hSmall' : (pOut bits p).toNat + (pHOut bits p).toNat +
    ...
    have hcP : C.P.card = 7 := by simpa using (Fintype.card_congr L.p).symm
    have hcZ : C.Z.card = zCount := by simpa using (Fintype.card_congr L.z).symm
    omega
...
theorem aNonSeymour_all_true {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (hRoot : edgeCount G C.P {C.s} = 0) (hzLe : zCount ≤ 5) :
    all 8 (aNonSeymour zCount (graphBits G L)) = true := by
  rw [all_eq_true_iff]
  intro a ha
  exact nonSeymour_graphBits_true G C L hG hPB hNoSeymour hRoot hzLe a
    (by omega)
...
theorem pNonSeymour_all_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  rw [all_eq_true_iff]
  intro p hp
  exact nonSeymour_graphBits_true G C L hG hPB hNoSeymour hRoot hzLe
    (8 + p) (by omega)

theorem edgeCount_P_root_zero (C : G.LocalConfiguration)
    (hNoRoot : epsilonS G C = 0) : edgeCount G C.P {C.s} = 0 :=
  RSeven.XFourNoRoot.BroadFourBridge.edgeCount_P_root_zero G C hNoRoot

theorem totalPToZ_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hHCard : C.H.card = 5)
    (hzLe : zCount ≤ 5) :
    (totalPToZ zCount (graphBits G L)).toNat = edgeCount G C.P C.Z := by
  let bits := graphBits G L
  ...
  have hz : C.Z.card = zCount := by simpa using (Fintype.card_congr L.z).symm
  rw [hp, hz] at hCap
...
theorem externalMissing_toNat {zCount : Nat} (C : G.LocalConfiguration)
    ...
    (externalMissing zCount (graphBits G L)).toNat =
      7 * zCount - edgeCount G C.P C.Z := by
  ...
  change ((256 - edgeCount G C.P C.Z + 7 * zCount) % 256) = _
  omega
...
theorem totalHToP_toNat {zCount : Nat} (C : G.LocalConfiguration)
  ...
  have hEach : ∀ i : Fin 5, (hPOut bits i).toNat =
      Shared.directCount G C.P (L.a ⟨i + 1, by omega⟩).1 := by
    intro i
    exact hPOut_toNat G C L i i.isLt
  have hSum : (∑ i ∈ Finset.range 5, (hPOut bits i).toNat) =
...
theorem pRowKey_toNat {zCount : Nat} (C : G.LocalConfiguration)
    (L : Labels G zCount C) (hG : G.IsOriented) (hPB : C.P = C.B)
    (hRoot : edgeCount G C.P {C.s} = 0) (hHCard : C.H.card = 5)
    (hzLe : zCount ≤ 5) (p : Nat) (hp : p < 7) :
    (pRowKey zCount (graphBits G L) p).toNat =
      Labels.pKey G C (L.p ⟨p, hp⟩).1 := by
  let bits := graphBits G L
  have hBlocks := pBlockCounts G C L hG hHCard hzLe p hp
  have hDirect := directCount_graphBits_toNat G C L hG hPB hRoot hzLe
    (8 + p) (by omega)
  have hD : (Core.directCount zCount bits (8 + p)).toNat =
  ...
  norm_num [BitVec.toNat_ofNat]
  change ((Core.directCount zCount bits (8 + p)).toNat * 4096 +
  ...
  have hZLe : (pZOut zCount bits p).toNat ≤ zCount := by
    rw [hBlocks.2.2]
    have hc : C.Z.card = zCount := by simpa using (Fintype.card_congr L.z).symm
    exact (Finset.card_le_card (Finset.filter_subset _ _)).trans_eq hc
  have hDLe : G.outdegree (L.p ⟨p, hp⟩).1 ≤ 20 := by
    rw [← hD, Core.directCount, toNat_count_eq_fin_sum _ _ (by omega)]
    calc
...
theorem orderedP_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  intro p hp
  simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
  rw [pRowKey_toNat G C L hG hPB hRoot hHCard hzLe (p + 1) (by omega),
    pRowKey_toNat G C L hG hPB hRoot hHCard hzLe p (by omega)]
  exact hOrder ⟨p, hp⟩
...
theorem orderedStructuralClasses_true {zCount : Nat}
  ...
  · rw [all_eq_true_iff]
    ...
    exact hX ⟨x, hx⟩
  · rw [hAP 7 (by omega), hAP 6 (by omega)]
    exact hR 0
...
theorem directZEffective_direct_subset_H (C : G.LocalConfiguration)
    ...
    directZEffectiveUnion G C p ∩ G.outNeighborFinset p ⊆
      C.H.filter (G.Adj p) :=
  RSeven.XFourNoRoot.BroadFourBridge.directZEffective_direct_subset_H
...
theorem PSecond_add_directZEffective_card_le_second_add_H
  ...
  RSeven.XFourNoRoot.BroadFourBridge.PSecond_add_directZEffective_card_le_second_add_H
    G C hG hPB hNoRoot p hpP

theorem pSecondPCount_le_graphPSecond {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hzLe : zCount ≤ 5) (p : Nat) (hp : p < 7) :
...
-- Exhaustively checks the small ranges for the missing-edge and row-size parameters.
theorem individualEffectiveLower_graph {zCount : Nat}
    (C : G.LocalConfiguration) (L : Labels G zCount C)
    (hG : G.IsOriented) (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hHCard : C.H.card = 5)
    (hZCases : zCount = 3 ∨ zCount = 4 ∨ zCount = 5)
    (hmBound : 7 * zCount - edgeCount G C.P C.Z ≤ 7 * zCount - 21)
    (p : Nat) (hp : p < 7) :
  ...
  let m := 7 * zCount - edgeCount G C.P C.Z
  let s := S.card
  ...
  have hRow : zCount - s ≤ m := by
    rcases hZCases with rfl | rfl | rfl
    · exact RSeven.XFourNoRoot.ZThreeBridge.row_missing_le_total_missing_three
        G C hPCard hZCard v hvP
    · exact row_missing_le_total_missing_four G C hPCard hZCard v hvP
    · exact FiveZUnionEightCapacity.directZ_row_missing_le_total
        G C hPCard hZCard v hvP
  have hToP : edgeCount G S C.P ≤ m - (zCount - s) := by
    rcases hZCases with rfl | rfl | rfl
    · exact RSeven.XFourNoRoot.ZThreeBridge.directZ_to_P_capacity_three
        G C hG hPCard hZCard v hvP
    · exact directZ_to_P_capacity_four G C hG hPCard hZCard v hvP
    · exact FiveZUnionEightCapacity.directZ_to_P_capacity
        G C hG hPCard hZCard v hvP
  ...
  have hSBV : pZOut zCount bits p = BitVec.ofNat 8 s := by
    ...
    rcases hZCases with rfl | rfl | rfl <;> omega
  have hm : m ≤ 7 * zCount - 21 := by simpa [m] using hmBound
  ...
  rcases hZCases with rfl | rfl | rfl
  all_goals
    simp only [individualEffectiveLower, individualEffectiveLowerThree,
    ...
    interval_cases m <;> interval_cases s <;>
      simp only [Nat.choose, add_zero, nonpos_iff_eq_zero, tsub_zero, zero_tsub,
        ...
        Nat.one_mod, Nat.reduceEqDiff, Nat.succ_ne_self]
        at hInternal hToP hLower ⊢ <;>
...
theorem pEffectiveConditionFive_true
    (C : G.LocalConfiguration) (L : Labels G 5 C)
    (hG : G.IsOriented) (hPB : C.P = C.B)
    (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hNoSeymour : ¬G.HasSeymourVertex)
    (hNoRoot : epsilonS G C = 0) (hHCard : C.H.card = 5)
    (hmBound : 35 - edgeCount G C.P C.Z ≤ 14) :
    all 7 (pEffectiveConditionFive (graphBits G L)) = true := by
  have h := pEffectiveCondition_true G C L hG hPB hMin hNoSeymour
    hNoRoot hHCard (Or.inr (Or.inr rfl)) hmBound
  rw [all_eq_true_iff] at h ⊢
  intro p hp
  have hpEff := h p hp
  have hTable : individualEffectiveLower 5 (graphBits G L) p =
      individualEffectiveLowerFive (graphBits G L) p := by
    simp [individualEffectiveLower]
  simpa only [pEffectiveCondition, pEffectiveConditionFive, hTable] using hpEff

theorem twentyOne_le_H_to_P (C : G.LocalConfiguration)
    (hG : G.IsOriented) (hMin : ∀ v, 8 ≤ G.outdegree v)
    (hRootDegree : G.outdegree C.s = 8) (hk : C.k = 2)
    (hx : C.x = 3) (hy : BSevenKTwo.y G C = 0) (hPB : C.P = C.B) :
    21 ≤ edgeCount G C.H C.P := by
  ...
  have hQ : C.Q.card = 0 := by simp [Digraph.LocalConfiguration.Q, hPB]
  rw [hx] at hHCard hR
...
theorem H_to_P_add_externalMissing_le_capacity {zCount : Nat}
    ...
    (hZCard : C.Z.card = zCount) (hHCard : C.H.card = 5)
    (hzLe : zCount ≤ 5) :
    edgeCount G C.H C.P + (7 * zCount - edgeCount G C.P C.Z) ≤
      7 * zCount := by
  ...
  have hPHLower : 35 - 7 * zCount +
      (7 * zCount - edgeCount G C.P C.Z) ≤ edgeCount G C.P C.H := by
    omega
...
theorem commonCore_true {zCount : Nat}
    ...
    (hPB : C.P = C.B) (hk : C.k = 2) (hXCard : C.X.card = 3)
    (hNoRoot : epsilonS G C = 0)
    (hHCard : C.H.card = 5) (hzLe : zCount ≤ 5)
    (hPOrder : ∀ q : Fin 6,
      Labels.pKey G C (L.p ⟨q.val + 1, by omega⟩).1 ≤
        Labels.pKey G C (L.p ⟨q.val, by omega⟩).1)
  ...
  have hThreeBool : (3 : BitVec 8).ule (count 6 fun q ↦
      aArc (graphBits G L) (1 + q / 3) (3 + q % 3)) = true := by
    simp only [BitVec.ule_eq_decide, decide_eq_true_eq]
    exact hThree
  have hAMin := aMinimumAndDegree_true G C L hG hPB hPivot hMin hk
  have hANon := aNonSeymour_all_true G C L hG hPB hNoSeymour hRoot hzLe
  have hPMin := pMinimumDegree_true G C L hG hPB hRoot hHCard hMin hzLe
  have hPNon := pNonSeymour_all_true G C L hG hPB hNoSeymour hRoot hzLe
  have hSharp := Core.sharpKing_of_orientedP zCount (graphBits G L) hOrP
  have hOP := orderedP_true G C L hG hPB hRoot hHCard hzLe hPOrder
...
theorem smallCore_true_of_components {zCount : Nat}
  ...
  have hzNat : (zCount : BitVec 8).toNat = zCount := by
    change zCount % 256 = zCount
    exact Nat.mod_eq_of_lt (by omega)
  have hRightNat : ((7 * zCount : BitVec 8).toNat) = 7 * zCount := by
    rw [BitVec.toNat_mul, hzNat]
    norm_num [show (7 : BitVec 8).toNat = 7 by decide,
      Nat.mod_eq_of_lt hzSmall]
  have hLower : (21 : BitVec 8).ule
  ...
  have hUpper : (totalHToP (graphBits G L) +
      externalMissing zCount (graphBits G L)).ule (7 * zCount) = true := by
    simp only [BitVec.ule_eq_decide, decide_eq_true_eq, BitVec.toNat_add,
      hHPNat, hMissingNat, hRightNat]
    rw [Nat.mod_eq_of_lt (hCapacity.trans_lt hzSmall)]
    exact hCapacity
  simp only [smallCore, hCommon, hEffective, hLower, hUpper, Bool.and_self]

theorem fiveCore_true_of_components
    (C : G.LocalConfiguration) (L : Labels G 5 C)
    (hG : G.IsOriented) (hHCard : C.H.card = 5)
    (hCommon : commonCore 5 (graphBits G L) = true)
    (hEffective : all 7 (pEffectiveConditionFive (graphBits G L)) = true)
    (hHP : 21 ≤ edgeCount G C.H C.P)
    (hCapacity : edgeCount G C.H C.P +
      (35 - edgeCount G C.P C.Z) ≤ 35) :
    fiveCore (graphBits G L) = true := by
  have hHPNat := totalHToP_toNat G C L hHCard
  have hMissingNat := externalMissing_toNat G C L hG hHCard (by omega)
  have hLower : (21 : BitVec 8).ule
      (totalHToP (graphBits G L)) = true := by
    simp only [BitVec.ule_eq_decide, decide_eq_true_eq, hHPNat]
    exact hHP
  have hUpper : (totalHToP (graphBits G L) +
      externalMissing 5 (graphBits G L)).ule 35 = true := by
    simp only [BitVec.ule_eq_decide, decide_eq_true_eq, BitVec.toNat_add,
      hHPNat, hMissingNat, Nat.reducePow]
    rw [Nat.mod_eq_of_lt (hCapacity.trans_lt (by omega))]
    exact hCapacity
  simp only [fiveCore, hCommon, hEffective, hLower, hUpper, Bool.and_self]

end SeymourEight.BSevenKTwo.RSeven.XThreeNoRoot.Assembly
