...
namespace SeymourEight.BSevenKTwo.RSeven.XThreeRoot.Assembly

open Shared Shared.FiniteCore Labels GraphFacts
open XThreeNoRoot.Encoding XThreeNoRoot.Core
...
theorem pMinimumDegree_true {zCount : Nat} (C : G.LocalConfiguration)
  ...
  have hSmall' : (pOut bits p).toNat + (pHOut bits p).toNat +
    ...
    have hH : Shared.directCount G C.H (L.p ⟨p, hp⟩).1 ≤ C.H.card :=
      Finset.card_le_card (Finset.filter_subset _ _)
    have hZ : Shared.directCount G (externalTargets G C) (L.p ⟨p, hp⟩).1 ≤
...
theorem pRowKey_toNat {zCount : Nat} (C : G.LocalConfiguration)
  ...
  have hDirect := directCount_graphBits_toNat G C L hG hPB hzLe
    (8 + p) (by omega)
  have hD : (XThreeNoRoot.Core.directCount zCount bits (8 + p)).toNat =
  ...
  norm_num [BitVec.toNat_ofNat]
  change ((XThreeNoRoot.Core.directCount zCount bits (8 + p)).toNat * 4096 +
  ...
  have hDLe : G.outdegree (L.p ⟨p, hp⟩).1 ≤ 20 := by
    rw [← hD, XThreeNoRoot.Core.directCount, toNat_count_eq_fin_sum _ _ (by omega)]
    calc
...
theorem individualEffectiveLower_graph {zCount : Nat}
    ...
    (hmBound : 7 * zCount - edgeCount G C.P (externalTargets G C) ≤ 7 * zCount - 21)
...
theorem pEffectiveCondition_true {zCount : Nat}
    ...
    (hNoSeymour : ¬G.HasSeymourVertex)
    (hRootDegree : G.outdegree C.s = 8) (hHCard : C.H.card = 5)
    (hZCases : zCount = 3 ∨ zCount = 4 ∨ zCount = 5)
    (hmBound : 7 * zCount - edgeCount G C.P (externalTargets G C) ≤ 7 * zCount - 21) :
...
theorem pEffectiveConditionFive_true
  ...
  have h := pEffectiveCondition_true G C L hG hPB hMin hNoSeymour
    hRootDegree hHCard (Or.inr (Or.inr rfl)) hmBound
...
theorem H_to_P_add_externalMissing_le_capacity {zCount : Nat}
    ...
    (hPCard : C.P.card = 7)
    (hZCard : (externalTargets G C).card = zCount) (hHCard : C.H.card = 5)
    (hzLe : zCount ≤ 5) :
  ...
  have hPHLower : 35 - 7 * zCount +
      (7 * zCount - edgeCount G C.P (externalTargets G C)) ≤ edgeCount G C.P C.H := by
    omega
...
theorem commonCore_true {zCount : Nat}
  ...
  have hSharp := XThreeNoRoot.Core.sharpKing_of_orientedP zCount (graphBits G L) hOrP
...
theorem smallCore_true_of_components {zCount : Nat}
    ...
    (hCapacity : edgeCount G C.H C.P +
      (7 * zCount - edgeCount G C.P (externalTargets G C)) ≤ 7 * zCount) :
...
theorem fiveCore_true_of_components
    ...
    (hHP : 21 ≤ edgeCount G C.H C.P)
    (hCapacity : edgeCount G C.H C.P +
      (35 - edgeCount G C.P (externalTargets G C)) ≤ 35) :
...
end SeymourEight.BSevenKTwo.RSeven.XThreeRoot.Assembly
