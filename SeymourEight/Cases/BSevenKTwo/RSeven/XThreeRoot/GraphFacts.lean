import SeymourEight.Cases.BSevenKTwo.RSeven.XThreeNoRoot.Encoding
import SeymourEight.Cases.BSevenKTwo.RSeven.XThreeRoot.Labels
...
namespace SeymourEight.BSevenKTwo.RSeven.XThreeRoot.GraphFacts
...
private abbrev graphBits {zCount : Nat} (L : Labels G zCount C) : XThreeNoRoot.Core.Encoding :=
    ...
    · rw [if_neg htA]
      ...
      · rw [if_pos htP]
        ...
        · rw [if_neg hs0, if_neg hsH,
            rToP_coreBits G.Adj _ _ _ (source - 6) (target - 8)
              (by omega) hti]
          have haeq : (L.a ⟨source - 6 + 6, by omega⟩).1 =
              (L.a ⟨source, hsA⟩).1 := by
  ...
  · have hsP : source < 15 := hs
    ...
    · rw [if_pos htA]
      ...
      · rw [if_neg (by simpa [Bool.and_eq_true] using htH)]
        have htCase : target = 0 ∨ target = 6 ∨ target = 7 := by omega
        rcases htCase with rfl | rfl | rfl
        · simp [labelledVertex, hsA, hsP, hP0]
        · simpa [labelledVertex, hsA, hsP] using
            hPR ⟨source - 8, by omega⟩ (0 : Fin 2)
        · simpa [labelledVertex, hsA, hsP] using
            hPR ⟨source - 8, by omega⟩ (1 : Fin 2)
    · rw [if_neg htA]
      by_cases htP : target < 15
...
theorem directCount_graphBits_toNat {zCount : Nat}
  ...
  let bits := graphBits G L
  rw [XThreeNoRoot.Core.directCount, toNat_count_eq_fin_sum (15 + zCount) _ (by omega)]
...
theorem nonSeymour_graphBits_true {zCount : Nat}
    ...
    (hzLe : zCount ≤ 5) (source : Nat) (hs : source < 15) :
...
theorem aPOut_toNat {zCount : Nat} (C : G.LocalConfiguration)
  ...
  · rw [if_neg hs0, if_pos hsH,
    ...
    simp [hfin]
  · rw [if_neg hs0, if_neg hsH,
      rToP_coreBits G.Adj _ _ _ (source - 6) j (by omega) j.isLt]
    have hfin : (⟨source - 6 + 6, by omega⟩ : Fin 8) =
        ⟨source, hs⟩ := Fin.ext (by simp; omega)
    simp [hfin]
...
end SeymourEight.BSevenKTwo.RSeven.XThreeRoot.GraphFacts
