...
import SeymourEight.Cases.BSevenKTwo.RSeven.XFourRoot.ZThreeMthreeLocalBridge
...
namespace SeymourEight.BSevenKTwo.RSeven.XFourRoot.ZThreeMthreeBridge

open CertificateBridge Shared
open SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.RepeatedSharedOmissionBridge
open SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMThreeHCore
...
noncomputable def labels (C : G.LocalConfiguration)
  ...
  let f : Fin 6 → {v : V // v ∈ C.H} := fun i =>
    if hi : i.val ≤ 1 then
      ⟨(ea ⟨i.val, by omega⟩).1,
        Finset.mem_union_left C.X (ea ⟨i.val, by omega⟩).2⟩
    else
  ...
  have hf : Function.Bijective f := by
    ...
    constructor
    · intro i j hij
      by_cases hi : i.val ≤ 1 <;> by_cases hj : j.val ≤ 1
      · have he : (⟨i.val, by omega⟩ : Fin 2) = ⟨j.val, by omega⟩ := by
          apply ea.injective
          apply Subtype.ext
          simpa [f, hi, hj] using congrArg Subtype.val hij
        apply Fin.ext
        simpa using congrArg Fin.val he
      · have hval : (ea ⟨i.val, by omega⟩).1 =
            (ex ⟨j.val - 2, by omega⟩).1 := by
          simpa [f, hi, hj] using congrArg Subtype.val hij
        exact ((Finset.disjoint_left.mp
          (Digraph.LocalConfiguration.disjoint_A1_X (G := G) C))
            (ea ⟨i.val, by omega⟩).2
              (hval ▸ (ex ⟨j.val - 2, by omega⟩).2)).elim
      · have hval : (ex ⟨i.val - 2, by omega⟩).1 =
            (ea ⟨j.val, by omega⟩).1 := by
          simpa [f, hi, hj] using congrArg Subtype.val hij
        exact ((Finset.disjoint_left.mp
          (Digraph.LocalConfiguration.disjoint_A1_X (G := G) C))
            (ea ⟨j.val, by omega⟩).2
              (hval ▸ (ex ⟨i.val - 2, by omega⟩).2)).elim
  ...
  refine ⟨ea, ex, eh, ?_, ?_⟩
  · intro i
    change (f ⟨i.val, by omega⟩).1 = (ea i).1
    simp [f, show i.val ≤ 1 by omega, ea]
  · intro i
    change (f ⟨i.val + 2, by omega⟩).1 = (ex i).1
    simp [f, show ¬i.val + 2 ≤ 1 by omega, ex]
...
private theorem any_eq_true_iff (n : Nat) (f : Nat → Bool) :
    XFourNoRoot.ZThreeMThreeHCore.any n f = true ↔ ∃ i < n, f i = true := by
  induction n with
  | zero => simp [XFourNoRoot.ZThreeMThreeHCore.any]
  | succ n ih =>
      rw [XFourNoRoot.ZThreeMThreeHCore.any, Bool.or_eq_true, ih]
      constructor
      · rintro (⟨i, hi, hf⟩ | hf)
        · exact ⟨i, by omega, hf⟩
        · exact ⟨n, by omega, hf⟩
...
private theorem all_eq_true_iff (n : Nat) (f : Nat → Bool) :
    XFourNoRoot.ZThreeMThreeHCore.all n f = true ↔ ∀ i < n, f i = true := by
  induction n with
  | zero => simp [XFourNoRoot.ZThreeMThreeHCore.all]
  | succ n ih =>
      rw [XFourNoRoot.ZThreeMThreeHCore.all, Bool.and_eq_true, ih]
      constructor
...
private theorem toNat_count (n : Nat) (f : Nat → Bool) (hn : n < 256) :
    (XFourNoRoot.ZThreeMThreeHCore.count n f).toNat =
      ∑ i ∈ Finset.range n, (XFourNoRoot.ZThreeMThreeHCore.bitCount (f i)).toNat := by
  induction n with
  | zero => simp [XFourNoRoot.ZThreeMThreeHCore.count]
  | succ n ih =>
      have hn' : n < 256 := by omega
      have hLe : (∑ i ∈ Finset.range n,
          (XFourNoRoot.ZThreeMThreeHCore.bitCount (f i)).toNat) ≤ n := by
        calc
          _ ≤ ∑ _i ∈ Finset.range n, 1 := by
            ...
            cases f i <;> decide
          _ = n := by simp
      rw [XFourNoRoot.ZThreeMThreeHCore.count, BitVec.toNat_add, ih hn',
        Finset.sum_range_succ]
      cases hf : f n
      · simpa [XFourNoRoot.ZThreeMThreeHCore.bitCount, hf] using Nat.mod_eq_of_lt
          (by omega : (∑ i ∈ Finset.range n,
            (XFourNoRoot.ZThreeMThreeHCore.bitCount (f i)).toNat) < 256)
      · simpa [XFourNoRoot.ZThreeMThreeHCore.bitCount, hf] using Nat.mod_eq_of_lt
          (by omega : (∑ i ∈ Finset.range n,
            (XFourNoRoot.ZThreeMThreeHCore.bitCount (f i)).toNat) + 1 < 256)
...
private theorem toNat_count_eq_fin_sum (n : Nat) (f : Nat → Bool)
  ...
  rw [toNat_count n f hn,
    ← Fin.sum_univ_eq_sum_range
      (fun i ↦ (XFourNoRoot.ZThreeMThreeHCore.bitCount (f i)).toNat) n]
  apply Finset.sum_congr rfl
  intro i hi
  cases f i <;> simp [XFourNoRoot.ZThreeMThreeHCore.bitCount]
...
theorem xReach_graphBits_iff (C : G.LocalConfiguration) (L : HLabels G C)
    ...
    xReach (graphBits G C L) x target = true ↔
      XFourNoRoot.RepeatedSharedOmissionBridge.reachesWithinH G C
        (L.x ⟨x, hx⟩).1 (L.x ⟨target, ht⟩).1 := by
  unfold xReach XFourNoRoot.RepeatedSharedOmissionBridge.reachesWithinH
...
theorem x_type_bound (C : G.LocalConfiguration)
    ...
    let r := (C.X.filter fun y ↦ y ≠ source ∧
      XFourNoRoot.RepeatedSharedOmissionBridge.reachesWithinH G C source y).card
    1 ≤ b ∧ (b = 1 ∨ (b = 2 ∧ r ≤ 3) ∨
      (b = 3 ∧ r ≤ 2) ∨ (4 ≤ b ∧ r ≤ 1)) := by
  classical
  ...
  let xReached := C.X.filter fun y ↦ y ≠ source ∧
    XFourNoRoot.RepeatedSharedOmissionBridge.reachesWithinH G C source y
  let xStrict := xReached.filter fun y ↦ ¬G.Adj source y
  let zReached := (externalTargets G C).filter fun z ↦
  ...
  have hSecondBound : pMiss.card + aMiss.card + xStrict.card + zReached.card ≤
    ...
    have hExternalH : Disjoint (externalTargets G C) C.H := by
      rw [Finset.disjoint_left]
      intro v hvE hvH
    ...
    have hExternalP : Disjoint (externalTargets G C) C.P := by
      rw [Finset.disjoint_left]
      intro v hvE hvP
      rcases Finset.mem_union.mp hvE with hvZ | hvRoot
      · exact (Finset.disjoint_left.mp
    ...
    have hUSubset : U ⊆ G.secondOutNeighborFinset source := by
      ...
      · rcases Finset.mem_filter.mp hvZ with ⟨hvZ, p, hp, hsp, hpv⟩
        rw [Digraph.mem_secondOutNeighborFinset, Digraph.mem_secondOutNeighborSet]
        exact ⟨⟨p, hsp, hpv⟩,
          BroadFourBridge.A_not_adj_external G C hG source v hSourceA hvZ,
          fun heq ↦ (Finset.disjoint_left.mp hExternalH) hvZ
    ...
    have hAllZ : Disjoint (pMiss ∪ aMiss ∪ xStrict) zReached := by
      ...
      · rcases Finset.mem_union.mp hvPA with hvP | hvA
        · exact (Finset.disjoint_left.mp hExternalP) hvZC
            (Finset.mem_filter.mp hvP).1
        · exact (Finset.disjoint_left.mp hExternalH) hvZC
  ...
  have hSplit : edgeCount G C.P (externalTargets G C) =
      edgeCount G S (externalTargets G C) + edgeCount G T (externalTargets G C) := by
    rw [← hUnion,
      XFourNoRoot.RepeatedSharedOmissionBridge.edgeCount_source_union G S T (externalTargets G C) hST]
  have hTCap := edgeCount_le_card_mul_card G T (externalTargets G C)
  rw [hTCard, hZCard] at hTCap
  have hSLower : 3 * b - 3 ≤ edgeCount G S (externalTargets G C) := by omega
  have hSEq : edgeCount G S (externalTargets G C) = edgeCount G S zReached := by
    unfold edgeCount
    apply Finset.sum_congr rfl
    intro p hp
    unfold directCount CertificateBridge.internalFirstNeighbors
  ...
  have hSCap : edgeCount G S (externalTargets G C) ≤ b * zReached.card := by
    rw [hSEq, ← hSCard]
...
theorem zThree_defectThree_impossible
    ...
    (hDefect : 21 - edgeCount G C.P (externalTargets G C) = 3) : False := by
  classical
  have hPB : C.P = C.B :=
  ...
  have hPZLe : edgeCount G C.P (externalTargets G C) ≤ 21 := by
    exact (edgeCount_le_card_mul_card G C.P (externalTargets G C)).trans_eq (by
      rw [hPCard, hZCard])
  have hPZ : edgeCount G C.P (externalTargets G C) = 18 := by omega
  ...
  rw [← hExternal, hPZ] at hAccounting
  ...
  have hHTournament : ∀ u ∈ C.H, ∀ v ∈ C.H, u ≠ v →
    ...
    intro u hu v hv hne
    exact XFourNoRoot.RepeatedSharedOmissionBridge.complete_of_internal_edgeCount_max
  ...
  have hDomR : ∀ u ∈ C.X, ∀ r ∈ C.R, G.Adj u r := by
    ...
    have hSubset : incoming ⊆ C.X := by
      ...
      rcases Finset.mem_union.mp hvH with hvA1 | hvX
      · exact (XFourNoRoot.RepeatedSharedOmissionBridge.A1_not_adj_R G C v r hvA1 hrMem hvr).elim
      · exact hvX
    have hIncomingCard : incoming.card = 4 := by
  ...
  have hKEdges : edgeCount G K K = 78 := by
    rw [XFourNoRoot.RepeatedSharedOmissionBridge.edgeCount_source_union G C.P C.H K hDisjoint,
  ...
  have hCompleteK {u v : V} (hu : u ∈ K) (hv : v ∈ K) (hne : u ≠ v) :=
    XFourNoRoot.RepeatedSharedOmissionBridge.complete_of_internal_edgeCount_max
  ...
  have hCore := ZThreeMthreeLocalBridge.core_true G hBound C L hG hPB
    hNoSeymour hPComplete hPHComplete hHDegreeLocal hPDegreeLocal
    hHTournamentLocal hXDomPivotLocal hXDomRLocal hPZ
  have hUnsat := XFourNoRoot.ZThreeMThreeLocalCore.unsat
    (ZThreeMthreeLocalBridge.graphBits G C L)
  rw [hCore] at hUnsat
  exact Bool.noConfusion hUnsat

end SeymourEight.BSevenKTwo.RSeven.XFourRoot.ZThreeMthreeBridge
