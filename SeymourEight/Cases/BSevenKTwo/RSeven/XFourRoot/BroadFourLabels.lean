...
namespace SeymourEight.BSevenKTwo.RSeven.XFourRoot.BroadFourLabels
...
def pKey (C : G.LocalConfiguration) (v : V) : Nat :=
  4096 * directCount G (externalTargets G C) v + 256 * G.outdegree v +
...
noncomputable def sortedZFinsetEquiv (p : Fin 7 → V) (C : G.LocalConfiguration)
    (eZ : Fin 4 ≃ {v : V // v ∈ (externalTargets G C)}) :
    Fin 4 ≃ {v : V // v ∈ (externalTargets G C)} :=
  (zSortPermutation G p (fun i => (eZ i).1)).trans eZ

theorem sortedZ_degree_anti (p : Fin 7 → V) (C : G.LocalConfiguration)
    (eZ : Fin 4 ≃ {v : V // v ∈ (externalTargets G C)}) {i j : Fin 4} (hij : i ≤ j) :
...
noncomputable def canonicalLabels (C : G.LocalConfiguration)
    ...
    (hZCard : (externalTargets G C).card = 4) : Labels G C := by
  ...
  let eZ : Fin 4 ≃ {v : V // v ∈ (externalTargets G C)} := finsetEquivFin (externalTargets G C) hZCard
...
theorem canonicalLabels_p_order (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 4)
    (hRCard : C.R.card = 1) (hHCard : C.H.card = 6)
    (hZCard : (externalTargets G C).card = 4) (q : Nat) (hq : q < 6) :
...
theorem canonicalLabels_z_order (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 4)
    (hRCard : C.R.card = 1) (hHCard : C.H.card = 6)
    (hZCard : (externalTargets G C).card = 4) (q : Nat) (hq : q < 3) :
    zColumnDegree G
  ...
  exact sortedZ_degree_anti G
    ...
    C (finsetEquivFin (externalTargets G C) hZCard) (Fin.mk_le_mk.mpr (by omega))
...
theorem canonicalLabels_z_key_order (C : G.LocalConfiguration)
  ...
  exact sortedZ_key_anti G
    (fun i => (sortedPFinsetEquiv G C (finsetEquivFin C.P hPCard) i).1)
    C (finsetEquivFin (externalTargets G C) hZCard)
      (Fin.mk_le_mk.mpr (by omega))
...
end SeymourEight.BSevenKTwo.RSeven.XFourRoot.BroadFourLabels
