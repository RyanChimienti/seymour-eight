...
namespace SeymourEight.BSevenKTwo.RSeven.XFourRoot.BroadFourLowBridge

open Shared XFourNoRoot.BroadFourCore XFourNoRoot.BroadFourLowCore BroadFourLabels
...
private theorem pZIncidenceCode_eq (C : G.LocalConfiguration)
  ...
  rw [XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 6 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 5 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 4 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 3 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 2 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 1 z (by omega) hz,
    XFourNoRoot.BroadFourBridge.pToZ_coreBits G.Adj _ _ _ 0 z (by omega) hz]
  simp
...
private theorem pHIn_toNat (C : G.LocalConfiguration) (L : Labels G C)
  ...
  calc
    (∑ i : Fin 7, if pToH (graphBits G C L) i h then 1 else 0) =
      ...
      intro i hi
      rw [XFourNoRoot.BroadFourBridge.pToH_coreBits G.Adj _ _ _ i h i.isLt hh]
      simp
    _ = ∑ x : {v : V // v ∈ C.P},
...
theorem zFour_impossible
  ...
  have hOP : orderedP bits = true := by
    simpa [bits] using orderedP_true G C L hG hPB hHCard hPOrder
  ...
  have hPH : (totalPToH bits).toNat = edgeCount G C.P C.H := by
    simpa [bits] using totalPToH_toNat G C L hG hHCard
  ...
  have hUpperBool :
    ...
    have hLt : edgeCount G C.H C.P +
        (28 - edgeCount G C.P (externalTargets G C)) < 256 := by omega
  ...
  by_cases hLow : 28 - edgeCount G C.P (externalTargets G C) ≤ 2
  · have hMBool : (externalMissing bits).ule 2 = true := by
    ...
    have hCases : selectedLowPattern 0 bits = true ∨
      ...
      simpa [XFourNoRoot.BroadFourCore.any, or_assoc] using hCover
  ...
  · have hHigh : 3 ≤
        28 - edgeCount G C.P (externalTargets G C) := by omega
...
end SeymourEight.BSevenKTwo.RSeven.XFourRoot.BroadFourLowBridge
