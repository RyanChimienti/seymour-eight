...
namespace SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMthreeLocalBridge

open CertificateBridge Shared BroadFourBridge RepeatedSharedOmissionBridge
open ZThreeMThreeLocalCore
...
noncomputable def labels (C : G.LocalConfiguration)
    ...
    (hZCard : C.Z.card = 3) : Labels G C := by
...
private theorem upper_bit (C : G.LocalConfiguration) (L : Labels G C)
    ...
    pUpperBit G L (ZThreeMThreeLocalCore.upperIndex i j) =
      decide (G.Adj (L.p ⟨i, hi⟩).1 (L.p ⟨j, hj⟩).1) := by
  interval_cases i <;> interval_cases j <;>
    simp_all [ZThreeMThreeLocalCore.upperIndex, pUpperBit]
...
noncomputable def retainedLabelEquiv (C : G.LocalConfiguration) (L : Labels G C) :
  ...
  · have hAP : Disjoint C.A C.P := by
    ...
    have hz : C.Z.card = 3 := by simpa using (Fintype.card_congr L.z).symm
    simp [ha, hp, hz]

@[simp] theorem retainedLabelEquiv_val (C : G.LocalConfiguration)
    (L : Labels G C) (i : Fin 18) :
...
theorem coreArc_graphBits (C : G.LocalConfiguration) (L : Labels G C)
  ...
  have hPR : ∀ i : Fin 7, ¬G.Adj (L.p i).1 (L.a 7).1 :=
    fun i ↦ RepeatedSharedOmissionBridge.P_not_adj_R G C _ _ (L.p i).2 L.a_r
  unfold coreArc
  ...
  · rw [if_pos hsA]
    ...
    · rw [if_neg htA]
      ...
      · simp [htP, ht, labelledVertex, hsA, htA,
          RepeatedSharedOmissionBridge.A_not_adj_Z G C hG
...
theorem directCount_graphBits_toNat (C : G.LocalConfiguration) (L : Labels G C)
  ...
  rw [ZThreeMThreeLocalCore.directCount,
  ...
  · have heq : labelledVertex G L source = (L.a ⟨source, hsA⟩).1 := by
    ...
    simpa [retainedVertexSet,
      RepeatedSharedOmissionBridge.retainedVertexSet] using
      RepeatedSharedOmissionBridge.A_outgoingCaptured_retained G C hG hPB
        _ (L.a _).2
  · have heq : labelledVertex G L source = (L.p ⟨source - 8, by omega⟩).1 := by
    ...
    simpa [retainedVertexSet,
      RepeatedSharedOmissionBridge.retainedVertexSet] using
      RepeatedSharedOmissionBridge.P_outgoingCaptured_retained G C hG hPB hRoot
        _ (L.p _).2
...
theorem localSecondCount_le_graph (C : G.LocalConfiguration) (L : Labels G C)
  ...
  have hFiltered := count_le_filterCard (V := V) (retainedVertexSet G C)
    (retainedLabelEquiv G C L) (strictSecondLocal (graphBits G C L) source)
...
theorem nonSeymour_true (C : G.LocalConfiguration) (L : Labels G C)
  ...
  rw [directCount_graphBits_toNat G C L hG hPB hRoot hPComplete hPHComplete
    source hs]
...
theorem xDeletionExpands_true
  ...
  have hESubset : E ⊆ retainedVertexSet G C := by
    ...
    rcases Finset.mem_union.mp
        (RepeatedSharedOmissionBridge.A_outgoingCaptured G C hG source hSourceA hmOut)
      with hmA | hmB
    · simpa [retainedVertexSet,
        RepeatedSharedOmissionBridge.retainedVertexSet] using
        RepeatedSharedOmissionBridge.A_outgoingCaptured_retained G C hG hPB
          middle hmA ((Digraph.mem_outNeighborFinset (G := G)).mpr hmv)
    · have hmP : middle ∈ C.P := by simpa [hPB] using hmB
      simpa [retainedVertexSet,
        RepeatedSharedOmissionBridge.retainedVertexSet] using
        RepeatedSharedOmissionBridge.P_outgoingCaptured_retained G C hG hPB hRoot
          middle hmP ((Digraph.mem_outNeighborFinset (G := G)).mpr hmv)
  have hCount : E.card ≤ (count 18 (fun target ↦
    ...
    have hFilter := filterCard_le_count (V := V) (retainedVertexSet G C)
      (retainedLabelEquiv G C L) (fun target ↦
        decide (target ≠ 3 + x) &&
          !retainedAfterAOneDeletion (graphBits G C L) x target &&
          any 15 (fun middle ↦
            retainedAfterAOneDeletion (graphBits G C L) x middle &&
...
theorem core_true
  ...
  have hFixedA : fixedA bits = true := by
    ...
    have h17 : (!aArc bits 1 7) = true := by
      ...
      have hn := RepeatedSharedOmissionBridge.A1_not_adj_R G C _ _
        (L.a_aone 0) L.a_r
      simpa using hn
  ...
  have hPDegreeBool : pDegreeEight bits = true := by
    ...
    intro p hp
    rw [ZThreeMThreeLocalCore.pDegree_eq_direct bits p hp, beq_iff_eq]
    apply BitVec.eq_of_toNat_eq
    rw [directCount_graphBits_toNat G C L hG hPB hRoot hPComplete hPHComplete
      (8 + p) (by omega)]
  ...
  have hTotalPZ : (totalPToZ bits == 18) = true := by
    ...
    have hEach (i : Fin 7) :
      ...
      symm
      apply directCount_eq_sum_bool G C.Z L.z
      intro j
    ...
    rw [hFlat]
    simp_rw [hEach]
    rw [← edgeCount_eq_sum_fin G C.P C.Z L.p, hPZ]
    decide
  have hANon : aNonSeymour bits = true := by
    rw [aNonSeymour, all_eq_true_iff]
    intro a ha
    exact nonSeymour_true G C L hG hPB hNoSeymour hRoot hPComplete hPHComplete
      a (by omega)
  ...
  have hDeletion : all 4 (xDeletionExpands bits) = true := by
    ...
    exact xDeletionExpands_true G hBound C L hG hPB hNoSeymour hRoot
...
end SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMthreeLocalBridge
