import SeymourEight.Cases.BSevenKTwo.RSeven.XFourNoRoot.BroadFourLowAssembly
import SeymourEight.Cases.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMthreeAssembly

set_option linter.style.header false

/-!
# The no-root `r = 7`, `x = 4` family

The graph-level arithmetic dispatch selects the appropriate finite obstruction
for each parameter branch.
-/
...
private theorem zThree_defect_le_three
  ...
  have hPPLe : edgeCount G C.P C.P ≤ 21 := by
    ...
    exact hInternal
  have hRootZero := BroadFourBridge.edgeCount_P_root_zero G C hNoRoot
...
theorem impossible
    ...
    (hyz : BSevenKTwo.y G C = 0 ∧ (C.z = 3 ∨ C.z = 4)) : False := by
  rcases hyz with ⟨hy, hz | hz⟩
  · have hDefectLe := zThree_defect_le_three G C hG hMin hRootDegree
      hBCard hk hr hx hNoRoot hy hz
    by_cases hLow : 21 - edgeCount G C.P C.Z ≤ 1
    · exact ZThreeLowBridge.zThree_lowDefect_impossible G C hG hMin
        hNoSeymour hRootDegree hPivot hBCard hk hr hx hNoRoot hy hz hLow
    by_cases hTwo : 21 - edgeCount G C.P C.Z = 2
    · exact ZThreeMtwoBridge.zThree_defectTwo_impossible G C hG hMin
        hNoSeymour hRootDegree hPivot hBCard hk hr hx hNoRoot hy hz hTwo
    · have hThree : 21 - edgeCount G C.P C.Z = 3 := by omega
      exact ZThreeMthreeBridge.zThree_defectThree_impossible G hBound C hG hMin
        hNoSeymour hRootDegree hPivot hBCard hk hr hx hNoRoot hy hz hThree
  · exact BroadFourLowBridge.zFour_impossible G C hG hMin
      hNoSeymour hRootDegree hPivot hBCard hk hr hx hNoRoot hy hz
