...
open RepeatedSharedOmissionBridge
...
theorem coreArc_coreBits (C : G.LocalConfiguration) (L : Labels G C)
    ...
    coreArc (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
    ...
    · rw [if_neg htA]
      ...
      · rw [if_pos htP]
        ...
        · have hsh : source - 1 < 6 := by omega
          rw [if_neg hs0, if_pos hs7, hToP_coreBits G.Adj _ _ _
...
theorem directCount_coreBits_toNat (C : G.LocalConfiguration)
    ...
    (BroadFourCore.directCount (coreBits G.Adj (fun i ↦ (L.p i).1)
      (fun i ↦ (L.a i).1) (fun i ↦ (L.z i).1)) source).toNat =
      G.outdegree (labelledVertex G L source) := by
  let bits := coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
    (fun i ↦ (L.z i).1)
  rw [BroadFourCore.directCount, toNat_count_eq_fin_sum 19 _ (by omega)]
  have hCount : (∑ j : Fin 19, if coreArc bits source j then 1 else 0) =
    ...
    intro j
    rw [retainedLabelEquiv_val, coreArc_coreBits G C L hG source j hs j.isLt]
    simp
  rw [hCount]
  ...
  · have heq : labelledVertex G L source =
    ...
    rw [heq]
    exact P_outgoingCaptured_retained G C hG hPB hRoot _ (L.p _).2
...
theorem nonSeymour_coreBits_true (C : G.LocalConfiguration)
  ...
  rw [directCount_coreBits_toNat G C L hG hPB hRoot source hs]
...
theorem aOut_toNat (C : G.LocalConfiguration) (L : Labels G C)
    ...
    (aOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
...
theorem aPOut_toNat (C : G.LocalConfiguration) (L : Labels G C)
    ...
    (aPOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
      (fun i ↦ (L.z i).1)) source).toNat =
  ...
  · have hsh : source - 1 < 6 := by omega
    rw [if_neg hs0, if_pos hs7,
      hToP_coreBits G.Adj _ _ _ (source - 1) j hsh j.isLt]
    have hfin : (⟨source - 1 + 1, by omega⟩ : Fin 8) =
...
theorem pBlockCounts (C : G.LocalConfiguration) (L : Labels G C)
    ...
    (p : Nat) (hp : p < 7) :
    (pOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
      (fun i ↦ (L.z i).1)) p).toNat =
        Shared.directCount G C.P (L.p ⟨p, hp⟩).1 ∧
    (pHOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
      (fun i ↦ (L.z i).1)) p).toNat =
        Shared.directCount G C.H (L.p ⟨p, hp⟩).1 ∧
    (pZOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
      (fun i ↦ (L.z i).1)) p).toNat =
...
theorem hPOut_toNat (C : G.LocalConfiguration) (L : Labels G C)
    ...
    (hPOut (coreBits G.Adj (fun i ↦ (L.p i).1) (fun i ↦ (L.a i).1)
