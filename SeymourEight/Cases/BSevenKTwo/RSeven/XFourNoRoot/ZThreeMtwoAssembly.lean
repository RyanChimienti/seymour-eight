import SeymourEight.Cases.BSevenKTwo.RSeven.XFourNoRoot.ZThreeLowAssembly
...
namespace SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMtwoBridge

open CertificateBridge Shared ZThreeCore ZThreeLabels ZThreeBridge
open ZThreeLowCore ZThreeLowBridge ZThreeNormalization
open FiveZExactGraphBridge BroadFourBridge RepeatedSharedOmissionBridge
open ZThreeSimpleLabels
...
theorem simpleRowsOrdered_true (C : G.LocalConfiguration)
    ...
    (_hZCard : C.Z.card = 3) (start countRows : Nat)
  ...
  rw [simpleRowsOrdered, ZThreeBridge.all_eq_true_iff]
  intro i hi
  let left := start + i
  let right := start + i + 1
  have hKL := hKey ⟨i, hi⟩
  ...
  have hS' : directCount G C.Z
      (L.p ⟨left, by dsimp [left]; omega⟩).1 = directCount G C.Z
      (L.p ⟨right, by dsimp [right]; omega⟩).1 := by
    simpa [left, right] using hS
  have hDL := pDegree_toNat G C L.p L.h L.z hG hPB hNoRoot left (by
    dsimp [left]; omega)
  have hDR := pDegree_toNat G C L.p L.h L.z hG hPB hNoRoot right (by
    dsimp [right]; omega)
  have hBL := pBlockCounts G C.P C.H C.Z L.p L.h L.z hG left (by
    dsimp [left]; omega)
  have hBR := pBlockCounts G C.P C.H C.Z L.p L.h L.z hG right (by
    dsimp [right]; omega)
...
private theorem directZ_eq_two_left (C : G.LocalConfiguration)
    ...
    directCount G C.Z (L.p ⟨p, hp⟩).1 = 2 := by
...
theorem simpleLabels_of_externalOrbit (C : G.LocalConfiguration)
    ...
    (hNoRoot : epsilonS G C = 0)
    (hPCard : C.P.card = 7) (hHCard : C.H.card = 6)
    (hZCard : C.Z.card = 3) (orbit : Nat) (hOrbit : orbit ≤ 2) :
    let L := canonicalLabels G C hPCard hHCard hZCard
  ...
  intro hExternal
  have hH : ZThreeLowCore.orderedH bits = true := by
  ...
  have hHLex : all 5 (fun h ↦
    ...
    simpa [ZThreeLowCore.orderedH] using hH
  ...
  · have hTail : ∀ q < 6,
    ...
    have hSame : ∀ i : Fin 5,
      ...
      omega
    have hRows := simpleRowsOrdered_true G C L hG hPB hNoRoot hPCard
  ...
  · norm_num [externalOrbit, Bool.and_eq_true] at hExternal
    ...
    have hTail : ∀ q < 5,
      ...
      simpa only [ZThreeBridge.all_eq_true_iff] using hTailBool
    have hDefSame : ∀ _i : Fin 1,
        directCount G C.Z (L.p 0).1 = directCount G C.Z (L.p 1).1 := by
      intro _i
    ...
    have hSatSame : ∀ i : Fin 4,
        directCount G C.Z (L.p ⟨2 + i.val, by omega⟩).1 =
          directCount G C.Z (L.p ⟨2 + i.val + 1, by omega⟩).1 := by
      intro i
      ...
      omega
    have hDefRows := simpleRowsOrdered_true G C L hG hPB hNoRoot hPCard
      hHCard hZCard 0 1 (by omega)
      (by simpa [L] using
        (canonicalKeyBlock G C hPCard hHCard hZCard 0 1 (by omega)))
      (by simpa using hDefSame)
    have hSatRows := simpleRowsOrdered_true G C L hG hPB hNoRoot hPCard
...
theorem zThree_defectTwo_impossible
    ...
    (hDefect : 21 - edgeCount G C.P C.Z = 2) : False := by
  have hPB : C.P = C.B :=
  ...
  have hPZLe : edgeCount G C.P C.Z ≤ 21 := by
    exact (edgeCount_le_card_mul_card G C.P C.Z).trans_eq (by
      rw [hPCard, hZCard])
  have hPZ : edgeCount G C.P C.Z = 19 := by omega
  ...
  have hCore : core alpha beta bits = true := by
    simpa [bits] using ZThreeBridge.core_true G C L.p L.h L.z hG hPB hMin
      hNoSeymour hNoRoot alpha beta hAlphaBeta hPZ hPH hPP hHP
  have hTotal : (totalPToZ bits == 19) = true := by
    rw [beq_iff_eq]
    apply BitVec.eq_of_toNat_eq
    have hNat := totalPToZ_toNat G C.P C.H C.Z L.p L.h L.z hG
    rw [show (totalPToZ bits).toNat = edgeCount G C.P C.Z by
      simpa [bits] using hNat, hPZ]
    decide
  have hRows : orderedExternalRows bits = true := by
    have := ZThreeLowBridge.orderedExternalRows_true G C hG hPB hNoRoot
  ...
  have hCombined :
    ...
    · have hLabel0 : simpleLabels 0 bits = true := by
        simpa [bits, L] using
          (simpleLabels_of_externalOrbit G C hG hPB hNoRoot hPCard hHCard
  ...
  · have hUnsat := ZThreeSimpleLabels.alphaZeroBetaZero_unsat bits
    rw [hWitness] at hUnsat
    exact Bool.noConfusion hUnsat
  · have hUnsat := ZThreeSimpleLabels.alphaZeroBetaOne_unsat bits
    rw [hWitness] at hUnsat
    exact Bool.noConfusion hUnsat
  · have hUnsat := ZThreeSimpleLabels.alphaOneBetaZero_unsat bits
    rw [hWitness] at hUnsat
    exact Bool.noConfusion hUnsat
  · omega

end SeymourEight.BSevenKTwo.RSeven.XFourNoRoot.ZThreeMtwoBridge
