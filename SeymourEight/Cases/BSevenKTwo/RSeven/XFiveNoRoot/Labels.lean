...
namespace SeymourEight.BSevenKTwo.RSeven.XFiveNoRoot.Labels
...
def zKey (p : Fin 7 → V) (v : V) : Nat := ∑ i, if G.Adj (p i) v then 1 else 0
...
noncomputable def canonicalLabels (C : G.LocalConfiguration)
  ...
  let eZ0 := finsetEquivFin C.Z hZCard
...
theorem canonicalLabels_p_order (C : G.LocalConfiguration)
    ...
    (hA1Card : C.A1.card = 2) (hXCard : C.X.card = 5)
    (hHCard : C.H.card = 7) (hRZero : C.R = ∅)
    (hZCard : C.Z.card = 3) (q : Fin 6) :
    let L := canonicalLabels G C hPCard hACard hA1Card hXCard hHCard hRZero hZCard
...
theorem canonicalLabels_z_order (C : G.LocalConfiguration)
  ...
  simpa only [canonicalLabels] using sorted_key_anti
    (zKey G (fun i => ((sortedFinsetEquiv (pKey G C) C.P
      (finsetEquivFin C.P hPCard)) i).1)) C.Z (finsetEquivFin C.Z hZCard)
...
end SeymourEight.BSevenKTwo.RSeven.XFiveNoRoot.Labels
