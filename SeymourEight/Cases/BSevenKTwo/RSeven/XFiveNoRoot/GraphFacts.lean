...
import SeymourEight.Cases.BSevenKTwo.RSeven.XFiveNoRoot.Labels
import SeymourEight.Cases.BSevenKTwo.RSeven.XThreeNoRoot.GraphFacts

set_option linter.style.header false
set_option maxRecDepth 10000

namespace SeymourEight.BSevenKTwo.RSeven.XFiveNoRoot.GraphFacts
...
private abbrev graphBits (L : Labels G C) : Core.Encoding :=
...
theorem directCount_graphBits_toNat (C : G.LocalConfiguration) (L : Labels G C)
    ...
    (hRoot : edgeCount G C.P {C.s} = 0) (source : Nat) (hs : source < 15) :
...
theorem localSecondCount_le_graph (C : G.LocalConfiguration) (L : Labels G C)
  ...
  have hFiltered := XThreeNoRoot.GraphFacts.count_le_filterCard (V := V)
    (retainedVertexSet G C) (retainedLabelEquiv G C L)
...
theorem nonSeymour_graphBits_true (C : G.LocalConfiguration) (L : Labels G C)
  ...
  rw [directCount_graphBits_toNat G C L hG hPB hRoot source hs]
...
end SeymourEight.BSevenKTwo.RSeven.XFiveNoRoot.GraphFacts
