...
/-!
# Finite core for the `r = 7`, `x = 3` families

The layout retains `A[8]`, `P[7]`, and five slots for `Z`.  The parameter
`zCount` activates the first three, four, or five `Z` columns, so the three
...
def orientedPH (bits : Encoding) : Bool :=
  all 7 fun p => all 5 fun h => !(pToH bits p h && hToP bits h p)
...
def individualEffectiveLowerFour (bits : Encoding) (p : Nat) : BitVec 8 :=
  ...
  else if m == 7 then effectiveAtRowSize s 4 5 5 5 5
  else if m == 8 then effectiveAtRowSize s 3 5 5 5 5
  else if m == 9 then effectiveAtRowSize s 2 4 5 5 5
  else effectiveAtRowSize s 1 4 4 4 4
...
def individualEffectiveLowerThree (bits : Encoding) (p : Nat) : BitVec 8 :=
  ...
  else if m == 3 then effectiveAtRowSize s 7 7 7 7 7
...
def individualEffectiveLower (zCount : Nat) (bits : Encoding)
    (p : Nat) : BitVec 8 :=
  if zCount = 3 then individualEffectiveLowerThree bits p
  else if zCount = 4 then individualEffectiveLowerFour bits p
...
def hDefect (bits : Encoding) : BitVec 8 := totalHToP bits - 21

def xThreeScalarPresolve (bits : Encoding) : Bool :=
  let m := externalMissing 5 bits
...
def xEligible (bits : Encoding) (x : Nat) : Bool :=
  aOut bits (3 + x) + aPOut bits (3 + x) == 8 &&
    aArc bits (3 + x) 0 && aArc bits (3 + x) 6 && aArc bits (3 + x) 7

def eligibleCount (bits : Encoding) : BitVec 8 := count 3 (xEligible bits)

def xReachedInH (bits : Encoding) (x target : Nat) : Bool :=
  decide (x ≠ target) &&
    (aArc bits (3 + x) (3 + target) || any 5 fun middle =>
...
def eligibleStructure (bits : Encoding) : Bool :=
  (3 + hMissing bits).ule (hDefect bits + eligibleCount bits) &&
    all 3 (eligibleAdmissible bits) &&
    all 3 fun x => !xEligible bits x || xDeletionExpands bits x
...
def orderedStructuralClasses (bits : Encoding) : Bool :=
  (aPOut bits 2).ule (aPOut bits 1) &&
    all 2 (fun x => (aPOut bits (4 + x)).ule (aPOut bits (3 + x))) &&
    (aPOut bits 7).ule (aPOut bits 6)
...
def fiveCore (bits : Encoding) : Bool :=
  ...
  (21 : BitVec 8).ule (totalHToP bits) &&
  (totalHToP bits + externalMissing 5 bits).ule 35

def smallCore (zCount : Nat) (bits : Encoding) : Bool :=
  commonCore zCount bits && all 7 (pEffectiveCondition zCount bits) &&
    (21 : BitVec 8).ule (totalHToP bits) &&
    (totalHToP bits + externalMissing zCount bits).ule (7 * zCount)
