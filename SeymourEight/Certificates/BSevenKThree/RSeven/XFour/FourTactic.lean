...
macro "solve_four_box" : tactic =>
  `(tactic|
    simp (config := { maxSteps := 1000000000 }) only
      [boxLeaf, symmetricCore, commonCore, ordered, SymmetricCore.orderedP,
      pInvariantKey, pIn, hToPIn, SymmetricCore.orderedStructuralClasses,
      hInvariantKey, hIncidenceCode, SymmetricCore.orderedZ, zIncidenceCode,
      Core.degreeAndDualConditions, Core.sharpKing, Core.sharpKingLower,
      Core.pEffectiveCondition, Core.individualEffectiveLower,
      Core.effectiveAtRowSize, Core.etaH, Core.alpha, Core.crossMissing,
      Core.internalMissing, Core.externalMissing, Core.aMissing,
      Core.totalHOut, Core.totalPOut, Core.totalHToP, Core.totalPToH,
      Core.totalPToZ, Core.pMinimumDegree, Core.aMinimumAndDegree,
      Core.inactiveZZero, Core.allZReached, Core.everyXReached,
      Core.orientedPH, Core.orientedP, Core.orientedA,
